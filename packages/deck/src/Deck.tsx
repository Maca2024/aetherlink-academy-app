...
function PresenterTools({slide, next, presence, onPlanBToggle}: {readonly slide: DeckSlide; readonly next?: DeckSlide; readonly presence?: ReactNode; readonly onPlanBToggle?: () => void}) {
  const [elapsed, setElapsed] = useState(0); const [remaining, setRemaining] = useState(typeof slide.timer === 'number' ? slide.timer * 60 : 0); const [running, setRunning] = useState(false);
  useEffect(() => { setRunning(false); setRemaining(typeof slide.timer === 'number' ? slide.timer * 60 : 0); }, [slide.id, slide.timer]);
  ...
  return <aside className="presenter-tools deck-presenter-tools">{presence && <div className="presence deck-presence" aria-label="Participants">{presence}</div>}<p>Elapsed {fmt(elapsed)}</p><p>Assignment timer {fmt(remaining)} <button type="button" onClick={() => setRunning((value) => !value)} disabled={remaining === 0}>{running ? 'Pause' : 'Start'}</button> <button type="button" onClick={() => { setRunning(false); setRemaining((typeof slide.timer === 'number' ? slide.timer : 0) * 60); }}>Reset</button></p><section><h2>Facilitator notes</h2><p>{text(slide.notes) || 'No facilitator notes on this slide.'}</p></section>{slide.prompt && <section><h2>Exact prompt</h2><textarea readOnly value={slide.prompt} aria-label="Copy-ready example prompt"/><button type="button" onClick={() => void navigator.clipboard?.writeText(text(slide.prompt))}>Copy prompt</button></section>}<p>Next slide: {next ? next.title : 'This is the last slide.'}</p>{(visual(slide).planB || slide.planB) && <section><h2>Plan B</h2><p>{text(slide.planB)}</p><button type="button" onClick={onPlanBToggle}>Toggle Plan B</button></section>}</aside>;
}
...
function DeckContent({slides, index, revealStep, mode, onIndexChange, onRevealStepChange, presence}: DeckProps) {
  ...
  return <div className={`academy-deck mode-${mode} deck-type-${type}${activeSlide.dark ? " spotlight" : ""}`} data-type={type} data-index={index} data-total={total}>
