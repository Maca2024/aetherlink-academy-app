...
export function createApp({dir,repository,presence,proofBase='http://127.0.0.1:4400',root=process.cwd(),hostKey,publicBaseUrl=process.env.ACADEMY_PUBLIC_URL||`http://127.0.0.1:${process.env.PORT||4317}`}={}){
 ...
 const hostFile=path.join(dir,'host-key');if(!hostKey){if(!existsSync(hostFile))writeFileSync(hostFile,secret(),{mode:0o600});hostKey=readFileSync(hostFile,'utf8').trim();}
 ...
 const buckets=new Map();app.use(['/game','/mcp'],(req,res,next)=>{const k=req.ip;const b=buckets.get(k)||{t:Date.now(),n:0};if(Date.now()-b.t>60000){b.t=Date.now();b.n=0;}b.n++;buckets.set(k,b);if(b.n>1500)return res.status(429).json({error:'Te veel verzoeken. Wacht even.'});next();});
 ...
 app.post('/game/create',wrap(async(req,res)=>{const key=Buffer.from(hash(req.body.hostKey||''));if(!timingSafeEqual(key,Buffer.from(hash(hostKey))))fail(403,'Ongeldige facilitator-startsleutel.');const name=text(req.body.name,60);const p=await proof.create(initialDocument,name+' — Onze intent');setSession(res,await store.create(name,p));}));
 ...
 app.post('/game/control',wrap(async(req,res)=>res.json(await store.withSession(token(req),'browser',({r,s})=>{if(s.personId!=='facilitator')fail(403,'Alleen de facilitator bedient de ronde.');Store.prototype.control.call({remaining:store.remaining,save(){}},r,req.body.action,req.body.value);return store.view(r,s);})))) ;
 ...
 app.post('/game/quiz',wrap(async(req,res)=>{if(!Array.isArray(req.body.answers)||req.body.answers.length!==3||req.body.answers.some(x=>!Number.isInteger(x)||x<0||x>2))fail(400,'Beantwoord de drie vragen.');res.json(await store.withSession(token(req),'browser',({p})=>{if(!p)fail(400,'Alleen deelnemers.');const score=req.body.answers.filter((a,i)=>a===[1,0,2][i]).length;p.quiz={score,at:new Date().toISOString()};p.route=score<=1?'guided':score===2?'standard':'stretch';return {score,route:p.route,note:'Voorlopige hulpkeuze op basis van 3 scenario’s; geen vaardigheidsbewijs of permanent label.'};}));}));
 ...
 app.use((e,_req,res,_next)=>res.status(e.status||500).json({error:e.status?e.message:'Onverwachte serverfout. Probeer opnieuw; je invoer blijft staan.'}));
