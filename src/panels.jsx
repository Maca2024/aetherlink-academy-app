...
export function Lesson({room,action,busy}){
  const t=useT();
  const [pack,setPack]=useState(null);
  const [error,setError]=useState('');
  const [answers,setAnswers]=useState({});
  const [result,setResult]=useState(null);
  useEffect(()=>{let active=true;setPack(null);setError('');setAnswers({});setResult(null);api('day-pack').then(d=>{if(active)setPack(d);}).catch(e=>{if(active)setError(e.message);});return()=>{active=false;};},[room.day]);
  ...
  return <section className="panel content-panel"><p className="cyan">{lesson.kicker}</p><h2>{lesson.title}</h2><p className="lede">{lesson.lede}</p><div className="learning-loop">{lesson.loop.map((s,i)=><div key={s.label}><span>0{i+1}</span><strong>{s.label}</strong><small>{s.prompt}</small></div>)}</div><div className="worked"><BookOpen size={20}/><div><h3>{t('lesson.explained')}</h3><p>{lesson.workedExample}</p></div></div><h3>{t('lesson.quickCheck')}</h3><p className="muted">{t('lesson.quizHint')}</p><form onSubmit={e=>{e.preventDefault();action(async()=>setResult(await api('quiz',{answers:questions.map((_,i)=>answers[i])})));}}>{questions.map((q,i)=><fieldset key={q.question}><legend>{i+1}. {q.question}</legend>{q.options.map((a,j)=><label className="radio" key={a}><input type="radio" name={'q'+i} required checked={answers[i]===j} onChange={()=>setAnswers({...answers,[i]:j})}/>{a}</label>)}</fieldset>)}<button className="gradient" disabled={busy||room.me.role==='Facilitator'}>{t('lesson.submit')}<ArrowRight size={17}/></button></form>{result&&<div className="notice" role="status"><strong>{result.score}/{questions.length} · {routeName(t,result.route)}</strong><p>{result.note}</p><p>{t('lesson.soloHint')}</p></div>}</section>;
}
...
export function Solo({room,action,busy,onNavigate}){
  ...
  return <section className="panel content-panel"><p className="cyan">{t('solo.meta',{minutes:mission.minutes||25,id:mission.id})}</p><h2>{mission.title}</h2><p className="lede">{mission.goal}</p>{mission.starterNote&&<p className="muted">{mission.starterNote}</p>}<div className="mission-goal"><Target size={22}/><div><h3>{t('solo.goal')}</h3><p>{mission.goal}</p></div></div><label className="route-select">{t('solo.helpAmount')}<select value={room.me.route||'standard'} disabled={room.me.role==='Facilitator'} onChange={e=>action(()=>api('route',{route:e.target.value}))}>{['guided','standard','stretch'].map(v=><option value={v} key={v}>{routeName(t,v)}</option>)}</select></label><details className="hint" open={room.me.route==='guided'}><summary>{t('solo.hintSummary')}</summary><p>{hintText||t('solo.hintFallback')}</p></details>{room.me.route==='stretch'&&<div className="notice"><strong>{t('solo.stretch')}</strong><p>{mission.stretch||t('solo.stretchFallback')}</p></div>}<h3>{t('solo.workspace')}</h3><p className="muted">{t('solo.workspaceHint')}</p><div className="files">{files.map(f=><a key={f} href={'/game/starter/'+f} download={f}><FileText size={16}/>{f}<Download size={15}/></a>)}</div><details className="hint"><summary>{t('solo.allowedSummary')}</summary><p>{allowedText} {mission.stop} {t('solo.noPush')}</p></details><button className="text-button" onClick={()=>onNavigate('coach')}><Sparkles size={17}/>{t('solo.openCoach')}</button><h3>{t('solo.submitHeading')}</h3><p className="muted">{t('solo.submitHint')}</p><form onSubmit={e=>{e.preventDefault();const form=e.currentTarget;const values=Object.fromEntries(new FormData(form));action(async()=>{await api('evidence',{...values,requestId:form.dataset.requestId||(form.dataset.requestId=crypto.randomUUID())});setSent(true);form.reset();delete form.dataset.requestId;});}}>{[['finding',t('solo.finding')],['command',t('solo.command')],['observed',t('solo.observed')],['limitation',t('solo.limitation')]].map(([n,l])=><label key={n}>{l}<textarea required name={n} maxLength={n==='command'?1000:4000}/></label>)}<button className="gradient" disabled={busy||room.me.role==='Facilitator'}>{t('solo.submit')}<ArrowRight size={17}/></button>{sent&&<p className="success" role="status"><Check size={16}/>{t('solo.sent')}</p>}</form></section>;
}
...
export function Review({room,action,busy}){
  ...
  return <section className="panel content-panel"><p className="cyan">{t('review.eyebrow')}</p><h2>{t('review.title')}</h2><p className="lede">{t('review.lede')}</p>{criteria?.length>0&&<div className="notice"><strong>{t('review.criteria')}</strong><ul className="criteria-list">{criteria.map(c=><li key={c}>{c}</li>)}</ul></div>}<SuggestionReview room={room} action={action} busy={busy}/><h3>{t('review.evidenceHeading',{count:room.evidence.length})}</h3>{!room.evidence.length&&<div className="empty"><FileText size={30}/><p>{t('review.empty')}</p><small>{criteria?.length?t('review.emptyCriteria'):t('review.emptyDefault')}</small></div>}{room.evidence.map(e=><article className="evidence" key={e.id}><div><strong>{e.name}</strong><span>{statusLabel[e.status]||e.status}</span></div><small>{e.source}{e.day!=null?` · ${t('review.day',{day:e.day})}`:''} · {new Date(e.at).toLocaleString(locale==='nl'?'nl-NL':'en-GB')}</small><p>{e.finding}</p><pre>{e.command}{'\n'}{e.observed}</pre><p><strong>{t('review.limitation')}</strong> {e.limitation}</p>{e.review&&<p><strong>{t('review.reviewLabel')}</strong> {e.review.note}</p>}{['Driver','Facilitator'].includes(room.me.role)&&e.personId!==room.me.id&&<form onSubmit={event=>{event.preventDefault();const form=event.currentTarget;const values=Object.fromEntries(new FormData(form));action(async()=>{await api('review',{id:e.id,...values,requestId:form.dataset.requestId||(form.dataset.requestId=crypto.randomUUID())});delete form.dataset.requestId;});}}><label>{t('review.note')}<textarea name="note" required maxLength={4000}/></label><div className="form-row"><select aria-label={t('review.decision')} name="status"><option value="accepted">{t('review.accepted')}</option><option value="needs-work">{t('review.needsWork')}</option></select><button disabled={busy}>{t('review.save')}</button></div></form>}</article>)}<h3>{t('review.handoffTitle')}</h3><form onSubmit={e=>{e.preventDefault();const form=e.currentTarget;const data=Object.fromEntries(new FormData(form));action(async()=>{await api('handoff',{...data,requestId:form.dataset.requestId||(form.dataset.requestId=crypto.randomUUID())});delete form.dataset.requestId;setSent(true);});}}>{[['decision',t('review.decisionField')],['checked',t('review.checkedField')],['open',t('review.openField')]].map(([n,l])=><label key={n}>{l}<textarea name={n} required maxLength={4000}/></label>)}<button className="gradient" disabled={busy||!['Driver','Facilitator'].includes(room.me.role)}>{t('review.handoffSubmit')}<ArrowRight size={17}/></button>{sent&&<p className="success" role="status">{t('review.handoffSaved')}</p>}</form>{room.handoffs.map(h=><div className="notice" key={h.id}><strong>{t('review.nextOwner',{next:h.next})}</strong><p>{h.decision}</p><small>{t('review.openLabel',{open:h.open})}</small></div>)}</section>;
}
