...
function ProgressivePath({steps,compact=false}){
  ...
  return <div className={compact?'progressive-path compact':'progressive-path'} aria-label={t('path.aria')}>{steps.map((s,i)=><div className="progressive-step" key={s.id||i}><span className="agent-badge">{s.agentCount===0?'0':s.agentCount===1?'1':'multi'}</span><strong>{s.title}</strong><p>{s.goal}</p>{!compact&&s.doneWhen&&<small><span className="muted">{t('path.doneWhen')}</span> {s.doneWhen}</small>}{compact&&s.hint&&<small className="muted">{s.hint}</small>}</div>)}</div>;
}
