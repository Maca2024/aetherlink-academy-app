...
function fixture(allowedDomain='allowed.example'){
 ...
 const sso=createGoogleSso({clientId:'client-id',clientSecret:'client-secret',allowedDomains:allowedDomain,publicUrl:'https://academy.example.test',fetchImpl});
 const claims=overrides=>({iss:'https://accounts.google.com',aud:'client-id',sub:'google-subject',email:'facilitator@allowed.example',email_verified:true,name:'Ada Facilitator',hd:'allowed.example',nonce:'expected-nonce',iat:now(),exp:now()+600,...overrides});
 return {sso,issue,claims,setToken:value=>{token=value;}};
}
...
for(const [name,overrides] of [['wrong aud',{aud:'other-client'}],['expired',{exp:now()-301}],['wrong nonce',{nonce:'other-nonce'}],['unverified email',{email_verified:false}],['disallowed hd',{hd:'blocked.example'}]])test(`id_token rejects ${name}`,async()=>{const f=fixture();await assert.rejects(f.sso.verifyIdToken(f.issue(f.claims(overrides)),{nonce:'expected-nonce'}),error=>error.code===(name==='disallowed hd'?'domain':'verify'));});
for(const [name,header] of [['alg none',{alg:'none'}],['HS256 signed with the client secret',{alg:'HS256'}],['missing kid',{kid:undefined}]])test(`id_token rejects ${name}`,async()=>{const f=fixture();await assert.rejects(f.sso.verifyIdToken(f.issue(f.claims(),header),{nonce:'expected-nonce'}),error=>error.code==='verify');});
