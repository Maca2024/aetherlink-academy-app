...
async function getStrictLiveClientCount(slug: string): Promise<number> {
...
export async function applyAgentEditV2(
  ...
  let doc = await getCanonicalReadableDocument(slug, 'state') ?? (await getDocumentBySlug(slug));
  if (!doc) {
  ...
  if (!usingAuthoritativeFallback) {
    let storedBlocks = (await listLiveDocumentBlocks(doc.doc_id));
  ...
  if (nextMarkdown === authoritativeMarkdown) {
    return (await finalizeAgentEditV2Response(slug, by, authoritativeMarkdown, marks, doc.revision));
  }
  if (singleWriterMode) {
    ...
    const committed = mutation.document ?? (await getDocumentBySlug(slug));
    if (!committed) {
      return {
        status: 500,
        body: {
  ...
  return (await finalizeAgentEditV2Response(slug, by, nextMarkdown, marks, mutation.document.revision));
}
