...
async function getPresentedSecret(req: Request, slug?: string | null): Promise<string | null> {
  ...
  if (resolvedSlug) {
    ...
    if (trimmedQueryToken) {
      const roleFromQuery = (await resolveDocumentAccessRole(resolvedSlug, trimmedQueryToken));
      if (roleFromQuery) return trimmedQueryToken;
    }
    if (trimmedCookie) {
      const roleFromCookie = (await resolveDocumentAccessRole(resolvedSlug, trimmedCookie));
...
async function storeIdempotentMutationResult(
  ...
  if (status >= 200 && status < 300) {
    (await completeMutationReservation(replay.reservation, body, status, {
      mutationRoute,
      subsystem: 'agent_routes',
      slug,
    }));
  } else {
    (await releaseMutationReservation(replay.reservation, {
      mutationRoute,
      subsystem: 'agent_routes',
      slug,
...
async function releaseIdempotentMutationResult(
  replay: IdempotencyReplayResult,
  mutationRoute: string,
  slug: string,
  reason: string,
...
async function resolveAgentIdentity(
  ...
  slug: string,
  body: Record<string, unknown>,
): Promise<{ id: string; name: string; color?: string; avatar?: string }> {
...
async function buildAutoAgentId(req: Request, slug: string): Promise<string> {
  const userAgent = (req.header('user-agent') || 'unknown-agent').trim().toLowerCase();
  const token = (await getPresentedSecret(req, slug)) || '';
...
async function prepareRewriteCollabBarrier(
  ...
  try {
    ...
    (await invalidateLoadedCollabDocument(slug));
    throw error;
  }
}
...
async function checkAuth(
  ...
  const secret = (await getPresentedSecret(req, slug));
  const role = secret ? (await resolveDocumentAccessRole(slug, secret)) : null;
...
async function enforceMutationPrecondition(
  ...
  const doc = (await getDocumentBySlug(slug));
  if (!doc) {
    if (replay) (await releaseIdempotentMutationResult(replay, mutationRoute, slug, 'document_not_found'));
  ...
  if (requiresProjectedMarkState(opType) && projectionStale && !hasProjectedMarkFallback(payload, mutationBase)) {
    if (replay) (await releaseIdempotentMutationResult(replay, mutationRoute, slug, 'PROJECTION_STALE'));
  ...
  if (!opPrecondition.ok) {
    if (replay) (await releaseIdempotentMutationResult(replay, mutationRoute, slug, opPrecondition.code));
...
async function ensureAgentPresenceForAuthenticatedCall(
  req: Request,
  slug: string,
  body: Record<string, unknown>,
  details: string,
  ...
  const identity = (await resolveAgentIdentity(req, slug, body));
  const fallbackName = deriveAgentNameFromUserAgent(req.header('user-agent') || '');
  const id = identity.id && identity.id !== 'ai:unknown' ? identity.id : (await buildAutoAgentId(req, slug));
  ...
  if (hasExplicitAgentIdentitySignal(req, body) && id && id !== 'ai:unknown') {
    (await upgradeProvisionalAutoPresence(req, slug, id));
  }
  ...
  (await addDocumentEvent(slug, 'agent.presence', entry, id));
...
async function upgradeProvisionalAutoPresence(
  req: Request,
  slug: string,
  ...
  const provisionalId = (await buildAutoAgentId(req, slug));
...
async function findQuoteForMarkId(slug: string, markId: string): Promise<string | null> {
  const doc = (await getDocumentBySlug(slug));
...
async function extractCursorQuote(slug: string, body: Record<string, unknown>): Promise<string | null> {
  ...
  if (markId) {
    const fromMarks = (await findQuoteForMarkId(slug, markId));
    if (fromMarks) return fromMarks;
  }
  return null;
}

async function buildParticipationFromMutation(
  req: Request,
  slug: string,
  body: Record<string, unknown>,
  options?: { quote?: string | null; details?: string | null; ttlMs?: number | null },
): Promise<AgentParticipation | null> {
  ...
  if (identity.id && identity.id !== 'ai:unknown') {
    (await upgradeProvisionalAutoPresence(req, slug, identity.id));
  }
  const now = new Date().toISOString();
  ...
  const quote = (options?.quote ?? (await extractCursorQuote(slug, body))) ?? null;
      ...
      if (options?.verify) {
        ...
        const verification = options?.apply === false
          ...
          : await (async () => {
            ...
            return (await verifyCanonicalDocumentInLoadedCollab(slug, {
              markdown: targetMarkdown,
              marks: targetMarks,
              source: options.source ?? 'agent',
            }, REWRITE_COLLAB_TIMEOUT_MS));
        ...
        if (!confirmed && options?.fallbackBarrier) {
          ...
          try {
            const refreshed = (await getDocumentBySlug(slug));
            if (!refreshed) {
              (await invalidateLoadedCollabDocument(slug));
              return { confirmed: false, reason: 'missing_document' };
            }
...
agentRoutes.post('/bug-reports', async (req: Request, res: Response) => {
  ...
  const evidence = (await buildBugReportEvidence(validation.report));
  traceServerIncident({
    requestId,
...
agentRoutes.post('/bug-reports/:issueNumber/follow-up', async (req: Request, res: Response) => {
  ...
  const evidence = (await buildBugReportFollowUpEvidence(validation.followUp));
  traceServerIncident({
    requestId,
...
agentRoutes.get('/:slug/state', async (req: Request, res: Response) => {
  ...
  const role = (await checkAuth(req, res, slug, ['viewer', 'commenter', 'editor', 'owner_bot']));
  if (!role) return;
  (await ensureAgentPresenceForAuthenticatedCall(req, slug, {}, 'state.read'));
...
agentRoutes.post('/:slug/quarantine', async (req: Request, res: Response) => {
  ...
  if (!(await checkAuth(req, res, slug, ['owner_bot']))) return;
  ...
  const note = typeof req.body?.note === 'string' && req.body.note.trim()
    ...
    : null;
  const requestId = readRequestId(req);
  const result = (await activateDurableCollabQuarantine(slug, {
    reason,
    source: 'agent_route',
    details: {
      by: requestedBy,
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, 200, responseBody));
...
agentRoutes.get('/:slug/snapshot', async (req: Request, res: Response) => {
  ...
  if (!(await checkAuth(req, res, slug, ['viewer', 'commenter', 'editor', 'owner_bot']))) return;
...
agentRoutes.post('/:slug/edit/v2', async (req: Request, res: Response) => {
  ...
  if (!(await checkAuth(req, res, slug, ['editor', 'owner_bot']))) return;
  const editV2Body = isRecord(req.body) ? req.body : {};
  (await ensureAgentPresenceForAuthenticatedCall(req, slug, editV2Body, 'edit.v2'));
  ...
  const result = await applyAgentEditV2(slug, req.body, {
    ...
    onCommitted: async (committed) => {
      ...
      (await storeIdempotentMutationResult(replay, mutationRoute, slug, committed.status, committed.body));
    },
  });
  if (result.status >= 200 && result.status < 300 && isRecord(result.body)) {
    const participation = (await buildParticipationFromMutation(req, slug, editV2Body, { details: 'edit.v2' }));
    ...
    (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  } else {
    (await releaseIdempotentMutationResult(replay, mutationRoute, slug, 'invalid_result_body'));
  }
...
agentRoutes.post('/:slug/edit', async (req: Request, res: Response) => {
  ...
  (await ensureAgentPresenceForAuthenticatedCall(req, slug, body, 'edit.request'));
  const routeKey = mutationRoute;
  const replay = await maybeReplayIdempotentMutation(req, res, slug, mutationRoute, routeKey);
  if (replay.handled) return;
  const failEdit = async (
    status: number,
    body: Record<string, unknown>,
    reason: string,
    retryWithState?: string,
  ): Promise<void> => {
    (await releaseIdempotentMutationResult(replay, mutationRoute, slug, reason));
    sendMutationResponse(
      res,
      status,
      body,
  ...
  if (operations.length === 0) {
    (await failEdit(400, { success: false, error: 'operations must be a non-empty array', code: 'INVALID_OPERATIONS' }, 'INVALID_OPERATIONS'));
    return;
  }
  if (operations.length > 50) {
    (await failEdit(400, { success: false, error: 'Too many operations (max 50)', code: 'INVALID_OPERATIONS' }, 'INVALID_OPERATIONS'));
    return;
  }
  ...
  const doc = (await getDocumentBySlug(slug));
  if (!doc) {
    (await failEdit(404, { success: false, error: 'Document not found' }, 'document_not_found'));
    return;
  }
  const mutationBase = await resolveRouteMutationBase(slug);
  const collabRuntime = getCollabRuntime();
  ...
  const collabClientBreakdown = collabRuntime.enabled
    ? (await getActiveCollabClientBreakdown(slug))
    : null;
  ...
  for (let i = 0; i < operations.length; i++) {
    ...
    if (!isRecord(op) || typeof op.op !== 'string') {
      (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `Invalid operation at index ${i}` }, 'INVALID_OPERATIONS'));
      return;
    }
    const kind = op.op;
    if (kind === 'append') {
      if (typeof op.section !== 'string' || typeof op.content !== 'string') {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `append requires section + content (index ${i})` }, 'INVALID_OPERATIONS'));
        return;
      }
      if (op.content.length > 200_000) {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `content too large (index ${i})` }, 'INVALID_OPERATIONS'));
        return;
      }
    ...
    if (kind === 'replace') {
      if (typeof op.content !== 'string') {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `replace requires content (index ${i})` }, 'INVALID_OPERATIONS'));
        return;
      }
      if (op.content.length > 200_000) {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `content too large (index ${i})` }, 'INVALID_OPERATIONS'));
      ...
      if (Object.prototype.hasOwnProperty.call(op, 'target')) {
        ...
        if (!parsedTarget.ok) {
          (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `${parsedTarget.error} (index ${i})` }, 'INVALID_OPERATIONS'));
          return;
        }
    ...
    if (kind === 'insert') {
      ...
      if (typeof op.content !== 'string') {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `insert requires content (index ${i})` }, 'INVALID_OPERATIONS'));
        return;
      }
      if (op.content.length > 200_000) {
        (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `content too large (index ${i})` }, 'INVALID_OPERATIONS'));
    ...
    (await failEdit(400, { success: false, code: 'INVALID_OPERATIONS', error: `Unknown op: ${JSON.stringify(kind)} (index ${i})` }, 'INVALID_OPERATIONS'));
    return;
  }
  ...
  let updated = (await getDocumentBySlug(slug));
  ...
  const participation = (await buildParticipationFromMutation(req, slug, body, { quote: quoteFromOps, details }));
  ...
  if (singleWriterMode) {
    ...
    updated = mutationResult.document ?? (await getDocumentBySlug(slug));
    if (!updated) {
      (await failEdit(500, { success: false, error: 'Document update persisted but could not be reloaded' }, 'document_reload_failed'));
      return;
    }
    ...
    if (!mutation.ok) {
      const latest = (await getDocumentBySlug(slug));
      (await failEdit(
        mutation.status,
        {
          success: false,
  ...
  (await addDocumentEvent(slug, 'agent.edit', { by, operations: parsedOps }, by));
...
agentRoutes.post('/:slug/presence', async (req: Request, res: Response) => {
  ...
  if (agentId && agentId !== 'ai:unknown') {
    (await upgradeProvisionalAutoPresence(req, slug, agentId));
  }
  ...
  (await addDocumentEvent(slug, 'agent.presence', entry, agentId));
...
agentRoutes.post('/:slug/presence/disconnect', async (req: Request, res: Response) => {
  const slug = getSlug(req);
  if (!slug) {
    res.status(400).json({ success: false, error: 'Invalid slug' });
    return;
  ...
  (await addDocumentEvent(slug, 'agent.disconnected', activity, actor));
  broadcastToRoom(slug, {
    type: 'agent.presence',
    source: 'agent',
    timestamp: now,
...
agentRoutes.post('/:slug/ops', async (req: Request, res: Response) => {
  ...
  (await ensureAgentPresenceForAuthenticatedCall(req, slug, participationBody, 'ops.request'));
  const requestId = readRequestId(req);
  ...
  if (!opRoute) {
    (await failOps(400, { success: false, error: 'Unsupported operation payload' }, 'unsupported_operation_payload'));
    return;
  }

  let rewriteGate: Awaited<ReturnType<typeof evaluateRewriteLiveClientGateWithOptions>> | null = null;
  ...
  if (op === 'rewrite.apply') {
    ...
    if (rewriteValidationError) {
      (await failOps(400, { success: false, error: rewriteValidationError }, 'invalid_rewrite_payload'));
      return;
    }
    rewriteGate = await evaluateRewriteLiveClientGateWithOptions(slug, payload, {
      route: mutationRoute,
      requestId,
    });
    if (rewriteGate.blocked) {
      ...
      (await failOps(409, rewriteBlockedResponseBody(rewriteGate, slug), 'rewrite_blocked_live_clients', `/api/agent/${slug}/state`));
      return;
    }
    ...
    try {
      await prepareRewriteCollabBarrier(slug, {
        validateWhileLocked: async () => {
          const lockedDoc = (await getDocumentBySlug(slug)) ?? mutationContext.doc;
      ...
      if (error instanceof RewriteBarrierPreconditionError) {
        const latestDoc = (await getDocumentBySlug(slug)) ?? mutationContext.doc;
        (await failOps(
          error.status,
          {
            success: false,
      ...
      (await failOps(503, rewriteBarrierFailedResponseBody(slug, reason), `rewrite_barrier_failed:${reason}`, `/api/agent/${slug}/state`));
      return;
    }
  }
  ...
  if (result.status >= 200 && result.status < 300 && op !== 'rewrite.apply') {
    await notifyCollabMutation(
      slug,
      (await buildParticipationFromMutation(req, slug, participationBody, {
...
agentRoutes.post('/:slug/marks/comment', async (req: Request, res: Response) => {
  ...
  if (!(await checkAuth(req, res, slug, ['commenter', 'editor', 'owner_bot']))) return;
  const routeKey = mutationRoute;
  const replay = await maybeReplayIdempotentMutation(req, res, slug, mutationRoute, routeKey);
  if (replay.handled) return;
  ...
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'comment.add' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/suggest-replace', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'suggestion.add.replace' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/suggest-insert', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'suggestion.add.insert' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/suggest-delete', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'suggestion.add.delete' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/accept', async (req: Request, res: Response) => {
  ...
  if (result.status >= 200 && result.status < 300) {
    const collabStatus = await notifyCollabMutation(
      slug,
      (await buildParticipationFromMutation(req, slug, payload, { details: 'suggestion.accept' })),
      {
        verify: true,
        source: 'marks.accept',
        stabilityMs: EDIT_COLLAB_STABILITY_MS,
    ...
    if (!collabStatus.confirmed) {
      ...
      (await storeIdempotentMutationResult(replay, mutationRoute, slug, 409, failureBody));
      sendMutationResponse(
        res,
        409,
        failureBody,
...
agentRoutes.post('/:slug/marks/reject', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'suggestion.reject' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/reply', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'comment.reply' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/resolve', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'comment.resolve' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/marks/unresolve', async (req: Request, res: Response) => {
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, result.status, result.body));
  if (result.status >= 200 && result.status < 300) {
    notifyCollabMutation(slug, (await buildParticipationFromMutation(req, slug, payload, { details: 'comment.unresolve' })), { apply: false });
  }
  sendMutationResponse(res, result.status, result.body, { route: mutationRoute, slug });
});
...
agentRoutes.post('/:slug/rewrite', async (req: Request, res: Response) => {
  ...
  const rewriteValidationError = validateRewriteApplyPayload(payload);
  if (rewriteValidationError) {
    (await failRewrite(400, { success: false, error: rewriteValidationError }, 'invalid_rewrite_payload'));
    return;
  }
  const rewriteGate = await evaluateRewriteLiveClientGateWithOptions(slug, payload, {
    route: mutationRoute,
    requestId: readRequestId(req),
  });
  res.setHeader('X-Proof-Agent-Routes', '1');
  ...
  if (rewriteGate.blocked) {
    ...
    (await failRewrite(409, rewriteBlockedResponseBody(rewriteGate, slug), 'rewrite_blocked_live_clients', `/api/agent/${slug}/state`));
    return;
  }
  console.warn('[agent-routes] rewrite allowed in hosted runtime', {
    slug,
  ...
  try {
    await prepareRewriteCollabBarrier(slug, {
      validateWhileLocked: async () => {
        const lockedDoc = (await getDocumentBySlug(slug)) ?? doc;
    ...
    if (error instanceof RewriteBarrierPreconditionError) {
      const latestDoc = (await getDocumentBySlug(slug)) ?? doc;
      (await failRewrite(
        error.status,
        {
          success: false,
    ...
    (await failRewrite(503, rewriteBarrierFailedResponseBody(slug, reason), `rewrite_barrier_failed:${reason}`, `/api/agent/${slug}/state`));
    return;
  }
  ...
  (await storeIdempotentMutationResult(replay, mutationRoute, slug, responseStatus, responseBody));
...
agentRoutes.post('/:slug/repair', async (req: Request, res: Response) => {
  ...
  const projectionRow = result.ok ? (await getDocumentProjectionBySlug(slug)) : null;
...
agentRoutes.get('/:slug/events/pending', async (req: Request, res: Response) => {
  ...
  const events = (await listDocumentEvents(slug, Number.isFinite(after) ? Math.max(0, after) : 0, Number.isFinite(limit) ? limit : 100));
...
agentRoutes.post('/:slug/events/ack', async (req: Request, res: Response) => {
  ...
  const acked = (await ackDocumentEvents(slug, Math.trunc(upToId), by));
...
agentRoutes.use(async (req: Request, res: Response) => {
  ...
  if (!(await checkAuth(req, res, slug, ['viewer', 'commenter', 'editor', 'owner_bot']))) return;
  if (routeRequiresMutation(method, path)) {
      const role = (await resolveDocumentAccessRole(slug, (await getPresentedSecret(req, slug)) ?? ''));
  ...
  if (routeRequiresMutation(method, path) && result.status >= 200 && result.status < 300) {
    notifyCollabMutation(
      slug,
      (await buildParticipationFromMutation(req, slug, asPayload(req.body), { details: `${method} ${path}` })),
      { apply: false },
    );
  }
  if (routeRequiresMutation(method, path)) {
