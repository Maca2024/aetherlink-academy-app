...
export function stabilizeAnchorTarget(
  ...
  const localTarget: AnchorTarget = {
    anchor,
    mode: resolved.mode,
    ...(before ? { contextBefore: before.split('\n').at(-1)! } : {}),
    ...(after ? { contextAfter: after.split('\n')[0]! } : {}),
  };
  const localMatch = resolveAnchorTarget(view.logical, localTarget, {
    failClosedDuplicates: true,
    stripAuthoredSpans: false,
  });
  if (localMatch.ok
    && localMatch.selection.logicalStart === resolved.selection.logicalStart
    && localMatch.selection.logicalEnd === resolved.selection.logicalEnd) {
    return localTarget;
  }
  return {
    anchor,
