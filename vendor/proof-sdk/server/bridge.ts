...
bridgeRouter.use(async (req: Request, res: Response) => {
  ...
  if (method === 'POST' && canonicalBridgePath === '/rewrite') {
    ...
    const rewriteDoc = (await getDocument(slug));
    if (!rewriteDoc) {
    ...
    const rewriteGate = await evaluateRewriteLiveClientGateWithOptions(slug, requestBody, {
  ...
  if (policy.auth === 'bridge-token') {
    ...
    const role = token ? (await resolveDocumentAccessRole(slug, token)) : null;
    if (role !== 'owner_bot') {
      traceServerIncident({
        slug,
        subsystem: 'bridge',
