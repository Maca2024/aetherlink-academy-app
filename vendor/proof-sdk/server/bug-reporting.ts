...
async function buildEvidenceForContext(input: {
  slug: string | null;
  requestId: string | null;
  occurredAt: string | null;
  capturedAt: string;
  ...
  rawEvidence: BugReportRawEvidence[];
  reporterEvents: BugReportReporterEvent[];
}): Promise<Omit<BugReportEvidenceBundle, 'report' | 'labels' | 'fixerBrief'> & { labels?: string[] }> {
  const window = computeEvidenceWindow(input);
  const requestScopedEvents = input.requestId
    ? normalizeServerIncidentEvents((await listServerIncidentEventsByRequestId(input.requestId, MAX_SERVER_INCIDENT_EVENTS)))
    : [];
  const slugScopedEvents = input.slug && window
    ? normalizeServerIncidentEvents((await listServerIncidentEventsInTimeRange(input.slug, window.from, window.to, MAX_SERVER_INCIDENT_EVENTS)))
    : [];
  ...
  const documentEvents = input.slug && window
    ? normalizeDocumentEvents((await listDocumentEventsInTimeRange(input.slug, window.from, window.to)).slice(0, MAX_DOCUMENT_EVENTS))
    : [];
...
export async function buildBugReportEvidence(report: NormalizedBugReport): Promise<BugReportEvidenceBundle> {
  const evidence = (await buildEvidenceForContext({
    slug: report.slug,
...
export async function buildBugReportFollowUpEvidence(followUp: NormalizedBugReportFollowUp): Promise<BugReportFollowUpEvidenceBundle> {
  const evidence = (await buildEvidenceForContext({
    slug: followUp.slug,
