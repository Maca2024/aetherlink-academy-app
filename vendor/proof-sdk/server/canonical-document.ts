...
async function shouldDeferOnDemandProjectionRepair(
  slug: string,
  ...
  const recentLeases = await getRecentCollabSessionLeaseCount(slug, accessEpoch);
...
async function buildVerifiedHealthyProjectionReadableDocument(
  slug: string,
  canonicalRow: DocumentRow,
  projection: Awaited<ReturnType<typeof getDocumentProjectionBySlug>>,
...
async function waitForHostedLiveLeaseMaterialization(
  slug: string,
): Promise<Awaited<ReturnType<typeof getActiveCollabClientBreakdown>>> {
  let breakdown = await getActiveCollabClientBreakdown(slug);
  ...
  while (Date.now() < deadline) {
    ...
    breakdown = await getActiveCollabClientBreakdown(slug);
...
async function buildPersistedCanonicalState(doc: DocumentRow): Promise<PersistedCanonicalState> {
  const ydoc = new Y.Doc();
  const { latestYStateVersion, snapshot, updates } = await getDb().transaction(async () => {
    await getDb().query('SELECT slug FROM documents WHERE slug = $1 FOR SHARE', [doc.slug]);
    const latestYStateVersion = await getLatestYStateVersion(doc.slug);
    const snapshot = await getLatestYSnapshot(doc.slug);
    const updates = snapshot
      ? await getYUpdatesAtOrAfter(doc.slug, snapshot.version)
      : await getYUpdatesAfter(doc.slug, 0);
    return { latestYStateVersion, snapshot, updates };
  });
  if (snapshot) {
    try {
      Y.applyUpdate(ydoc, snapshot.snapshot);
    } catch (error) {
      const fallbackDoc = await quarantineCorruptPersistedYjsState(doc.slug, {
...
async function persistCanonicalProjectionRow(
  slug: string,
  markdown: string,
  marks: Record<string, unknown>,
  revision: number,
...
export async function mutateCanonicalDocument(args: CanonicalMutationArgs): Promise<CanonicalMutationResult> {
  const doc = await getDocumentBySlug(args.slug);
  ...
  let collabClientBreakdown = await getActiveCollabClientBreakdown(args.slug);
  ...
  if (strictLiveDocRequested && activeCollabClients === 0) {
    await noteStaleEpochBypassAdmission(
      args.slug,
      'canonical_mutation',
      args.source,
  ...
  try {
    ...
    if (args.guardPathologicalGrowth !== false) {
      ...
      if (runawayGuard.blocked) {
        await pauseDocumentAndPropagate(args.slug, args.source);
    ...
    if (deltaUpdate.byteLength > 0) {
      ...
      if (!(persistedPreview.markdownMatches && persistedPreview.marksMatch)) {
        ...
        if (!persistedPreview.safety.safe) {
          await maybeFastQuarantineProjectionPathology(args.slug, {
            source: 'persist',
          ...
          await queueProjectionRepair(args.slug, 'canonical_mutation_persist_preview_mismatch');
          await invalidateLoadedCollabDocumentAndWait(args.slug);
        }
        return {
          ok: false,
          status: 409,
    ...
    Y.applyUpdate(ydoc, deltaUpdate, canonicalTransactionOrigin(args.source));
    ...
    await getDb().transaction(async () => {
      const locked = await getDb().query(
        'SELECT revision, access_epoch, share_state FROM documents WHERE slug = $1 FOR UPDATE',
        [args.slug],
      );
      const current = locked.rows[0];
      if (!current || current.revision !== doc.revision || current.access_epoch !== doc.access_epoch
        || !['ACTIVE', 'PAUSED'].includes(String(current.share_state))
        || await getLatestYStateVersion(args.slug) !== persistedState.yStateVersion) {
        throw new Error('STALE_BASE');
      }
      if (deltaUpdate.byteLength > 0) {
        nextYStateVersion = await appendYUpdate(args.slug, deltaUpdate, args.source);
        await updateYStateBlob(args.slug, Y.encodeStateAsUpdate(persistedCandidateDoc));
        const latestSnapshot = await getLatestYSnapshot(args.slug);
        const updatesSinceSnapshot = latestSnapshot ? (nextYStateVersion - latestSnapshot.version) : nextYStateVersion;
        const bytesSinceSnapshot = await getAccumulatedYUpdateBytesAfter(args.slug, latestSnapshot?.version ?? 0);
        if (updatesSinceSnapshot >= compactionEvery || bytesSinceSnapshot >= compactionMaxBytes) {
          await saveYSnapshot(args.slug, nextYStateVersion, Y.encodeStateAsUpdate(persistedCandidateDoc));
          await pruneObsoleteYHistory(args.slug, nextYStateVersion);
        }
      }
      ...
      await persistCanonicalProjectionRow(args.slug, authoritativeNextMarkdown, effectiveNextMarks, nextRevision, nextYStateVersion, now);
    });

    const updated = await getDocumentBySlug(args.slug);
    if (!updated) {
    ...
    await noteDocumentIntegrityWarning(args.slug, {
      actor: args.source,
    ...
    await refreshSnapshotForSlug(args.slug);
    ...
    if (error instanceof OversizedYjsUpdateError) {
      await quarantineOversizedYjsUpdate(args.slug, {
        bytes: error.bytes,
...
export async function recoverCanonicalDocumentIfNeeded(
  ...
  if ((await getLiveCollabBlockStatus(slug)).active) {
    return current;
  }
  ...
  if (await shouldDeferOnDemandProjectionRepair(slug, source)) {
    ...
    const healthyProjection = canonicalRow
      ? await buildVerifiedHealthyProjectionReadableDocument(slug, canonicalRow, await getDocumentProjectionBySlug(slug))
      : null;
  ...
  const recovery = (async (): Promise<CanonicalReadableDocument | DocumentRow | undefined> => {
    ...
    if (repair.ok) {
      ...
      const repairedProjection = await getDocumentProjectionBySlug(slug);
      if (
        repairedProjection
    ...
    await queueProjectionRepair(slug, reason);
    const fallback = await getCanonicalReadableDocument(slug, readSource) ?? await getDocumentBySlug(slug);
    if (
      fallback
...
export async function executeCanonicalRewrite(
  ...
  const eventId = await addDocumentEvent(slug, 'document.rewritten', {
    by,
...
export async function repairCanonicalProjection(
  ...
  const expectedYStateVersion = await getLatestYStateVersion(slug);
  ...
  try {
    const projectionBeforeRepair = await getDocumentProjectionBySlug(slug);
    ...
    const { updated, yStateVersion } = await getDb().transaction(async () => {
      const locked = await getDb().query(
        'SELECT revision, access_epoch FROM documents WHERE slug = $1 FOR UPDATE', [slug],
      );
      if (!locked.rows[0] || locked.rows[0].revision !== doc.revision
        || locked.rows[0].access_epoch !== doc.access_epoch) {
        throw new Error('Document changed during projection repair');
      }
      const yStateVersion = await getLatestYStateVersion(slug);
      if (yStateVersion !== expectedYStateVersion) {
        throw new Error('Collaborative state changed during projection repair');
      }
      const replaced = await replaceDocumentProjection(slug, derived.markdown, derived.marks, yStateVersion);
      if (!replaced) {
        throw new Error('Projection missing after projection repair');
      }
      await persistCanonicalProjectionRow(
        slug,
        derived.markdown,
        derived.marks,
        doc.revision,
      ...
      return { updated: await getDocumentBySlug(slug), yStateVersion };
    });
    if (!updated) {
...
export async function cloneFromCanonical(slug: string, actor: string = 'system'): Promise<CanonicalRepairResult & { cloneSlug?: string; ownerSecret?: string }> {
  ...
  try {
    ...
    const updatedClone = await getDb().transaction(async () => {
      const clone = await createDocument(
        cloneSlug,
        repair.markdown,
      ...
      if (authoritativeSnapshot.byteLength > 0) {
        await saveYSnapshot(cloneSlug, nextYStateVersion, authoritativeSnapshot);
        await pruneObsoleteYHistory(cloneSlug, nextYStateVersion);
      }
      await getDb().prepare(`
        UPDATE documents
        SET markdown = $1, marks = $2, y_state_version = $3
      ...
      return await getDocumentBySlug(cloneSlug);
    });
    if (!updatedClone) {
      return {
        ok: false,
    ...
    await refreshSnapshotForSlug(cloneSlug);
    return {
      ok: true,
