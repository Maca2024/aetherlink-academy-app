import { getDocumentBySlug, type DocumentRow } from './db-postgres.js';
...
async function shouldQueueProjectionRepairAfterMutation(
  slug: string,
  activeCollabClients: number,
): Promise<boolean> {
...
export async function applySingleWriterMutation(
  ...
  return withSlugQueue(slug, async () => {
    ...
    if (policy.queueProjectionRepair && (await shouldQueueProjectionRepairAfterMutation(slug, activeCollabClients))) {
      (await queueProjectionRepair(slug, reason ?? 'markdown_mismatch'));
    }
