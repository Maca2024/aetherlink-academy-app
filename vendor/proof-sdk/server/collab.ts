...
import { runOutsideDatabaseTransaction, appendYUpdate, bumpGlobalCollabAdmissionEpoch, bumpDocumentAccessEpoch, clearPersistedGlobalCollabAdmissionGuard, clearYjsState, countActiveCollabConnectionsForInstance, OversizedYjsUpdateError, getDocumentAuthStateBySlug, getDocumentBySlug, getDocumentProjectionBySlug, getAccumulatedYUpdateBytesAfter, getPersistedGlobalCollabAdmissionGuard, getProjectedDocumentBySlug, getDb, getLatestYUpdate, getLatestYStateVersion, getLatestYSnapshot, listActiveCollabConnectionSlugs, listRecentDocumentLiveCollabLeaseSlugs, getYStateBlob, pruneObsoleteYHistory, updateYStateBlob, getYUpdatesAtOrAfter, getYUpdatesAfter, listDocsWithStaleProjection, listSuspiciousProjectionCandidates, noteDocumentLiveCollabLease, replaceDocumentProjection, saveYSnapshot, setDocumentProjectionHealth, removeActiveCollabConnection, upsertActiveCollabConnection, upsertPersistedGlobalCollabAdmissionGuard, updateDocument, type DocumentProjectionRow, type DocumentRow, type ProjectedDocumentRow, } from './db-postgres.js';
import { recordCollabAdmissionGuard, recordCollabPathologyQuarantine, recordCollabSessionBuildLatency, recordCollabLogSuppressed, recordStaleOnStoreDrop, recordProjectionChars, recordProjectionDrift, recordFragmentCacheMismatch, recordProjectionGuardBlock, recordProjectionLag, recordProjectionMarkedStale, recordProjectionRepair, recordProjectionReadFallback, recordProjectionWipe, recordCanonicalSyncFailure, recordCanonicalSyncRecoveryFailure, recordCanonicalSyncRefusal, recordLegacyReverseFlowBlocked, recordPersistedYjsUpdateBytes, recordLegacyReseedAttempt, recordSuspiciousDocBlocked, } from './metrics.js';
...
import { getHeadlessMilkdownParser, getHeadlessMilkdownParserIfReady, parseMarkdownWithHtmlFallback, serializeMarkdown, summarizeParseError, warmHeadlessMilkdown, getWarmHeadlessMilkdownParserSync, warmHeadlessMilkdownParserInBackground, } from './milkdown-headless.js';
import { extractAuthoredMarksFromDoc, extractAuthoredMarksFromMarkdown, synchronizeAuthoredMarks, } from './proof-authored-mark-sync.js';
...
function setInterval(callback: (...args: any[]) => void, delay?: number, ...args: any[]): NodeJS.Timeout {
    return runOutsideDatabaseTransaction(() => scheduleInterval(callback, delay, ...args));
}
...
export type DegradedCollabReadTraceSurface = 'state' | 'share_json' | 'recovery' | 'open_context' | 'collab_refresh' | 'collab_session' | 'collab_auth';
...
export type LiveCollabBlockCode = 'HOT_SLUG_QUARANTINED' | 'COLLAB_ADMISSION_GUARDED' | 'COLLAB_AUTO_QUARANTINED';
...
async function isDurablyCollabQuarantined(slug: string): Promise<boolean> {
    return isAutoCollabQuarantined(slug) || (await hasPersistedProjectionQuarantine(slug));
}
async function isCollabQuarantined(slug: string): Promise<boolean> {
    return isHotSlugQuarantined(slug) || (await isDurablyCollabQuarantined(slug));
}
...
async function hasPersistedProjectionQuarantine(slug: string): Promise<boolean> {
    if (!slug)
        return false;
    return (await getProjectedDocumentBySlug(slug))?.projection_health === 'quarantined';
}
async function getPersistedProjectionQuarantineReason(slug: string): Promise<string | null> {
    if (!slug)
        return null;
    return (await getProjectedDocumentBySlug(slug))?.projection_health_reason ?? null;
}
...
async function getGlobalCollabAdmissionGuardEntry(): Promise<GlobalCollabAdmissionGuardEntry | null> {
    const now = Date.now();
    const inMemory = globalCollabAdmissionGuard && globalCollabAdmissionGuard.untilMs > now
        ? globalCollabAdmissionGuard
        : null;
    const persisted = (await getPersistedGlobalCollabAdmissionGuard());
...
async function countDurablyQuarantinedCollabDocuments(): Promise<number> {
    const now = Date.now();
...
function getGlobalCollabAdmissionGuardScopedSlugSet(entry: GlobalCollabAdmissionGuardEntry | null): {
    present: boolean;
    slugs: Set<string>;
} {
...
function getGlobalCollabAdmissionGuardRequiredAdmissionEpoch(entry: GlobalCollabAdmissionGuardEntry | null): number | null {
...
function getDocumentCollabBootstrapEpoch(doc: Pick<DocumentRow, 'collab_bootstrap_epoch'> | null | undefined): number {
...
async function isDocFreshForGlobalCollabAdmissionGuard(slug: string): Promise<boolean> {
    if (!slug)
        return false;
    const entry = (await getGlobalCollabAdmissionGuardEntry());
    const requiredAdmissionEpoch = getGlobalCollabAdmissionGuardRequiredAdmissionEpoch(entry);
    if (requiredAdmissionEpoch === null)
        return false;
...
async function snapshotGlobalCollabAdmissionGuardScopedSlugs(observedAtMs: number): Promise<string[]> {
    ...
    for (const slug of (await listActiveCollabConnectionSlugs(observedAtIso))) {
        slugs.add(slug);
    }
    for (const slug of (await listRecentDocumentLiveCollabLeaseSlugs(observedAtIso))) {
        slugs.add(slug);
    }
...
function buildGlobalCollabAdmissionGuardScopeDetails(scopedSlugs: string[]): {
    scopedSlugs: string[];
...
async function fenceScopedSlugsForGlobalCollabAdmissionGuard(scopedSlugs: string[]): Promise<{
    ...
    for (const slug of scopedSlugs) {
        if (!slug)
            continue;
        const nextAccessEpoch = (await bumpDocumentAccessEpoch(slug));
...
async function shouldApplyGlobalCollabAdmissionGuardToSlug(slug: string): Promise<boolean> {
    if (!slug)
        return true;
    return !(await isDocFreshForGlobalCollabAdmissionGuard(slug));
}
export async function getGlobalCollabAdmissionGuardStatus(): Promise<{
    active: boolean;
    reason: string | null;
    untilMs: number | null;
    remainingMs: number | null;
...
export async function getCollabQuarantineGateStatus(slug: string): Promise<{
    active: boolean;
    reason: string | null;
    untilMs: number | null;
    remainingMs: number | null;
    ...
    if ((await hasPersistedProjectionQuarantine(slug))) {
        const reason = (await getPersistedProjectionQuarantineReason(slug));
        return {
            active: true,
...
export async function getLiveCollabBlockStatus(slug: string): Promise<LiveCollabBlockStatus> {
    ...
    const admissionGuardEntry = (await getGlobalCollabAdmissionGuardEntry());
    if (admissionGuardEntry && (await shouldApplyGlobalCollabAdmissionGuardToSlug(slug))) {
    ...
    const quarantine = (await getCollabQuarantineGateStatus(slug));
...
type ShutdownFlushSkipReason = 'access_epoch_mismatch' | 'auto_quarantined' | 'invalidated' | 'read_only' | 'rewrite_locked' | 'share_state_blocked' | 'stale_generation' | 'superseded_loaded_doc';
...
function traceCollabStartupIncident(level: IncidentTraceLevel, eventType: string, reason: string, data: Record<string, unknown> = {}): void {
    traceServerIncident({
        subsystem: 'collab',
        level,
        eventType,
...
const persistOperations = new Map<Promise<void>, string>();
const persistOperationContext = new AsyncLocalStorage<{ operation?: Promise<void> }>();
const invalidationOperations = new Map<string, { promise: Promise<void>; clearPersistedState: boolean }>();
class PersistInvalidatedError extends Error {}
...
let persistPauseHookForTests: ((context: {
    slug: string;
...
const DEFAULT_COLLAB_COMPACTION_MAX_BYTES = 500000;
...
const DEFAULT_STARTUP_STALE_PROJECTION_RECONCILE_DELAY_MS = 30000;
const DEFAULT_STARTUP_STALE_PROJECTION_RECONCILE_LIMIT = 25;
const DEFAULT_PROJECTION_GUARD_MAX_CHARS = 1500000;
...
const DEFAULT_PROJECTION_GUARD_MAX_SMALL_BASELINE_GROWTH_CHARS = 12000;
...
const DEFAULT_STALE_ONSTORE_DB_MISSING_QUARANTINE_BYTES = 1500000;
const DEFAULT_STALE_ONSTORE_LOCAL_UNSAVED_QUARANTINE_BYTES = 1500000;
...
const DEFAULT_COLLAB_ADMISSION_GUARD_MAX_BYTES = 4000000;
...
const DEFAULT_PROJECTION_REPAIR_RETRY_SCHEDULE_MS = [0, 500, 2000];
const DEFAULT_PROJECTION_REPAIR_WORKER_ENABLED = true;
const DEFAULT_PROJECTION_REPAIR_WORKER_DELAY_MS = 45000;
const DEFAULT_PROJECTION_REPAIR_WORKER_INTERVAL_MS = 120000;
const DEFAULT_PROJECTION_REPAIR_WORKER_LIMIT = 10;
const DEFAULT_PROJECTION_REPAIR_WORKER_MIN_CHARS = 500000;
const DEFAULT_PROJECTION_REPAIR_WORKER_OVERSIZED_COOLDOWN_MS = 15 * 60 * 1000;
const DEFAULT_DOCUMENT_LIVE_COLLAB_LEASE_HEARTBEAT_MS = 15000;
...
const projectionRepairWorkerOversizedSeen = new Map<string, {
    fingerprint: string;
    queuedAt: number;
}>();
...
const REWRITE_LOCK_COOLDOWN_MS = 5000; // keep lock for 5s after rewrite completes
...
const ACTIVE_COLLAB_INSTANCE_ID = (process.env.RAILWAY_REPLICA_ID
    || process.env.RAILWAY_DEPLOYMENT_ID
    || process.env.HOSTNAME
    || `pid-${process.pid}-${randomUUID()}`).trim();
        ...
        doc.transact(() => {
            ...
            for (const key of Array.from(presenceMap.keys())) {
                ...
                if (!normalizedKey || !normalizedValueId || normalizedKey !== normalizedValueId) {
                    ...
                    if (typeof key === 'string' && key.trim())
                        removedPresenceIds.add(key.trim());
                    if (typeof value?.id === 'string' && value.id.trim())
                        removedPresenceIds.add(value.id.trim());
                    continue;
                }
            ...
            for (const key of Array.from(cursorMap.keys())) {
                ...
                if (!normalizedKey || !normalizedValueId || normalizedKey !== normalizedValueId || removedPresenceIds.has(normalizedKey)) {
                    ...
                    if (typeof key === 'string' && key.trim())
                        removedCursorIds.add(key.trim());
                    if (typeof value?.id === 'string' && value.id.trim())
                        removedCursorIds.add(value.id.trim());
                    continue;
                }
                const atRaw = value?.at;
...
function mergeAgentPresence(existing: unknown, incoming: AgentPresenceEntry): AgentPresenceEntry {
    ...
    try {
        ydoc.transact(() => {
            ...
            const currentAt = (current && typeof current === 'object' && !Array.isArray(current))
                ? (current as any).at
                : null;
            if (typeof currentAt === 'string' && currentAt !== at)
                return;
...
function extractCollabAuthToken(data: {
    token?: string;
    requestParameters: URLSearchParams;
    requestHeaders?: unknown;
}, label: 'onAuthenticate' | 'onConnect', documentName: string): string {
...
async function buildCollabPresenceContextForConnection(data: {
    documentName: string;
    requestParameters: URLSearchParams;
    requestHeaders?: unknown;
    connection: {
    ...
    if (isCollabAuthContext(data.context)) {
        ...
        return (await attachAuthenticatedCollabPresence(data.socketId ?? '', data.context));
    }
    ...
    return (await attachAuthenticatedCollabPresence(data.socketId ?? '', auth));
}
async function authenticateCollabSession(documentName: string, token: string): Promise<CollabAuthContext> {
    ...
    const authDoc = (await getDocumentAuthStateBySlug(documentName));
    ...
    const liveCollabBlock = (await getLiveCollabBlockStatus(documentName));
    ...
    if (!readableDoc) {
        const row = await getDocumentBySlug(documentName);
        if (!row) throw new Error('document-not-found');
        readableDoc = buildCanonicalRowReadableDocument(row, null);
    }
    if (!isCanonicalReadMutationReady(readableDoc)) {
        const { recoverCanonicalDocumentIfNeeded } = await import('./canonical-document.js');
        const recovered = await recoverCanonicalDocumentIfNeeded(documentName, 'share');
        if (recovered) readableDoc = await getCanonicalReadableDocument(documentName, 'share') ?? buildCanonicalRowReadableDocument(recovered, null);
    }
    const refreshedAuthDoc = (await getDocumentAuthStateBySlug(documentName));
    ...
    const refreshedLiveCollabBlock = (await getLiveCollabBlockStatus(documentName));
    ...
    const writeEligibleByRole = ((claims.role === 'owner_bot'
...
async function attachAuthenticatedCollabPresence(socketId: string, auth: CollabAuthContext): Promise<CollabPresenceContext> {
    ...
    if (typeof auth.accessEpoch === 'number' && Number.isFinite(auth.accessEpoch)) {
        (await noteDocumentLiveCollabLease(auth.slug, auth.accessEpoch));
        ...
        const heartbeatMs = parsePositiveInt(process.env.DOCUMENT_LIVE_COLLAB_LEASE_HEARTBEAT_MS, DEFAULT_DOCUMENT_LIVE_COLLAB_LEASE_HEARTBEAT_MS);
        if (heartbeatMs > 0) {
            ...
            const timer = setInterval(async () => {
                try {
                    (await noteDocumentLiveCollabLease(auth.slug, auth.accessEpoch as number));
                }
                catch {
        ...
        (await upsertActiveCollabConnection({
            connectionId,
            slug: auth.slug,
            role: auth.role,
            accessEpoch: auth.accessEpoch,
...
async function detachAuthenticatedCollabPresence(context: unknown): Promise<void> {
    const connectionId = (context
        && typeof context === 'object'
        && typeof (context as {
            activeCollabConnectionId?: unknown;
        }).activeCollabConnectionId === 'string')
        ? (context as {
            activeCollabConnectionId: string;
        }).activeCollabConnectionId
        : '';
    if (!connectionId)
        return;
    ...
    try {
        (await removeActiveCollabConnection(connectionId));
    }
    catch (error) {
...
function getWsStatusCode(error: unknown): number | undefined {
    ...
    for (const symbol of Object.getOwnPropertySymbols(error)) {
        if (symbol.description !== 'status-code')
            continue;
...
function summarizeWsError(error: unknown): {
    message: string;
    code?: string;
    statusCode?: number;
} {
    const message = error instanceof Error ? error.message : String(error ?? 'unknown socket error');
    const code = error && typeof error === 'object' && typeof (error as {
        code?: unknown;
    }).code === 'string'
        ? (error as {
            code: string;
...
function isOversizedWsError(summary: {
    message: string;
    code?: string;
    statusCode?: number;
}): boolean {
...
function buildWsOversizeSuppressionKey(request: unknown, source: string, slug: string | null): string {
    const websocketKey = readHeaderValue((request as {
        headers?: unknown;
    } | null)?.headers, 'sec-websocket-key').trim();
    const userAgent = readHeaderValue((request as {
        headers?: unknown;
    } | null)?.headers, 'user-agent').trim();
    const remoteAddress = getRequestRemoteAddress(request).trim();
    const tokenPresent = extractCollabTokenFromHeaders((request as {
        headers?: unknown;
    } | null)?.headers).trim().length > 0;
...
function buildWsOversizeSuppressionFingerprint(request: unknown, source: string, slug: string | null, summary: {
    message: string;
    code?: string;
    statusCode?: number;
}): string {
...
function logWsOversizeSuppressionSummary(slug: string | null, source: string, summary: {
    code?: string;
    statusCode?: number;
    message: string;
}, suppressedCount: number): void {
    if (!shouldEmitSuppressionSummary(suppressedCount))
        return;
...
export function logCollabSocketErrorWithSuppression(request: unknown, source: string, error: unknown): void {
    ...
    if (isOversizedWsError(summary)) {
        ...
        const cooldown = registerPathologyCooldown(collabWsOversizeCooldowns, buildWsOversizeSuppressionKey(request, source, slug), reason, buildWsOversizeSuppressionFingerprint(request, source, slug, summary), Date.now(), parsePositiveInt(process.env.COLLAB_WS_OVERSIZE_LOG_COOLDOWN_MS, DEFAULT_WS_OVERSIZE_LOG_COOLDOWN_MS));
...
function releaseCollabInvalidation(slug: string): void {
    ...
    const cooldownMs = parsePositiveInt(process.env.COLLAB_INVALIDATION_COOLDOWN_MS, DEFAULT_INVALIDATION_COOLDOWN_MS);
    if (cooldownMs <= 0) {
...
function logStaleEpochWrite(slug: string, source: string, details: Record<string, unknown>): void {
...
async function shouldDropStaleContextWrite(slug: string, context: unknown, source: 'onChange' | 'onStoreDocument' | 'durablePersistTracking'): Promise<boolean> {
...
function buildComparableAuthoritativeDoc(baselineSnapshot: Uint8Array | null | undefined, source: Y.Doc): Y.Doc {
...
function hasLegacyEphemeralCollabState(ydoc: Y.Doc): boolean {
    const share = (ydoc as {
        share?: Map<string, SharedYDocEntry>;
    }).share;
...
function encodeAuthoritativeStateAsUpdate(ydoc: Y.Doc, stateVector?: Uint8Array): Uint8Array {
...
async function markSkipNextOnStorePersistFromAuthoritativeState(slug: string, ydoc: Y.Doc, options: {
    sourceActor: string;
    ...
    if (typeof fingerprintMarkdown === 'string') {
        syncAuthoritativeMarkdownCache(fingerprintDoc, fingerprintMarkdown, `${options.sourceActor}-skip-next-fingerprint`);
    }
...
function inferLoadedDocAuthorityOrigin(slug: string, ydoc: Y.Doc, fallback: LoadedDocAuthorityOrigin = 'persisted'): LoadedDocAuthorityOrigin {
    const liveDoc = getLiveHocuspocusDoc(slug);
    if (liveDoc && liveDoc === ydoc)
        return 'live';
    const existing = loadedDocAuthorityOrigins.get(slug);
    if (existing && existing.ydoc === ydoc)
        return existing.origin;
    return fallback;
}
function rememberLoadedDoc(slug: string, ydoc: Y.Doc, origin: LoadedDocAuthorityOrigin = inferLoadedDocAuthorityOrigin(slug, ydoc)): void {
...
function ensureFragmentEditTracking(ydoc: Y.Doc): FragmentEditState {
    ...
    if (!fragmentEditListenerAttached.has(ydoc)) {
        ...
        ydoc.on('afterTransaction', (transaction: any) => {
            ...
            if (!changedParentTypes || typeof changedParentTypes.has !== 'function')
                return;
            const fragment = ydoc.getXmlFragment('prosemirror');
            if (!changedParentTypes.has(fragment))
                return;
...
function shouldIgnoreDurablePersistOrigin(origin: unknown): boolean {
    if (typeof origin !== 'string' || !origin.trim())
        return false;
    if (FRAGMENT_REPAIR_ORIGINS.has(origin))
        return true;
...
function ensureDurablePersistTracking(slug: string, ydoc: Y.Doc): void {
    if (durablePersistListenerAttached.has(ydoc))
        return;
    ...
    ydoc.on('afterTransaction', (transaction: any) => {
        ...
        if (!docChanged)
            return;
        if (shouldIgnoreDurablePersistOrigin(transaction?.origin))
            return;
        const originContext = (transaction?.origin
            && typeof transaction.origin === 'object'
            && !Array.isArray(transaction.origin)
            && 'context' in transaction.origin)
            ? (transaction.origin as {
                context?: unknown;
            }).context
            : null;
...
function shouldDropWriteDuringShutdown(slug: string, source: 'durablePersistTracking' | 'persistDoc' | 'schedulePersistDoc' | 'onStoreDocument' | 'onChange'): boolean {
    if (!isShuttingDown())
        return false;
...
function traceShutdownIncident(level: IncidentTraceLevel, eventType: string, message: string, options: {
    slug?: string | null;
...
function traceShutdownFlushSkipped(slug: string, reason: string, data: Record<string, unknown> = {}): void {
...
async function waitForPersistInFlightDrain(timeoutMs: number = 10000): Promise<void> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() <= deadline) {
        if (!hasPersistInFlightWrites() && persistInFlightPromises.size === 0 && persistOperations.size === 0)
            return;
        await new Promise((resolve) => setTimeout(resolve, 10));
    }
    const stuckSlugs = Array.from(new Set([
        ...persistInFlightPromises.keys(),
        ...persistOperations.values(),
    ...
    const error = new Error(`Timed out waiting for in-flight collab persistence to drain before snapshot flush (${stuckSlugs.join(', ') || 'unknown slugs'})`) as Error & {
        slugs?: string[];
    };
...
function getShutdownAuthoritativeBaseline(slug: string): AuthoritativeBaseline | null {
    ...
    if (!(loadedMeta?.baselineSnapshot && loadedMeta?.baselineStateVector))
        return null;
    return {
...
async function hasDirtyShutdownSignal(slug: string, ydoc: Y.Doc): Promise<boolean> {
    if (docLastChangedAt.has(slug))
        return true;
    const currentFingerprint = buildAuthoritativeDocFingerprint(ydoc);
    ...
    try {
        const persisted = (await readPersistedDocState(slug, { allowFragmentRecovery: false }));
...
function buildShutdownGuardError(slug: string, reason: ShutdownFlushSkipReason, data: Record<string, unknown> = {}): Error & {
    slug: string;
    slugs: string[];
    reason: ShutdownFlushSkipReason;
    data: Record<string, unknown>;
} {
    const error = new Error(`Refused to drop dirty collab room during shutdown (${slug}: ${reason})`) as Error & {
        slug: string;
        slugs: string[];
        reason: ShutdownFlushSkipReason;
        data: Record<string, unknown>;
...
async function maybeThrowOnDirtyShutdownGuard(slug: string, ydoc: Y.Doc, reason: ShutdownFlushSkipReason, data: Record<string, unknown> = {}): Promise<void> {
    if (!(await hasDirtyShutdownSignal(slug, ydoc)))
        return;
...
async function waitForCollabConnectionDrain(timeoutMs: number = 500): Promise<void> {
    const activeSlugs = (await listActiveCollabConnectionSlugs());
    const traceSlug = activeSlugs.length === 1 ? activeSlugs[0] : null;
    const deadline = Date.now() + timeoutMs;
    while (Date.now() <= deadline) {
        if (getActiveCollabConnectionCount() === 0)
            return;
        await new Promise((resolve) => setTimeout(resolve, 10));
    }
    let forcedClients = 0;
...
function cancelPendingPersistWork(slug: string, options?: {
...
function verifyCollabToken(token: string): CollabSessionClaims | null {
    ...
    if (!claims || typeof claims !== 'object' || Array.isArray(claims))
        return null;
    const slug = (claims as {
        slug?: unknown;
    }).slug;
    const role = (claims as {
        role?: unknown;
    }).role;
    const exp = (claims as {
        exp?: unknown;
    }).exp;
    const accessEpoch = (claims as {
        accessEpoch?: unknown;
    }).accessEpoch;
    const tokenId = (claims as {
        tokenId?: unknown;
    }).tokenId;
    const jti = (claims as {
        jti?: unknown;
    ...
    if (typeof exp !== 'number' || !Number.isFinite(exp))
        return null;
    if (typeof accessEpoch !== 'number' || !Number.isFinite(accessEpoch) || accessEpoch < 0)
        return null;
    if (tokenId !== null && typeof tokenId !== 'string')
        return null;
    if (typeof jti !== 'string' || jti.length < 6)
        return null;
...
export function getCollabSessionClaims(token: string): {
    slug: string;
    role: ShareRole;
    accessEpoch: number;
} | null {
...
function hasAuthoredMarks(marks: Record<string, unknown>): boolean {
    return Object.values(marks).some((value) => (value
        && typeof value === 'object'
        && !Array.isArray(value)
        && (value as {
            kind?: unknown;
...
function mergeMissingAuthoredMarks(incomingMarks: Record<string, unknown>, rowMarks: Record<string, unknown>): Record<string, unknown> {
    const merged = { ...incomingMarks };
    for (const [markId, value] of Object.entries(rowMarks)) {
        if (merged[markId] !== undefined)
            continue;
...
async function recoverRichProjectionSnapshot(slug: string, markdown: string, incomingMarks: Record<string, unknown>): Promise<{
    markdown: string;
...
function preferEquivalentRichYTextMarkdown(derivedMarkdown: string, yTextMarkdown: string): string {
    if (!hasRichProjectionStructure(yTextMarkdown))
        return derivedMarkdown;
    if (hasRichProjectionStructure(derivedMarkdown))
        return derivedMarkdown;
...
function shouldAllowLiveAuthoritativeSmallBaselineProjectionWrite(args: {
    ...
    safety: ProjectionSafetyDecision;
}): boolean {
    if (args.safety.reason !== 'growth_multiplier_exceeded')
        return false;
    ...
    if (baselineChars >= minBaselineChars)
        return false;
    if (absoluteGrowthChars > maxSmallBaselineGrowthChars)
        return false;
...
function canonicalRowDiffersFromPersistedState(row: DocumentRow, persistedState: PersistedDocState): {
    markdown: string;
    marks: Record<string, unknown>;
...
export type MutationBaseSource = 'projection' | 'live_yjs' | 'persisted_yjs' | 'canonical_row';
...
export type AuthoritativeMutationBaseResolution = {
    ok: true;
    base: AuthoritativeMutationBase;
} | {
    ok: false;
    reason: 'missing_document' | 'live_doc_unavailable' | 'persisted_yjs_corrupt';
};
...
function buildMutationBaseToken(markdown: string, marks: Record<string, unknown>, accessEpoch: number): string {
...
export function buildAuthoritativeMutationBase(row: DocumentRow, source: MutationBaseSource, markdown: string, marks: Record<string, unknown>): AuthoritativeMutationBase {
...
type AuthoritativeMarksMatchSource = 'live_yjs' | 'persisted_yjs' | 'persisted_yjs_corrupt' | 'canonical_row' | 'missing_document';
type AuthoritativeMarksMatchResult = {
    matches: boolean;
    source: AuthoritativeMarksMatchSource;
};
async function hasPersistedYjsAuthority(slug: string): Promise<boolean> {
    const row = (await getDocumentBySlug(slug));
    const latestYStateVersion = (await getLatestYStateVersion(slug));
    const snapshot = (await getLatestYSnapshot(slug));
    const updates = snapshot
        ? (await getYUpdatesAtOrAfter(slug, snapshot.version)) : (await getYUpdatesAfter(slug, 0));
    const snapshotIsCurrent = Boolean(snapshot && snapshot.version === latestYStateVersion);
    const compactedBlob = (await getYStateBlob(slug));
    const blobIsCurrent = Boolean(compactedBlob
        && row
        && (row.y_state_version ?? 0) > 0
        && (row.y_state_version ?? 0) === latestYStateVersion) && !snapshotIsCurrent;
...
async function loadPreexistingAuthoritativeYDocForMarksMatch(slug: string): Promise<CanonicalYDocHandle | null> {
    if (!slug)
        return null;
    const inMemory = (await getCurrentInMemoryAuthoritativeYDoc(slug));
    if (inMemory)
        return inMemory;
    if (!(await hasPersistedYjsAuthority(slug)))
        return null;
    const cacheKey = (await readPersistedDocCacheKey(slug));
    if (cacheKey) {
...
async function resolveAuthoritativeMarksMatch(slug: string, expectedMarks: Record<string, unknown>): Promise<AuthoritativeMarksMatchResult> {
    ...
    try {
        ...
        const actualMarks = normalizeMutationBaseMarks(canonicalizeStoredMarks(encodeMarksMap(handle.ydoc.getMap('marks')) as Record<string, StoredMark>) as unknown as Record<string, unknown>);
        return {
...
export async function preserveMarksOnlyWriteIfAuthoritativeYjsMatches(slug: string, expectedMarks: Record<string, unknown>): Promise<boolean> {
    const match = await resolveAuthoritativeMarksMatch(slug, expectedMarks);
    if (!match.matches)
        return false;
    if (match.source !== 'live_yjs' && match.source !== 'persisted_yjs')
        return false;
    const liveOrLoaded = (await getCurrentInMemoryAuthoritativeYDoc(slug))?.ydoc ?? null;
    if (liveOrLoaded) {
        (await refreshLoadedDocDbMetaFromDb(slug, liveOrLoaded));
        touchDoc(slug);
    }
    traceServerIncident({
        slug,
...
function sameAuthoritativeMutationBase(left: AuthoritativeMutationBase | null, right: AuthoritativeMutationBase | null): boolean {
    if (!left || !right)
        return false;
    return left.token === right.token;
}
function sameAuthoritativeContent(leftMarkdown: string | null | undefined, leftMarks: Record<string, unknown> | null | undefined, rightMarkdown: string | null | undefined, rightMarks: Record<string, unknown> | null | undefined): boolean {
...
function shouldPreserveMissingMark(value: unknown, options: {
...
export async function mergePreservedActionMarks(slug: string, incomingMarks: Record<string, unknown>, options: {
    includeSuggestions?: boolean;
    ...
    for (const [markId, value] of Object.entries(existingMarks)) {
        if (incomingMarks[markId] !== undefined)
            continue;
        if (!shouldPreserveMissingMark(value, options))
            continue;
...
async function recordProjectionWipeWarning(slug: string, previousLength: number, nextLength: number): Promise<{
    quarantined: boolean;
    reason?: string;
}> {
    if (previousLength <= 0)
    ...
    if (nextLength === 0) {
        const driftQuarantine = (await maybeQuarantineRepeatedFragmentDrift(slug, {
            source: 'materialize',
...
function evaluateNonDirtyFragmentRefreshCollapse(currentMarkdown: string, derivedFragmentMarkdown: string, currentRowMarkdown: string): FragmentRefreshCollapseGuardResult {
...
export function detectPathologicalProjectionRepeat(baselineMarkdown: string, candidateMarkdown: string, minRepeats: number = DEFAULT_PATHOLOGICAL_REPEAT_MIN_REPEATS, minBaseChars: number = DEFAULT_PATHOLOGICAL_REPEAT_MIN_BASE_CHARS): number {
    if (typeof baselineMarkdown !== 'string' || typeof candidateMarkdown !== 'string')
        return 0;
    const baseLen = baselineMarkdown.length;
    const nextLen = candidateMarkdown.length;
    if (baseLen < minBaseChars)
        return 0;
    if (nextLen < baseLen * minRepeats)
        return 0;
    if (nextLen % baseLen !== 0)
        return 0;
    const repeatCount = Math.floor(nextLen / baseLen);
    if (repeatCount < minRepeats)
        return 0;
    for (let offset = 0; offset < nextLen; offset += baseLen) {
        if (candidateMarkdown.slice(offset, offset + baseLen) !== baselineMarkdown)
            return 0;
    }
    return repeatCount;
}
...
function detectCanonicalReplayRepeatCount(canonicalMarkdown: string, candidateMarkdown: string): number {
    ...
    if (!baseline || candidate === baseline)
        return 0;
    if (baseline.length < CANONICAL_REPLAY_MIN_BASE_CHARS)
        return 0;
    let cursor = 0;
    let repeatCount = 0;
    while (cursor < candidate.length) {
        if (!candidate.startsWith(baseline, cursor))
            break;
        repeatCount += 1;
    ...
    if (repeatCount >= CANONICAL_REPLAY_MIN_REPEATS)
        return repeatCount;
    const pathologicalRepeatCount = detectPathologicalProjectionRepeat(baseline, candidate, CANONICAL_REPLAY_MIN_REPEATS, CANONICAL_REPLAY_MIN_BASE_CHARS);
    if (pathologicalRepeatCount >= CANONICAL_REPLAY_MIN_REPEATS)
        return pathologicalRepeatCount;
    const prefixLength = Math.min(CANONICAL_REPLAY_PREFIX_MAX_CHARS, Math.max(CANONICAL_REPLAY_PREFIX_MIN_CHARS, Math.floor(baseline.length / 3)));
    const prefix = baseline.slice(0, prefixLength).trim();
    if (prefix.length < CANONICAL_REPLAY_PREFIX_MIN_CHARS)
        return 0;
    ...
    const candidateMuchLonger = candidate.length >= Math.max(baseline.length + prefix.length, Math.floor(baseline.length * 1.5));
...
export function evaluateProjectionSafety(baselineMarkdown: string, candidateMarkdown: string, doc: Y.Doc): ProjectionSafetyDecision {
    const maxChars = parsePositiveInt(process.env.COLLAB_PROJECTION_GUARD_MAX_CHARS, DEFAULT_PROJECTION_GUARD_MAX_CHARS);
    const maxGrowthMultiplier = parsePositiveFloat(process.env.COLLAB_PROJECTION_GUARD_MAX_GROWTH_MULTIPLIER, DEFAULT_PROJECTION_GUARD_MAX_GROWTH_MULTIPLIER);
    const smallBaselineBypassEnabled = parseBooleanFlag(process.env.COLLAB_PROJECTION_GUARD_SMALL_BASELINE_BYPASS_ENABLED, DEFAULT_PROJECTION_GUARD_SMALL_BASELINE_BYPASS_ENABLED);
    const minBaselineChars = parsePositiveInt(process.env.COLLAB_PROJECTION_GUARD_MIN_BASELINE_CHARS, DEFAULT_PROJECTION_GUARD_MIN_BASELINE_CHARS);
    const maxSmallBaselineGrowthChars = parsePositiveInt(process.env.COLLAB_PROJECTION_GUARD_MAX_SMALL_BASELINE_GROWTH_CHARS, DEFAULT_PROJECTION_GUARD_MAX_SMALL_BASELINE_GROWTH_CHARS);
    const maxLengthDriftRatio = parsePositiveFloat(process.env.COLLAB_PROJECTION_GUARD_MAX_LENGTH_DRIFT_RATIO, DEFAULT_PROJECTION_GUARD_MAX_LENGTH_DRIFT_RATIO);
    const minTokenOverlap = parsePositiveFloat(process.env.COLLAB_PROJECTION_GUARD_MIN_TOKEN_OVERLAP, DEFAULT_PROJECTION_GUARD_MIN_TOKEN_OVERLAP);
...
async function materializeProjection(slug: string, doc: Y.Doc, options?: {
    ...
    const recoveredSnapshot = (await recoverRichProjectionSnapshot(slug, rawMarkdownText, (await mergePreservedActionMarks(slug, encodeMarksMap(marksMap)))));
    ...
    if (previousLength !== undefined) {
        wipeQuarantine = (await recordProjectionWipeWarning(slug, previousLength, markdownText.length));
    }
    ...
    if (options?.bumpRevision === false) {
        const replaced = (await replaceDocumentProjection(slug, markdownText, marks, yStateVersion, {
            health: 'healthy',
            healthReason: null,
        }));
        if (!replaced) {
        ...
        const updated = (await updateDocument(slug, markdownText, marks, yStateVersion));
        if (!updated) {
...
function getProjectionRepairRetryScheduleMs(): number[] {
    ...
    if (!raw)
        return [...DEFAULT_PROJECTION_REPAIR_RETRY_SCHEDULE_MS];
    const parsed = raw
        .split(',')
...
function registerPathologyCooldown(state: Map<string, PathologyCooldownEntry>, key: string, reason: string, fingerprint: string, nowMs: number = Date.now(), cooldownMs: number = DEFAULT_PROJECTION_PATHOLOGY_COOLDOWN_MS): ProjectionPathologyCooldownResult {
    if (!key || !fingerprint)
        return { suppressed: false, suppressedCount: 0 };
...
export function registerProjectionPathologyCooldown(state: Map<string, PathologyCooldownEntry>, slug: string, reason: string, fingerprint: string, nowMs: number = Date.now(), cooldownMs: number = parsePositiveInt(process.env.COLLAB_PROJECTION_PATHOLOGY_COOLDOWN_MS, DEFAULT_PROJECTION_PATHOLOGY_COOLDOWN_MS)): ProjectionPathologyCooldownResult {
...
function buildProjectionPathologyFingerprint(reason: string, details: Record<string, unknown> | undefined, extras: Record<string, unknown> = {}): string {
    return stableStringify({
        reason: reason || 'unknown',
...
function buildStaleOnStoreSuppressionFingerprint(reason: string, extras: Record<string, unknown> = {}): string {
    return stableStringify({
        reason: reason || 'unknown',
        extras,
    });
}
function buildStaleOnStoreDropSuppressionFingerprint(reason: string, extras: Record<string, unknown> = {}): string {
    return stableStringify({
        reason: reason || 'unknown',
        extras,
    });
...
function shouldCountTowardsCollabRepairLoopBreaker(pathology: 'empty_fragment_repair' | 'empty_fragment_projection' | 'projection_guard_block', details: Record<string, unknown>): boolean {
    if (pathology !== 'projection_guard_block')
        return true;
    const guardReason = typeof details.guardReason === 'string' ? details.guardReason : '';
    if (COLLAB_REPAIR_LOOP_GUARD_REASONS.has(guardReason))
        return true;
...
function shouldFastQuarantineProjectionGuard(reason: string, details: Record<string, unknown> | undefined): boolean {
    if (FAST_QUARANTINE_PROJECTION_GUARD_REASONS.has(reason))
        return true;
    if (!MASKED_REPEAT_PROJECTION_GUARD_REASONS.has(reason))
        return false;
...
function getFastQuarantineReasonForProjectionGuard(reason: string): string {
    if (reason === 'pathological_repeat')
        return 'projection_guard_pathological_repeat';
...
function registerFragmentDriftSuppression(slug: string): ProjectionPathologyCooldownResult {
    return registerPathologyCooldown(fragmentDriftCycleCooldowns, slug, 'fragment_markdown_drift', buildFragmentDriftSuppressionFingerprint(slug), Date.now(), parsePositiveInt(process.env.COLLAB_PROJECTION_PATHOLOGY_COOLDOWN_MS, DEFAULT_PROJECTION_PATHOLOGY_COOLDOWN_MS));
}
...
function logLocalAuthorityAdmissionSuppressionSummary(slug: string, surface: string, suppressedCount: number): void {
    if (!shouldEmitSuppressionSummary(suppressedCount))
        return;
...
function logStaleEpochBypassSuppressionSummary(slug: string, surface: string, source: string, suppressedCount: number): void {
    if (!shouldEmitSuppressionSummary(suppressedCount))
        return;
...
function noteRepairGuardEscalationPathology(slug: string, guardReason: string, fingerprint: string, cycleId: number): {
    tripped: boolean;
    count: number;
    windowMs: number;
    max: number;
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_REPAIR_GUARD_ESCALATION_WINDOW_MS, DEFAULT_COLLAB_REPAIR_GUARD_ESCALATION_WINDOW_MS);
    const max = parsePositiveInt(process.env.COLLAB_REPAIR_GUARD_ESCALATION_MAX, DEFAULT_COLLAB_REPAIR_GUARD_ESCALATION_MAX);
    const now = Date.now();
...
function noteFragmentDriftCycle(slug: string, cycleId: number): {
    tripped: boolean;
    count: number;
    windowMs: number;
    max: number;
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_FRAGMENT_DRIFT_BREAKER_WINDOW_MS, DEFAULT_FRAGMENT_DRIFT_BREAKER_WINDOW_MS);
    const max = parsePositiveInt(process.env.COLLAB_FRAGMENT_DRIFT_BREAKER_MAX, DEFAULT_FRAGMENT_DRIFT_BREAKER_MAX);
    const now = Date.now();
...
async function maybeQuarantineRepeatedFragmentDrift(slug: string, options: {
    ...
    const gate = (await getCollabQuarantineGateStatus(slug));
    if (gate.active) {
        return {
            quarantined: true,
    ...
    (await registerAutoCollabQuarantine(slug, reason, {
...
async function maybeQuarantineRepeatedRepairGuardBlock(slug: string, options: {
    source: ProjectionOperationSource;
    guardReason: string;
    details?: Record<string, unknown>;
    ...
    const existingGate = (await getCollabQuarantineGateStatus(slug));
    ...
    const cycleId = getOrStartProjectionRepairCycleId(slug);
    const cooldown = registerPathologyCooldown(collabRepairGuardLogCooldowns, slug, options.guardReason, fingerprint, Date.now(), parsePositiveInt(process.env.COLLAB_PROJECTION_PATHOLOGY_COOLDOWN_MS, DEFAULT_PROJECTION_PATHOLOGY_COOLDOWN_MS));
    if (cooldown.suppressed) {
...
export async function maybeFastQuarantineProjectionPathology(slug: string, options: {
    source: ProjectionOperationSource;
    guardReason: string;
    details?: Record<string, unknown>;
    extras?: Record<string, unknown>;
    ...
    const cooldown = registerProjectionPathologyCooldown(projectionPathologyCooldowns, slug, quarantineReason, fingerprint);
    if (!cooldown.suppressed) {
    ...
    (await registerAutoCollabQuarantine(slug, quarantineReason, {
        source: options.source,
        guardReason: options.guardReason,
        ...(options.extras ?? {}),
        ...(options.details ? { details: options.details } : {}),
...
function noteCollabRepairLoopPathology(slug: string): {
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_REPAIR_LOOP_BREAKER_WINDOW_MS, DEFAULT_COLLAB_REPAIR_LOOP_BREAKER_WINDOW_MS);
    const max = parsePositiveInt(process.env.COLLAB_REPAIR_LOOP_BREAKER_MAX, DEFAULT_COLLAB_REPAIR_LOOP_BREAKER_MAX);
    const now = Date.now();
...
async function maybeQuarantineCollabRepairLoop(slug: string, pathology: 'empty_fragment_repair' | 'empty_fragment_projection' | 'projection_guard_block', details: Record<string, unknown> = {}): Promise<{
    quarantined: boolean;
    reason?: string;
}> {
    if (!slug)
...
async function getProjectionHealthForSlug(slug: string): Promise<ProjectedDocumentRow['projection_health'] | null> {
    return (await getProjectedDocumentBySlug(slug))?.projection_health ?? null;
}
function registerLargeDocPathologyCooldown(slug: string, reason: string, fingerprint: string): ProjectionPathologyCooldownResult {
    return registerPathologyCooldown(largeDocPathologyCooldowns, `${slug}:${reason}`, reason, fingerprint, Date.now(), parsePositiveInt(process.env.COLLAB_PROJECTION_PATHOLOGY_COOLDOWN_MS, DEFAULT_PROJECTION_PATHOLOGY_COOLDOWN_MS));
}
...
function noteRepeatedSuspiciousDocPathology(state: Map<string, RepeatedSuspiciousDocState>, slug: string): {
    tripped: boolean;
    count: number;
    windowMs: number;
    max: number;
} {
    const windowMs = parsePositiveInt(process.env.COLLAB_SUSPICIOUS_DOC_REPEAT_WINDOW_MS, DEFAULT_SUSPICIOUS_DOC_REPEAT_WINDOW_MS);
    const max = parsePositiveInt(process.env.COLLAB_SUSPICIOUS_DOC_REPEAT_MAX, DEFAULT_SUSPICIOUS_DOC_REPEAT_MAX);
    const now = Date.now();
...
async function quarantineLargeDocPathology(slug: string, reason: string, source: string, details?: Record<string, unknown>): Promise<void> {
...
async function maybeQuarantineRepeatedLargeDocPathology(slug: string, kind: 'legacy_reseed' | 'pending_delta_clear', options: {
    source: string;
    details?: Record<string, unknown>;
    fingerprint?: string;
}): Promise<{
    ...
    (await quarantineLargeDocPathology(slug, reason, options.source, {
        count: repeat.count,
        windowMs: repeat.windowMs,
        max: repeat.max,
        ...(options.details ?? {}),
...
function getLargeDocWriteWindowState(slug: string, bytes: number): {
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_MAX_SLUG_YJS_WINDOW_MS, DEFAULT_MAX_SLUG_YJS_WINDOW_MS);
    const maxBytes = parsePositiveInt(process.env.COLLAB_MAX_SLUG_YJS_BYTES_PER_WINDOW, DEFAULT_MAX_SLUG_YJS_BYTES_PER_WINDOW);
    const now = Date.now();
...
async function maybeQuarantineOversizedYjsWriteBurst(slug: string, bytes: number, source: string, sourceActor?: string | null): Promise<{
    quarantined: boolean;
    reason?: string;
}> {
    if (!slug || bytes <= 0)
    ...
    (await quarantineLargeDocPathology(slug, reason, source, {
        bytes,
        totalBytes: window.totalBytes,
        maxBytes: window.maxBytes,
        count: window.count,
...
export async function quarantineOversizedYjsUpdate(slug: string, options: {
    bytes: number;
    limitBytes: number;
    source: string;
...
function classifyIntegrityWarningReason(integrity: DocumentIntegrityWarning, baseline: IntegrityWarningBaseline | null = null): string | null {
    const baselineBlockCount = baseline?.topLevelBlockCount ?? null;
    const crossedBlockExplosionThreshold = (integrity.topLevelBlockCount >= DEFAULT_INTEGRITY_WARNING_BLOCK_EXPLOSION
    ...
    if (integrity.topLevelBlockCount >= DEFAULT_INTEGRITY_WARNING_REPEAT_BLOCK_THRESHOLD
...
function buildIntegrityWarningFingerprint(reason: string, integrity: DocumentIntegrityWarning): string {
    return stableStringify({
...
function logIntegrityWarningSuppressionSummary(slug: string, reason: string, suppressedCount: number, severity: 'warning' | 'observed'): void {
    if (!shouldEmitSuppressionSummary(suppressedCount))
        return;
...
function shouldLogIntegrityObservation(integrity: DocumentIntegrityWarning, baseline: IntegrityWarningBaseline | null, severityReason: string | null): boolean {
    if (severityReason)
        return true;
    const repeatedStructureDelta = analyzeRepeatedStructureDelta(integrity, baseline);
...
export async function noteDocumentIntegrityWarning(slug: string, options: {
    actor: string;
    revision: number;
    ...
    if (shouldLog) {
        const fingerprint = buildIntegrityWarningFingerprint(severityReason ?? 'integrity_warning_observed', options.integrity);
        const cooldown = registerPathologyCooldown(integrityWarningCooldowns, slug, severityReason ?? 'integrity_warning_observed', fingerprint, Date.now(), parsePositiveInt(process.env.COLLAB_INTEGRITY_WARNING_LOG_COOLDOWN_MS, DEFAULT_INTEGRITY_WARNING_LOG_COOLDOWN_MS));
        suppressed = cooldown.suppressed;
        if (cooldown.suppressed) {
            recordCollabLogSuppressed('integrity_warning', severityReason ?? 'integrity_warning_observed');
            logIntegrityWarningSuppressionSummary(slug, severityReason ?? 'integrity_warning_observed', cooldown.suppressedCount, severityReason ? 'warning' : 'observed');
        }
    ...
    (await registerAutoCollabQuarantine(slug, severityReason, {
        source,
        actor: options.actor,
        revision: options.revision,
...
async function shouldBlockLegacyReseed(slug: string, accessEpoch: number | null = null): Promise<{
    blocked: boolean;
    reason: string | null;
}> {
    if (!slug)
    ...
    const projectionHealth = (await getProjectionHealthForSlug(slug));
    if (projectionHealth && projectionHealth !== 'healthy') {
        return { blocked: true, reason: projectionHealth };
    }
    return { blocked: false, reason: null };
}
async function shouldBlockAutomaticRepairForSuspiciousDoc(slug: string): Promise<{
    blocked: boolean;
    reason: string | null;
}> {
    if (!slug)
...
async function maybeRecordBlockedLegacyReseed(slug: string, source: string, row: DocumentRow): Promise<void> {
    const gate = (await shouldBlockLegacyReseed(slug, typeof row.access_epoch === 'number' ? row.access_epoch : null));
    if (!gate.blocked)
        return;
    const details = {
        source,
        ...
        projectionHealth: (await getProjectionHealthForSlug(slug)),
    };
    if (gate.reason === 'recent_live_collab_lease') {
        const cooldown = registerLargeDocPathologyCooldown(slug, 'legacy_reseed_recent_live_collab_lease', stableStringify({
    ...
    const quarantine = (await maybeQuarantineRepeatedLargeDocPathology(slug, 'legacy_reseed', {
        source,
        details,
...
async function deriveMarkdownProjectionFromFragment(doc: Y.Doc): Promise<string | null> {
    ...
    try {
        ...
        const root = yXmlFragmentToProseMirrorRootNode(doc.getXmlFragment('prosemirror') as any, parser.schema as any) as ProseMirrorNode;
...
let canonicalSyncPreviewPauseHookForTests: ((context: {
    slug: string;
    source: string;
    hasMarkdown: boolean;
    hasMarks: boolean;
...
function buildPersistedDocCacheKey(snapshot: {
    version: number;
    snapshot: Uint8Array;
} | null, updates: Array<{
    seq: number;
...
async function readPersistedDocCacheKey(slug: string): Promise<PersistedDocCacheKey | null> {
    const snapshot = (await getLatestYSnapshot(slug));
    const updates = snapshot
        ? (await getYUpdatesAtOrAfter(slug, snapshot.version)) : (await getYUpdatesAfter(slug, 0));
    return buildPersistedDocCacheKey(snapshot, updates);
}
function getPersistedDocCacheEntry(slug: string, key: PersistedDocCacheKey, requiredHydration: PersistedDocCacheHydration = 'sync', requiredRecoveryMode: PersistedDocRecoveryMode = 'allowed'): PersistedDocCacheEntry | null {
...
function setPersistedDocCacheEntry(slug: string, ydoc: Y.Doc, key: PersistedDocCacheKey, hydration: PersistedDocCacheHydration, recoveryMode: PersistedDocRecoveryMode, degradedReason: PersistedDocDegradationReason | null = null): void {
...
async function refreshPersistedDocCacheFromDb(slug: string, ydoc: Y.Doc, hydration: PersistedDocCacheHydration, recoveryMode: PersistedDocRecoveryMode, degradedReason: PersistedDocDegradationReason | null = null): Promise<void> {
    const key = (await readPersistedDocCacheKey(slug));
    if (!key) {
        persistedDocCache.delete(slug);
        touchDoc(slug);
        return;
...
async function resolveLoadedDocFragmentMarkdown(slug: string, ydoc: Y.Doc, options: {
    ...
    const currentMarks = (await mergePreservedActionMarks(slug, encodeMarksMap(ydoc.getMap('marks'))));
    const recoveredSnapshot = (await recoverRichProjectionSnapshot(slug, derivedMarkdown, currentMarks));
    const effectiveMarkdown = preferEquivalentRichYTextMarkdown(recoveredSnapshot.markdown, yTextMarkdown);
...
async function sampleYDocMarkdownForVerification(slug: string, ydoc: Y.Doc): Promise<{
    markdown: string | null;
    source: 'ytext' | 'fragment' | 'none';
}> {
    try {
...
async function ensureFragmentSeededFromMarkdownIfEmptySync(slug: string, ydoc: Y.Doc, sourceActor: string): Promise<boolean> {
    if ((await shouldBlockAutomaticRepairForSuspiciousDoc(slug)).blocked)
        return false;
    if (fragmentSeedAttempted.has(ydoc))
        return false;
    const fragment = ydoc.getXmlFragment('prosemirror');
    if (!isProsemirrorFragmentStructurallyEmpty(fragment))
        return false;
    ...
    if (markdown.length > 0) {
        const canonicalRow = (await getDocumentBySlug(slug));
    ...
    if ((repairMode as string) === 'blocked_rich' || isProsemirrorFragmentStructurallyEmpty(ydoc.getXmlFragment('prosemirror'))) {
        return false;
    }
    ...
    (await maybeQuarantineCollabRepairLoop(slug, 'empty_fragment_repair', {
        markdownChars: markdown.length,
        mode: repairMode,
    }));
    return true;
}
async function ensureFragmentSeededFromMarkdownIfEmpty(slug: string, ydoc: Y.Doc, sourceActor: string): Promise<boolean> {
    if ((await shouldBlockAutomaticRepairForSuspiciousDoc(slug)).blocked)
        return false;
    if (fragmentSeedAttempted.has(ydoc))
        return false;
...
async function refreshMarkdownTextFromFragment(slug: string, ydoc: Y.Doc, sourceActor: string): Promise<{
    ...
    if ((await getCollabQuarantineGateStatus(slug)).active) {
        (await setDocumentProjectionHealth(slug, 'quarantined'));
        return { deriveFailed: false, refreshed: false, markdown: null };
    }
    const fragmentState = ensureFragmentEditTracking(ydoc);
    const currentRowMarkdown = (await getDocumentBySlug(slug))?.markdown ?? '';
    const resolved = await resolveLoadedDocFragmentMarkdown(slug, ydoc, {
        allowRecovery: true,
        refreshCache: false,
        sourceActor,
    ...
    if (derivedFragmentMarkdown !== currentMarkdown) {
        ...
        // Redis can merge concurrent edits to the derived markdown cache.
        // The fragment remains authoritative; cache drift must never rebuild it.
        if (!fragmentState.dirty && !fragmentMatchesRow) {
            const suspiciousCollapse = evaluateNonDirtyFragmentRefreshCollapse(currentMarkdown, derivedFragmentMarkdown, currentRowMarkdown);
            if (suspiciousCollapse.blocked) {
                ...
                if (driftQuarantine.quarantined) {
                    ...
                    (await setDocumentProjectionHealth(slug, 'projection_stale'));
...
async function syncFragmentFromMarkdownText(slug: string, ydoc: Y.Doc, markdown: string, sourceActor: string): Promise<{
    markdown: string;
}> {
...
async function repairProjectionFromFragment(slug: string, reasons: string[], source: 'repair' | 'startup' = 'repair'): Promise<'success' | 'retry' | 'stop'> {
    const suspiciousGate = (await shouldBlockAutomaticRepairForSuspiciousDoc(slug));
    if (suspiciousGate.blocked) {
        recordProjectionRepair('skipped', reasons.join('|') || suspiciousGate.reason || 'quarantined');
        return 'stop';
    }
    const row = (await getDocumentBySlug(slug));
    const projectedRow = (await getProjectedDocumentBySlug(slug));
    ...
    const persistedState = (await readPersistedDocState(slug));
    let ydoc = persistedState.ydoc;
    if (liveDoc) {
        const comparablePersistedDoc = buildComparableAuthoritativeDoc(persistedState.authoritativeSnapshot, persistedState.ydoc);
        const comparableLiveDoc = buildComparableAuthoritativeDoc(persistedState.authoritativeSnapshot, liveDoc);
        const dbMissingLive = Y.encodeStateAsUpdate(comparablePersistedDoc, Y.encodeStateVector(comparableLiveDoc));
    ...
    if (fragmentPlain.length === 0 && rowPlain.length > 0) {
        ...
        const quarantine = (await maybeQuarantineCollabRepairLoop(slug, 'empty_fragment_projection', {
            source,
            reasons,
            rowChars: row.markdown.length,
        }));
    ...
    const recoveredSnapshot = (await recoverRichProjectionSnapshot(slug, derivedMarkdown, (await mergePreservedActionMarks(slug, encodeMarksMap(marksMap)))));
    ...
    if (effectiveDerivedMarkdown === row.markdown && marksUnchanged) {
        ...
        if (projectionNeedsHeal) {
            const replaced = (await replaceDocumentProjection(slug, row.markdown, marks, yStateVersion, {
                health: 'healthy',
                healthReason: null,
            }));
            if (!replaced) {
    ...
    if (!safety.safe) {
        ...
        const shouldReloadSmallRepairReplay = (source === 'repair'
            && reason === 'pathological_repeat'
        ...
        if (shouldReloadSmallRepairReplay) {
            ...
            const repairLoopQuarantine = (await maybeQuarantineCollabRepairLoop(slug, 'projection_guard_block', {
                source,
                guardReason: reason,
                reasons,
        ...
        const fastQuarantine = (await maybeFastQuarantineProjectionPathology(slug, {
            source,
            guardReason: reason,
            details: safety.details,
            extras: {
        ...
        const repairGuardQuarantine = (await maybeQuarantineRepeatedRepairGuardBlock(slug, {
            source,
            guardReason: reason,
            details: safety.details,
            extras: {
        ...
        if (reason === 'fragment_markdown_drift') {
            ...
            const pathology = registerProjectionPathologyCooldown(projectionPathologyCooldowns, slug, reason, fingerprint);
            ...
            (await maybeQuarantineCollabRepairLoop(slug, 'projection_guard_block', {
                source,
                guardReason: reason,
                reasons,
                details: safety.details,
    ...
    try {
        (await materializeProjection(slug, ydoc, {
            bumpRevision: false,
...
async function runQueuedProjectionRepair(slug: string): Promise<void> {
    if (projectionRepairRunning.has(slug))
        return;
    ...
    try {
        ...
        const retryTimer = setTimeout(() => {
            projectionRepairScheduled.delete(slug);
            void runQueuedProjectionRepair(slug).catch(error => console.error('[collab] projection repair failed', { slug, error: summarizeParseError(error) }));
        }, retryDelay);
        if (typeof (retryTimer as {
            unref?: () => void;
        }).unref === 'function') {
            (retryTimer as {
                unref: () => void;
...
export function queueProjectionRepair(slug: string, reason: string): void {
    if (!slug)
        return;
    if ((autoCollabQuarantines.get(slug)?.untilMs ?? 0) > Date.now())
        return;
    ...
    if (projectionRepairRunning.has(slug) || projectionRepairScheduled.has(slug))
        return;
...
function buildLegacyFallbackPersistedDocState(row: DocumentRow | null, yStateVersion: number, degradedReason: PersistedDocDegradationReason | null = null): PersistedDocState {
...
export async function quarantineCorruptPersistedYjsState(slug: string, options: {
    ...
    (await registerAutoCollabQuarantine(slug, 'corrupt_persisted_yjs_state', {
        surface: options.surface,
        stage: options.stage,
        yStateVersion,
        ...(typeof options.seq === 'number' && Number.isFinite(options.seq) ? { seq: Math.max(0, Math.trunc(options.seq)) } : {}),
    ...
    return (await getDocumentBySlug(slug)) ?? options.row ?? null;
}
async function buildCorruptPersistedYjsFallbackState(slug: string, options: {
    ...
    const refreshedRow = (await quarantineCorruptPersistedYjsState(slug, options));
    return buildLegacyFallbackPersistedDocState(refreshedRow, options.yStateVersion, 'corrupt_persisted_yjs_state');
}
...
function setPersistedDocDegradationReason(ydoc: Y.Doc, degradedReason: PersistedDocDegradationReason | null): void {
    if (degradedReason) {
...
function seedFragmentFromMarkdownSyncBestEffort(slug: string, ydoc: Y.Doc, markdown: string, sourceActor: string): {
    seeded: boolean;
    mode: 'empty' | 'warm_headless' | 'fallback' | 'blocked_rich';
} {
    const normalized = normalizeLegacyMarkdownForFragmentSeed(markdown);
...
function recoverLegacyAuthoredMarksSync(markdown: string, marks: Record<string, StoredMark>): Record<string, StoredMark> {
    ...
    if (Object.keys(extractedAuthoredMarks).length === 0)
        return marks;
    return synchronizeAuthoredMarks(marks, extractedAuthoredMarks, { preserveExistingAnchors: true });
}
async function recoverLegacyAuthoredMarks(markdown: string, marks: Record<string, StoredMark>): Promise<Record<string, StoredMark>> {
    const hasAuthoredSpans = markdown.includes('data-proof="authored"') || markdown.includes("data-proof='authored'");
    if (!hasAuthoredSpans)
        return marks;
...
async function persistCanonicalYjsBaseline(slug: string, row: DocumentRow, ydoc: Y.Doc): Promise<PersistedDocState> { return getDb().transaction(async () => {
 await getDb().query('SELECT slug FROM documents WHERE slug = $1 FOR UPDATE', [slug]);
 if (await getLatestYStateVersion(slug) > 0) return readPersistedDocStateAsync(slug);
    ...
    if (snapshot.byteLength > 0) {
        (await saveYSnapshot(slug, 1, snapshot));
        (await updateYStateBlob(slug, snapshot));
        (await pruneObsoleteYHistory(slug, 1));
        (await replaceDocumentProjection(slug, markdown, marks, 1));
    }
...
function buildEphemeralCanonicalDocState(row: DocumentRow, ydoc: Y.Doc): PersistedDocState {
    const authoritativeBaseline = buildAuthoritativeBaseline(ydoc);
    return {
        ydoc,
        updatedAt: row.updated_at ?? null,
...
async function seedLegacyDocumentToPersistedYjs(slug: string, row: DocumentRow): Promise<PersistedDocState & {
    persisted: boolean;
    seedMode: 'empty' | 'warm_headless' | 'fallback' | 'blocked_rich';
}> {
    const ydoc = new Y.Doc();
    const rawMarkdown = row.markdown ?? '';
    const markdown = stripEphemeralCollabSpans(rawMarkdown);
    const marks = recoverLegacyAuthoredMarksSync(rawMarkdown, parseStoredMarks(row.marks) as Record<string, StoredMark>);
    ...
    if ((seedMode as string) === 'blocked_rich') {
        return {
    ...
    return {
        ...(await persistCanonicalYjsBaseline(slug, row, ydoc)),
        persisted: true,
        seedMode,
    };
}
...
export async function ensureCanonicalYjsBaselineForDocument(slug: string): Promise<boolean> {
    if (!slug)
        return false;
    const row = (await getDocumentBySlug(slug));
    if (!row || row.share_state === 'DELETED' || row.share_state === 'REVOKED')
        return false;
    const snapshot = (await getLatestYSnapshot(slug));
    const updates = snapshot
        ? (await getYUpdatesAtOrAfter(slug, snapshot.version)) : (await getYUpdatesAfter(slug, 0));
    const persistedYStateVersion = (await getLatestYStateVersion(slug));
    if (snapshot || updates.length > 0) {
        if (persistedYStateVersion > 0) {
            const projection = (await getProjectedDocumentBySlug(slug));
            if ((row.y_state_version ?? 0) !== persistedYStateVersion || !projection || projection.projection_y_state_version !== persistedYStateVersion) {
                (await replaceDocumentProjection(slug, stripEphemeralCollabSpans(row.markdown ?? ''), parseStoredMarks(row.marks), persistedYStateVersion));
            }
        }
        return false;
    }
    const reseedGate = (await shouldBlockLegacyReseed(slug));
    if (reseedGate.blocked) {
        (await maybeRecordBlockedLegacyReseed(slug, 'ensure_canonical_yjs_baseline', row));
        return false;
    }
...
async function readPersistedDocState(slug: string, options: {
    allowFragmentRecovery?: boolean;
} = {}): Promise<PersistedDocState> { return readPersistedDocStateAsync(slug, options); }
async function readPersistedDocStateAsync(slug: string, options: {
    allowFragmentRecovery?: boolean;
} = {}): Promise<PersistedDocState> { return getDb().transaction(async () => { await getDb().query('SELECT slug FROM documents WHERE slug = $1 FOR UPDATE', [slug]);
    ...
    if (compactedBlob && blobIsCurrent) {
        ...
        try {
            ...
            return (await buildCorruptPersistedYjsFallbackState(slug, {
        ...
        if (allowFragmentRecovery && !(await isCollabQuarantined(slug))) {
    ...
    if (!snapshot && updates.length === 0 && row) {
        const reseedGate = (await shouldBlockLegacyReseed(slug, typeof row.access_epoch === 'number' ? row.access_epoch : null));
        if (reseedGate.blocked) {
            (await maybeRecordBlockedLegacyReseed(slug, 'read_persisted_doc_state_async', row));
...
function buildProjectionReadableDocument(projected: ProjectedDocumentRow, options: {
...
function buildCanonicalRowReadableDocument(row: DocumentRow, projected: ProjectedDocumentRow | null | undefined, options: {
    mutationReady?: boolean;
    repairPending?: boolean;
    fallbackReason?: string | null;
...
async function resolveHandleDerivedAuthority(slug: string, row: DocumentRow, handle: CanonicalYDocHandle, options: {
    allowSafeYTextFallback?: boolean;
    ...
    const marks = (await mergePreservedActionMarks(slug, encodeMarksMap(handle.ydoc.getMap('marks')), {
...
async function deriveAuthoritativeMutationBaseFromHandle(slug: string, row: DocumentRow, handle: CanonicalYDocHandle): Promise<AuthoritativeMutationBase> {
    ...
    const source: MutationBaseSource = derived.source === 'fragment'
        ...
        : 'canonical_row';
    return buildAuthoritativeMutationBase(row, source, derived.markdown, derived.marks);
}
export async function resolveAuthoritativeMutationBase(slug: string, options: {
    ...
    const latestPersistedYStateVersion = (await getLatestYStateVersion(slug));
...
export async function verifyAuthoritativeMutationBaseStable(slug: string, expectedMarkdown: string, expectedMarks: Record<string, unknown>, options: {
    liveRequired?: boolean;
    stabilityMs?: number;
    sampleMs?: number;
...
export function isCanonicalReadMutationReady(doc: {
    mutation_ready?: boolean;
...
async function getCurrentInMemoryAuthoritativeYDoc(slug: string): Promise<CanonicalYDocHandle | null> {
    if (!slug)
        return null;
    const row = (await getDocumentBySlug(slug));
    if (!row)
...
export async function loadCanonicalYDocSync(slug: string, options: {
    allowFragmentRecovery?: boolean;
    ...
    if (cacheKey) {
        const cached = getPersistedDocCacheEntry(slug, cacheKey, 'sync', allowFragmentRecovery ? 'allowed' : 'blocked');
        if (cached) {
            return {
                ydoc: cached.ydoc,
                source: 'persisted',
    ...
    const persisted = (await readPersistedDocState(slug, { allowFragmentRecovery }));
    setPersistedDocCacheEntry(slug, persisted.ydoc, cacheKey, 'sync', allowFragmentRecovery ? 'allowed' : 'blocked', persisted.degradedReason);
    return {
        ydoc: persisted.ydoc,
        source: 'persisted',
        degradedReason: persisted.degradedReason,
...
export function isProjectionFresh(doc: ProjectedDocumentRow | null | undefined): boolean {
    if (!doc)
        return false;
    if (doc.projection_revision === null || doc.projection_y_state_version === null)
        return false;
    if (doc.projection_health !== 'healthy')
        return false;
...
function projectionPayloadMatchesCanonical(doc: ProjectedDocumentRow | null | undefined): boolean {
    ...
    if (doc.markdown !== canonicalMarkdown)
        return false;
...
function getProjectionFallbackReason(doc: ProjectedDocumentRow | null | undefined): string {
    ...
    if (doc.projection_health !== 'healthy')
        return `projection_${doc.projection_health}`;
    if (doc.projection_y_state_version !== doc.y_state_version)
        return 'projection_y_state_version_mismatch';
    if (!projectionPayloadMatchesCanonical(doc))
        return 'projection_content_mismatch';
    return 'projection_unavailable';
}
function buildFallbackReadOptions(source: 'state' | 'snapshot' | 'share' | 'unknown'): {
    mutationReady?: boolean;
    repairPending: boolean;
} {
...
async function buildYjsFallbackReadableDocument(slug: string, row: DocumentRow, projected: ProjectedDocumentRow | null | undefined, handle: CanonicalYDocHandle, source: 'state' | 'snapshot' | 'share' | 'unknown', fallbackReason: string, markdownOverride?: string, marksOverride?: Record<string, unknown>, options: {
    mutationReady?: boolean;
    repairPending?: boolean;
} = {}): Promise<CanonicalReadableDocument> {
    ...
    if (fallbackSafety.reason === 'pathological_repeat') {
        if (source === 'share') {
            (await maybeFastQuarantineProjectionPathology(slug, {
                source: 'share',
    ...
    const marks = marksOverride ?? (await mergePreservedActionMarks(slug, encodeMarksMap(handle.ydoc.getMap('marks')), {
        includeSuggestions: true,
    }));
    const rowMarks = parseStoredMarks(row.marks);
    const sanitizedRowMarkdown = stripEphemeralCollabSpans(row.markdown ?? '');
    const breakdown = (await getActiveCollabClientBreakdown(slug));
    const localLiveAuthorityDecision = (await getLocalLiveAuthorityDecision(slug, handle, breakdown, { allowRecentSessionLeaseBootstrap: true }));
...
export async function getCanonicalReadableDocumentSync(slug: string, source: 'state' | 'snapshot' | 'share' | 'unknown' = 'unknown'): Promise<CanonicalReadableDocument | undefined> {
    const result = (await getCanonicalReadableDocumentSyncCore(slug, source));
    if (!result || !result.mutation_ready)
        return result;
    ...
    if ((await isDocFreshForGlobalCollabAdmissionGuard(slug))) {
        return result;
    }
    ...
    const breakdown = (await getActiveCollabClientBreakdown(slug));
    const activePresenceCount = Math.max(breakdown.exactEpochCount, breakdown.documentLeaseExactCount);
    if (activePresenceCount > 0) {
        const handle = (await loadCanonicalYDocSync(slug, { allowFragmentRecovery: false }));
        const localLiveAuthorityDecision = (await getLocalLiveAuthorityDecision(slug, handle, breakdown));
...
async function getCanonicalReadableDocumentSyncCore(slug: string, source: 'state' | 'snapshot' | 'share' | 'unknown' = 'unknown'): Promise<CanonicalReadableDocument | undefined> {
    const projected = (await getProjectedDocumentBySlug(slug));
    ...
    const row = (await getDocumentBySlug(slug));
    const handle = row
        ? (await loadCanonicalYDocSync(slug, { allowFragmentRecovery: false })) : null;
    if (projectionFresh && projectionMatchesCanonical && row && handle?.source === 'live') {
        ...
        const liveMarks = (await mergePreservedActionMarks(slug, encodeMarksMap(handle.ydoc.getMap('marks')), {
            includeSuggestions: true,
        }));
        const rowMarks = parseStoredMarks(row.marks);
        const liveMatchesRow = sameAuthoritativeContent(row.markdown ?? '', rowMarks, liveMarkdown, liveMarks);
        if (!liveMatchesRow) {
            return (await buildYjsFallbackReadableDocument(slug, row, projected, handle, source, 'live_doc_ahead', undefined, undefined, buildFallbackReadOptions(source)));
        }
    }
    ...
    if (projected && projectionFresh && row && handle) {
        ...
        const authoritativeMarks = (await mergePreservedActionMarks(slug, encodeMarksMap(handle.ydoc.getMap('marks')), {
            includeSuggestions: true,
        }));
        const projectionMatchesAuthoritative = sameAuthoritativeContent(projected.markdown, parseStoredMarks(projected.marks), authoritativeMarkdown, authoritativeMarks);
    ...
    if (row && handle) {
        ...
        const rowMarks = parseStoredMarks(row.marks);
        const yjsAheadOfCanonical = !sameAuthoritativeContent(row.markdown ?? '', rowMarks, markdown, marks);
        if (yjsAheadOfCanonical) {
            ...
            return (await buildYjsFallbackReadableDocument(slug, row, projected, handle, source, fallbackReason, undefined, undefined, buildFallbackReadOptions(source)));
        }
    }
    ...
    if (projectionFresh && !projectionMatchesCanonical) {
        ...
        if (handle && row) {
            return (await buildYjsFallbackReadableDocument(slug, row, projected, handle, source, 'projection_content_mismatch', undefined, undefined, { mutationReady: false, repairPending: true }));
        }
...
async function applyLiveDocWriteGateToReadableDocument(slug: string, result: CanonicalReadableDocument | undefined, handle?: CanonicalYDocHandle | null, options: {
    ...
    const activePresenceCount = Math.max(breakdown.exactEpochCount, breakdown.documentLeaseExactCount);
    if (activePresenceCount <= 0) {
        noteStaleEpochBypassAdmission(slug, 'write_gate_async', options.source ?? 'unknown', breakdown, 'current_epoch_cold_room');
        return result;
    }
    ...
    if (options.allowSessionBootstrapBypass === true
    ...
    if (handle && handle.source !== 'live') {
        ...
        try {
            const liveHandleDecision = (await getLocalLiveAuthorityDecision(slug, liveHandle, breakdown));
...
type LocalLiveAuthorityBranch = 'live_source' | 'persisted_live_presence' | 'persisted_recent_session_bootstrap';
type LocalLiveAuthorityDecision = {
    allowed: boolean;
    branch: LocalLiveAuthorityBranch | null;
};
async function getLocalLiveAuthorityDecision(slug: string, handle: CanonicalYDocHandle | null | undefined, breakdown: ActiveCollabClientBreakdown, options: {
    allowRecentSessionLeaseBootstrap?: boolean;
} = {}): Promise<LocalLiveAuthorityDecision> {
    if (!handle)
        return { allowed: false, branch: null };
    if (handle.source === 'live') {
        return { allowed: true, branch: 'live_source' };
    }
    if (handle.source !== 'persisted')
        return { allowed: false, branch: null };
    const hasLocalLivePresence = (await countActiveCollabConnectionsForInstance(slug, ACTIVE_COLLAB_INSTANCE_ID, breakdown.accessEpoch)) > 0;
    ...
    if (!hasLivePresenceOrBootstrapLease)
        return { allowed: false, branch: null };
    if (!hasLocalLivePresence && !bootstrapLeaseLocallyOwned)
        return { allowed: false, branch: null };
    ...
    const currentYStateVersion = (await getLatestYStateVersion(slug));
    const isFreshResidentLocalDoc = Boolean(loadedMeta
...
function noteLocalAuthorityGateAdmission(slug: string, surface: 'yjs_fallback' | 'write_gate_sync' | 'write_gate_async', decision: LocalLiveAuthorityDecision, handle: CanonicalYDocHandle | null | undefined, breakdown: ActiveCollabClientBreakdown): void {
    if (!slug || !handle || !decision.allowed || !decision.branch || decision.branch === 'live_source')
        return;
    const fingerprint = stableStringify({
    ...
    const cooldown = registerPathologyCooldown(localAuthorityAdmissionCooldowns, `${slug}:${surface}:${decision.branch}`, 'local_authority_gate_admit', fingerprint);
    if (cooldown.suppressed) {
...
type StaleEpochBypassSurface = 'write_gate_sync' | 'write_gate_async' | 'canonical_mutation' | 'rewrite_admission';
...
function normalizeStaleEpochBypassSource(surface: StaleEpochBypassSurface, source: string): NormalizedStaleEpochBypassSource {
    ...
    if (surface === 'canonical_mutation') {
        if (source === 'rest-put')
            return { source: 'rest_put', sourceDetail: null, route: null };
        return {
...
function shouldNoteStaleEpochBypass(breakdown: ActiveCollabClientBreakdown): boolean {
    if (breakdown.total > 0)
        return false;
    return (breakdown.anyEpochCount > breakdown.exactEpochCount
        || breakdown.documentLeaseAnyEpochCount > breakdown.documentLeaseExactCount);
}
export function noteStaleEpochBypassAdmission(slug: string, surface: StaleEpochBypassSurface, source: string, breakdown: ActiveCollabClientBreakdown, authorityBranch: StaleEpochBypassAuthorityBranch = 'current_epoch_cold_room'): void {
    if (!slug || !shouldNoteStaleEpochBypass(breakdown))
        return;
    ...
    const cooldown = registerPathologyCooldown(staleEpochBypassAdmissionCooldowns, `${slug}:${surface}:${normalizedSource.source}:${authorityBranch}`, 'stale_epoch_bypass_admitted', fingerprint);
    if (cooldown.suppressed) {
...
export async function getCanonicalReadableDocument(slug: string, source: 'state' | 'snapshot' | 'share' | 'unknown' = 'unknown', options: {
    preferPersisted?: boolean;
    allowSessionBootstrapBypass?: boolean;
    avoidPersistedHydrationWhenBlocked?: boolean;
} = {}): Promise<CanonicalReadableDocument | undefined> {
    const finalize = (result: CanonicalReadableDocument | undefined, gateHandle?: CanonicalYDocHandle | null) => applyLiveDocWriteGateToReadableDocument(slug, result, gateHandle, {
    ...
    const skipBlockedPersistedHydration = options.avoidPersistedHydrationWhenBlocked === true
        && source === 'share'
        && (await getLiveCollabBlockStatus(slug)).active;
    if (!row) {
    ...
    try {
        if (handle) {
            ...
            if (derived.source === 'fragment') {
                const rowMarks = parseStoredMarks(row.marks);
                const rowMatchesAuthoritative = sameAuthoritativeContent(row.markdown ?? '', rowMarks, derived.markdown, derived.marks);
                const projectionMatchesAuthoritative = Boolean(projected
                    && projectionFresh
                    && sameAuthoritativeContent(projected.markdown, parseStoredMarks(projected.marks), derived.markdown, derived.marks));
                ...
                return finalize((await buildYjsFallbackReadableDocument(slug, row, projected, handle, source, handle.source === 'live' ? 'live_doc_ahead' : 'loaded_doc_ahead', derived.markdown, derived.marks, buildFallbackReadOptions(source))), handle);
            }
            ...
            return finalize((await buildYjsFallbackReadableDocument(slug, row, projected, handle, source, derived.fallbackReason ?? 'unknown', normalizeMutationBaseMarkdown(row.markdown ?? ''), derived.marks, {
                mutationReady: false,
                repairPending: true,
            })), handle);
        }
...
export async function loadCanonicalYDoc(slug: string, options: {
    liveRequired?: boolean;
    allowFragmentRecovery?: boolean;
    preferPersisted?: boolean;
    ...
    const cacheKey = (await readPersistedDocCacheKey(slug));
    if (cacheKey) {
        const cached = getPersistedDocCacheEntry(slug, cacheKey, 'async', allowFragmentRecovery ? 'allowed' : 'blocked');
        if (cached) {
            return {
                ydoc: cached.ydoc,
                source: 'persisted',
...
export async function registerCanonicalYDocPersistence(slug: string, ydoc: Y.Doc, meta: {
    updatedAt: string | null;
    yStateVersion: number;
    accessEpoch: number | null;
}): Promise<void> {
...
async function persistDoc(slug: string, ydoc: Y.Doc, sourceActor: string = 'collab', expectedGeneration: number | null = null, options: PersistDocOptions = {}): Promise<void> {
    const context: { operation?: Promise<void> } = {};
    const operation = persistOperationContext.run(context, () => persistDocInner(slug, ydoc, sourceActor, expectedGeneration, options));
    context.operation = operation;
    persistOperations.set(operation, slug);
    try { await operation; } finally { persistOperations.delete(operation); }
}
async function persistDocInner(slug: string, ydoc: Y.Doc, sourceActor: string = 'collab', expectedGeneration: number | null = null, options: PersistDocOptions = {}): Promise<void> {
    ...
    if (isCollabPersistenceReadOnly()) {
        if (allowDuringShutdown) {
            (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'read_only'));
        }
    ...
    const docRow = (await getDocumentBySlug(slug));
    if (docRow?.share_state === 'REVOKED' || docRow?.share_state === 'DELETED') {
        if (allowDuringShutdown) {
            (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'share_state_blocked', {
    ...
    if (expectedGeneration !== null && expectedGeneration !== currentGeneration) {
        if (allowDuringShutdown) {
            (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'stale_generation', {
                expectedGeneration,
                currentGeneration,
            }));
        }
    ...
    if (sourceActor === 'collab') {
        ...
        if (currentLoadedDoc && currentLoadedDoc !== ydoc) {
            if (allowDuringShutdown) {
                (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'superseded_loaded_doc'));
            }
            persistPending.delete(slug);
            console.warn('[collab] stale collab write dropped', {
                slug,
        ...
        if (loadedMeta && typeof docRow?.access_epoch === 'number' && loadedMeta.accessEpoch !== docRow.access_epoch) {
            if (allowDuringShutdown) {
                (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'access_epoch_mismatch', {
    ...
    if (sourceActor === 'collab') {
        if ((await isCollabQuarantined(slug))) {
            if (allowDuringShutdown) {
                (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'auto_quarantined'));
            }
            persistPending.delete(slug);
            invalidateLoadedCollabDocument(slug);
            return;
        ...
        if (shouldResolveConflict) {
            const resolution = (await resolveOnStoreConflict(slug, ydoc));
    ...
    const frozenDoc = new Y.Doc();
    Y.applyUpdate(frozenDoc, Y.encodeStateAsUpdate(ydoc));
    ydoc = frozenDoc;
    const completion = (async () => {
        ...
        try {
            ...
            let authoritativeSnapshot = Y.encodeStateAsUpdate(authoritativeDoc);
            let authoritativeStateVector = Y.encodeStateVector(authoritativeDoc);
            const priorVector = priorBaseline?.stateVector;
            let deltaUpdate = priorBaseline && sameStateVector(priorBaseline.snapshot, authoritativeSnapshot)
                ? new Uint8Array()
                : priorVector
                    ? Y.encodeStateAsUpdate(authoritativeDoc, priorVector)
                    : authoritativeSnapshot;
            const compactionEvery = parsePositiveInt(process.env.COLLAB_COMPACTION_EVERY, DEFAULT_COLLAB_COMPACTION_EVERY);
            const compactionMaxBytes = parsePositiveInt(process.env.COLLAB_COMPACTION_MAX_BYTES, DEFAULT_COLLAB_COMPACTION_MAX_BYTES);
            const priorUpdateCount = updatesSinceCompaction.get(slug) ?? 0;
            let nextUpdateCount = priorUpdateCount;
            const shouldMaterializeProjection = sourceActor === 'collab' && !(await isCollabQuarantined(slug));
            let shouldBumpRevision = shouldMaterializeProjection;
            let skipPersistedStateWrite = false;
            const db = getDb();
            let aborted = false;
            const persistTx = async () => await db.transaction(async () => {
                const locked = await db.query('SELECT access_epoch, share_state FROM documents WHERE slug = $1 FOR UPDATE', [slug]);
                ...
                const snapshot = await getLatestYSnapshot(slug);
                if (snapshot) Y.applyUpdate(durable, snapshot.snapshot);
                const updates = snapshot
                    ? await getYUpdatesAtOrAfter(slug, snapshot.version)
                    : await getYUpdatesAfter(slug, 0);
                for (const update of updates) Y.applyUpdate(durable, update.update);
                if (!snapshot && updates.length === 0) {
                    const blob = await getYStateBlob(slug);
                    if (blob) Y.applyUpdate(durable, blob);
                }
                const durableVector = Y.encodeStateVector(durable);
                Y.applyUpdate(durable, deltaUpdate.byteLength ? deltaUpdate : authoritativeSnapshot);
                authoritativeDoc = durable;
                ydoc = durable;
                const mergedMarkdown = await deriveMarkdownProjectionFromFragment(durable);
                if (mergedMarkdown === null) {
                    skipProjectionWriteDueToDeriveFailure = true;
                    projectionMarkdownOverride = null;
                } else {
                    projectionMarkdownOverride = mergedMarkdown;
                    syncAuthoritativeMarkdownCache(durable, mergedMarkdown, 'server-postgres-merge');
                }
                deltaUpdate = Y.encodeStateAsUpdate(durable, durableVector);
                authoritativeSnapshot = Y.encodeStateAsUpdate(durable);
                ...
                if (!skipPersistedStateWrite && deltaUpdate.byteLength > 0) {
                    const writeBurst = (await maybeQuarantineOversizedYjsWriteBurst(slug, deltaUpdate.byteLength, 'persist', sourceActor));
                    if (writeBurst.quarantined) {
                        ...
                        try {
                            const seq = (await appendYUpdate(slug, deltaUpdate, sourceActor));
                            recordPersistedYjsUpdateBytes(deltaUpdate.byteLength, 'persist', 'accepted');
                            nextUpdateCount = priorUpdateCount + 1;
                            const latestSnapshot = (await getLatestYSnapshot(slug));
                            const bytesSinceSnapshot = (await getAccumulatedYUpdateBytesAfter(slug, latestSnapshot?.version ?? 0));
                            if (nextUpdateCount >= compactionEvery || bytesSinceSnapshot >= compactionMaxBytes) {
                                const fullSnapshot = Y.encodeStateAsUpdate(ydoc);
                                (await saveYSnapshot(slug, seq, fullSnapshot));
                                (await pruneObsoleteYHistory(slug, seq));
                                nextUpdateCount = 0;
                            }
                        }
                        catch (error) {
                            if (error instanceof OversizedYjsUpdateError) {
                                (await quarantineOversizedYjsUpdate(slug, {
                                    bytes: error.bytes,
                ...
                if (!skipPersistedStateWrite && deltaUpdate.byteLength > 0) {
                    try {
                        ...
                        (await updateYStateBlob(slug, compactedBlob));
                    }
                    catch (blobError) {
                ...
                if (shouldWriteProjection) {
                    ...
                    const yStateVersion = (await getLatestYStateVersion(slug));
                    (await db.prepare('UPDATE documents SET y_state_version = $1 WHERE slug = $2 AND share_state IN (\'ACTIVE\', \'PAUSED\')').run(yStateVersion, slug));
                }
            ...
            if (error instanceof PersistInvalidatedError) return;
...
function schedulePersistDoc(slug: string, ydoc: Y.Doc): void {
    if (shouldDropWriteDuringShutdown(slug, 'schedulePersistDoc'))
        return;
...
function registerStaleOnStoreDriftSuppression(slug: string, reason: string, extras: Record<string, unknown>): ProjectionPathologyCooldownResult {
    return registerPathologyCooldown(staleOnStoreDriftCooldowns, slug, reason, buildStaleOnStoreSuppressionFingerprint(reason, extras), Date.now(), parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_DRIFT_COOLDOWN_MS, DEFAULT_STALE_ONSTORE_DRIFT_COOLDOWN_MS));
}
...
function registerStaleOnStoreDropCooldown(slug: string, reason: string, extras: Record<string, unknown> = {}): ProjectionPathologyCooldownResult {
    return registerPathologyCooldown(staleOnStoreDropCooldowns, `${slug}:onStoreDocument:${reason || 'unknown'}`, reason, buildStaleOnStoreDropSuppressionFingerprint(reason, extras), Date.now(), parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_DRIFT_COOLDOWN_MS, DEFAULT_STALE_ONSTORE_DRIFT_COOLDOWN_MS));
}
function maybeLogStaleOnStoreDropSuppressionSummary(slug: string, reason: string, suppressedCount: number): void {
    if (!shouldEmitSuppressionSummary(suppressedCount))
        return;
...
function logStaleOnStoreDocumentWriteDropped(slug: string, reason: 'invalidated_doc_reference' | 'doc_generation_mismatch' | 'superseded_doc_reference', details: Record<string, unknown> = {}): void {
...
function shouldSuppressProjectionDriftNoise(inMemoryMarkdown: string, persistedMarkdown: string): boolean {
...
async function registerGlobalCollabAdmissionGuard(reason: string, details?: Record<string, unknown>): Promise<void> {
    const now = Date.now();
    const cooldownMs = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_COOLDOWN_MS, DEFAULT_COLLAB_ADMISSION_GUARD_COOLDOWN_MS);
    const existing = (await getGlobalCollabAdmissionGuardEntry());
    const nextCount = (existing?.count ?? 0) + 1;
    const untilMs = now + cooldownMs;
    const shouldLog = !existing || existing.reason !== reason || untilMs > existing.untilMs;
    const scopedSlugs = (await snapshotGlobalCollabAdmissionGuardScopedSlugs(now));
    const requiredAdmissionEpoch = (await bumpGlobalCollabAdmissionEpoch());
    const scopeDetails = buildGlobalCollabAdmissionGuardScopeDetails(scopedSlugs);
    const fenceDetails = (await fenceScopedSlugsForGlobalCollabAdmissionGuard(scopedSlugs));
    ...
    (await upsertPersistedGlobalCollabAdmissionGuard(nextEntry));
...
async function registerAutoCollabQuarantine(slug: string, reason: string, details?: Record<string, unknown>): Promise<void> {
    const now = Date.now();
    const cooldownMs = parsePositiveInt(process.env.COLLAB_AUTO_QUARANTINE_COOLDOWN_MS, DEFAULT_COLLAB_AUTO_QUARANTINE_COOLDOWN_MS);
    ...
    (await setDocumentProjectionHealth(slug, 'quarantined', reason));
    if (!sameActiveReason) {
...
async function applyDurableCollabQuarantine(slug: string): Promise<{
    projectionQuarantined: boolean;
    accessEpoch: number | null;
}> {
    const projectionQuarantined = (await setDocumentProjectionHealth(slug, 'quarantined', 'COLLAB_AUTO_QUARANTINED'));
    const accessEpoch = (await bumpDocumentAccessEpoch(slug));
...
export async function activateDurableCollabQuarantine(slug: string, options: {
    reason: string;
    source?: string;
    details?: Record<string, unknown>;
}): Promise<{
    ...
    const projectionQuarantined = (await setDocumentProjectionHealth(slug, 'quarantined', options.reason));
    const accessEpoch = (await bumpDocumentAccessEpoch(slug));
    invalidateLoadedCollabDocument(slug);
    const result = {
        projectionQuarantined,
...
async function noteGlobalCollabAdmissionPathology(slug: string, resolution: Extract<StoreConflictResolution, {
    action: 'reload';
}>, context: {
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_WINDOW_MS, DEFAULT_COLLAB_ADMISSION_GUARD_WINDOW_MS);
    const maxEvents = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_MAX_EVENTS, DEFAULT_COLLAB_ADMISSION_GUARD_MAX_EVENTS);
    const maxSlugs = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_MAX_SLUGS, DEFAULT_COLLAB_ADMISSION_GUARD_MAX_SLUGS);
    const maxBytes = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_MAX_BYTES, DEFAULT_COLLAB_ADMISSION_GUARD_MAX_BYTES);
    const maxQuarantined = parsePositiveInt(process.env.COLLAB_ADMISSION_GUARD_MAX_QUARANTINED, DEFAULT_COLLAB_ADMISSION_GUARD_MAX_QUARANTINED);
    const now = Date.now();
    ...
    const activeQuarantined = (await countDurablyQuarantinedCollabDocuments());
    const multiSlugStorm = uniqueSlugs > 1;
    const activeGuard = (await getGlobalCollabAdmissionGuardEntry());
    if (activeGuard) {
        const requiredAdmissionEpoch = getGlobalCollabAdmissionGuardRequiredAdmissionEpoch(activeGuard);
        const slugBootstrapEpoch = getDocumentCollabBootstrapEpoch((await getDocumentBySlug(slug)));
    ...
    if (!guardReason)
        return { tripped: false };
    (await registerGlobalCollabAdmissionGuard(guardReason, {
...
function shouldCountTowardGlobalCollabAdmissionGuard(resolution: Extract<StoreConflictResolution, {
    action: 'reload';
}>, quarantineReason: string | null): boolean {
    if (quarantineReason)
        return true;
    if (resolution.reason === 'concurrent_external_edit')
        return true;
    if (resolution.reason === 'projection_drift_onstore_reload')
        return true;
...
function noteConcurrentExternalEditReload(slug: string): {
    ...
    const windowMs = parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_CONCURRENT_BREAKER_WINDOW_MS, DEFAULT_STALE_ONSTORE_CONCURRENT_BREAKER_WINDOW_MS);
    const max = parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_CONCURRENT_BREAKER_MAX, DEFAULT_STALE_ONSTORE_CONCURRENT_BREAKER_MAX);
    const now = Date.now();
...
async function maybeQuarantineStaleOnStoreReload(slug: string, resolution: Extract<StoreConflictResolution, {
    action: 'reload';
}>, context: {
    source: 'persistDoc' | 'onStoreDocument';
    sourceActor?: string;
    ...
    const dbMissingLimit = parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_DB_MISSING_QUARANTINE_BYTES, DEFAULT_STALE_ONSTORE_DB_MISSING_QUARANTINE_BYTES);
    const localUnsavedLimit = parsePositiveInt(process.env.COLLAB_STALE_ONSTORE_LOCAL_UNSAVED_QUARANTINE_BYTES, DEFAULT_STALE_ONSTORE_LOCAL_UNSAVED_QUARANTINE_BYTES);
    ...
    if (quarantineReason) {
        (await registerAutoCollabQuarantine(slug, quarantineReason, details));
        if (quarantineReason === 'stale_onstore_db_missing_oversized'
            || quarantineReason === 'stale_onstore_local_unsaved_oversized') {
    ...
    if (shouldCountTowardGlobalCollabAdmissionGuard(resolution, quarantineReason)) {
        (await noteGlobalCollabAdmissionPathology(slug, resolution, context));
    }
    if (quarantineReason) {
...
async function resolveOnStoreConflict(slug: string, _inMemoryDoc: Y.Doc): Promise<StoreConflictResolution> {
    const loadedMeta = loadedDocDbMeta.get(slug);
    const row = (await getDocumentBySlug(slug));
    const currentUpdatedAt = row?.updated_at ?? null;
    const currentYStateVersion = (await getLatestYStateVersion(slug));
    const currentAccessEpoch = typeof row?.access_epoch === 'number' ? row.access_epoch : null;
    if (currentAccessEpoch !== null && loadedMeta && loadedMeta.accessEpoch !== currentAccessEpoch) {
        const persistedState = (await readPersistedDocState(slug));
        return {
            action: 'reload',
            persistedState,
            ...
            currentUpdatedAt,
            loadedYStateVersion: loadedMeta?.yStateVersion ?? -1,
            currentYStateVersion,
            dbMissingBytes: 0,
            localUnsavedBytes: 0,
        };
    }
    // Same-epoch replicas retain their Yjs identities. The durable row-locked
    // persist transaction merges both histories before deriving the projection.
    return { action: 'persist' };
}

async function persistOnStoreDocument(slug: string, inMemoryDoc: Y.Doc, options: PersistDocOptions & {
    expectedGeneration?: number | null;
} = {}): Promise<void> {
    if (isCollabPersistenceReadOnly()) {
        if (!warnedReadOnlyPersistSlugs.has(slug)) {
    ...
    const resolution = (await resolveOnStoreConflict(slug, inMemoryDoc));
    ...
    if (resolution.action === 'reload') {
        ...
        try {
            ...
            if (currentRow) {
                ...
                if (reconciled.markdownChanged || reconciled.marksChanged) {
                    ...
                    const safety = evaluateProjectionSafety(currentRow.markdown ?? '', reconciled.markdown, resolution.persistedState.ydoc);
                    if (safety.safe) {
                        (await updateDocument(slug, reconciled.markdown, reconciled.marks, resolution.persistedState.yStateVersion));
                    }
                }
            }
        }
        ...
        const quarantine = (await maybeQuarantineStaleOnStoreReload(slug, resolution, { source: 'onStoreDocument' }));
        if (!resolution.logSuppressed) {
...
function isProsemirrorFragmentStructurallyEmpty(fragment: Y.XmlFragment | null | undefined): boolean {
    ...
    if (!first)
        return true;
    if (first.nodeName !== 'paragraph')
        return false;
    try {
        if (String(first) === '<paragraph></paragraph>')
            return true;
    }
    catch {
        // ignore
...
export function stripEphemeralCollabSpans(markdown: string): string {
    if (!markdown || markdown.indexOf('<span') === -1)
        return markdown;
...
export type CanonicalCollabSyncFailureReason = 'missing_slug' | 'missing_document' | 'live_doc_unretrievable' | 'parse_failed_live_doc' | 'fragment_unhealthy_content_write' | 'fragment_unhealthy_marks_only' | 'persist_only_reconcile_failed';
...
const canonicalApplyQueues = new Map<string, Promise<boolean>>();
async function maybePauseCanonicalSyncPreviewForTests(context: {
    slug: string;
    source: string;
    hasMarkdown: boolean;
    hasMarks: boolean;
}): Promise<void> {
    if (!canonicalSyncPreviewPauseHookForTests)
        return;
    await canonicalSyncPreviewPauseHookForTests(context);
}
function resolveCanonicalSyncLiveState(hasHocuspocusEntry: boolean, hadLoadedDoc: boolean): 'live_doc' | 'loaded_doc' | 'persisted_doc' {
    if (hasHocuspocusEntry)
        return 'live_doc';
    if (hadLoadedDoc)
        return 'loaded_doc';
    return 'persisted_doc';
}
function resolveCanonicalSyncLiveStateForSlug(slug: string): 'live_doc' | 'loaded_doc' | 'persisted_doc' {
    const instance = hocuspocusInstance as any;
    return resolveCanonicalSyncLiveState(Boolean(instance?.documents?.has?.(slug)), loadedDocs.has(slug));
}
async function fenceCanonicalSyncRecoveryFailure(slug: string): Promise<{
    projectionQuarantined: boolean;
    accessEpoch: number | null;
}> {
    const projectionQuarantined = projectionHealthWriteFailureForTests
        ? false
        : (await setDocumentProjectionHealth(slug, 'quarantined', 'canonical_sync_recovery_failed'));
    const accessEpoch = (await bumpDocumentAccessEpoch(slug));
    evictLocalDocState(slug);
    return {
        projectionQuarantined,
...
export async function reportCanonicalSyncRecoveryFailure(slug: string, options: {
    surface: string;
    route?: string;
    ...
    const fenced = slug
        ? (await fenceCanonicalSyncRecoveryFailure(slug)) : { projectionQuarantined: false, accessEpoch: null };
    ...
    recordCanonicalSyncRecoveryFailure(options.stage, options.surface, options.reason, containment, liveState);
    traceServerIncident({
        slug,
        subsystem: 'collab',
        level: 'error',
...
function buildCanonicalSyncFailureResult(slug: string, sourceActor: string, reason: CanonicalCollabSyncFailureReason, liveState: 'live_doc' | 'loaded_doc' | 'persisted_doc', details: Record<string, unknown> = {}): CanonicalCollabSyncResult {
    const incidentData = {
        source: sourceActor,
        reason,
        liveState,
...
async function syncCanonicalDocumentStateToCollabInner(slug: string, options: CanonicalCollabSyncOptions): Promise<CanonicalCollabSyncResult> {
    if (!slug) {
    ...
    if (canonicalSyncForcedRefusalForTests) {
        return buildCanonicalSyncFailureResult(slug, sourceActor, canonicalSyncForcedRefusalForTests, liveState, {
            forcedForTests: true,
            hasMarkdown,
            hasMarks,
        });
    ...
    if (hasHocuspocusEntry && !hocuspocusDoc) {
        ...
        await cleanup?.();
        return buildCanonicalSyncFailureResult(slug, sourceActor, 'live_doc_unretrievable', liveState, {
            hasMarkdown,
            hasMarks,
        });
    }
    ...
    if (!row) {
        await cleanup?.();
        return buildCanonicalSyncFailureResult(slug, sourceActor, 'missing_document', liveState, {
            hasMarkdown,
            hasMarks,
        });
    }
    ...
    try {
        if (hasMarkdown) {
            ...
            if (!parsedDoc) {
                if (hadLiveDoc) {
                    ...
                    return buildCanonicalSyncFailureResult(slug, sourceActor, 'parse_failed_live_doc', liveState, {
                        hasMarkdown,
                        hasMarks,
                    });
                }
                ...
                return reconciled
                    ? { applied: true }
                    : buildCanonicalSyncFailureResult(slug, sourceActor, 'persist_only_reconcile_failed', liveState, {
                        hasMarkdown,
                        hasMarks,
            ...
            if (derived.source !== 'fragment') {
                ...
                if (hadLiveDoc) {
                    ...
                    return buildCanonicalSyncFailureResult(slug, sourceActor, 'fragment_unhealthy_marks_only', liveState, {
                        hasMarkdown,
                        hasMarks,
                        fallbackReason: derived.fallbackReason ?? 'unknown',
                    });
        ...
        if (parsedDoc && sanitizedMarkdown !== null) {
            ...
            if (previewResolved.markdown === null || previewResolved.source !== 'fragment') {
                ...
                queueProjectionRepair(slug, 'derive_fragment_markdown_failed');
                return buildCanonicalSyncFailureResult(slug, sourceActor, 'fragment_unhealthy_content_write', liveState, {
                    hasMarkdown,
                    hasMarks,
                    fallbackReason: 'fragment_derive_failed',
                });
            }
            const suspiciousCollapse = evaluateNonDirtyFragmentRefreshCollapse(sanitizedMarkdown, previewResolved.markdown, sanitizedMarkdown);
            if (suspiciousCollapse.blocked) {
                await invalidateLoadedCollabDocumentAndWait(slug);
                queueProjectionRepair(slug, 'fragment_markdown_drift');
                return buildCanonicalSyncFailureResult(slug, sourceActor, 'fragment_unhealthy_content_write', liveState, {
...
export async function syncCanonicalDocumentStateToCollab(slug: string, options: CanonicalCollabSyncOptions): Promise<CanonicalCollabSyncResult> {
...
async function markProjectionStaleForLegacyReverseFlowBlock(slug: string, source: string, liveState: 'live_doc' | 'loaded_doc', options: {
    hasHocuspocusEntry: boolean;
    hadLoadedDoc: boolean;
}): Promise<boolean> {
    const alreadyQuarantined = (await getDocumentProjectionBySlug(slug))?.health === 'quarantined';
    const projectionMarkedStale = !alreadyQuarantined && (await setDocumentProjectionHealth(slug, 'projection_stale'));
...
function getLiveHocuspocusDoc(slug: string): Y.Doc | null {
    ...
    try {
        ...
        if (entry && typeof entry.getText === 'function')
            return entry as Y.Doc;
        const candidates = [
            entry?.document,
            entry?.ydoc,
            entry?.doc,
...
async function getOrLoadHocuspocusDoc(slug: string, options: {
    ...
    if (typeof openDirectConnection !== 'function')
        return { doc: null };
    const directConnectionTimeoutMs = parsePositiveInt(process.env.COLLAB_DIRECT_CONNECTION_TIMEOUT_MS, DEFAULT_DIRECT_CONNECTION_TIMEOUT_MS);
    ...
    try {
        const directPromise = Promise.resolve(openDirectConnection.call(instance, slug, { source: 'external-write' })) as Promise<DirectConnectionLike | null | undefined>;
    ...
    if (doc && typeof (doc as any).getText === 'function')
        return { doc, cleanup };
    if (direct)
        return { doc: null, cleanup };
    return { doc: null };
}
async function waitForLiveHocuspocusDocRegistration(slug: string): Promise<Y.Doc | null> {
    const timeoutMs = parsePositiveInt(process.env.COLLAB_LIVE_DOC_REGISTRATION_GRACE_MS, 1500);
    const pollMs = parsePositiveInt(process.env.COLLAB_LIVE_DOC_REGISTRATION_POLL_MS, 100);
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        const liveDoc = getLiveHocuspocusDoc(slug);
        if (liveDoc)
...
async function applyCanonicalDocumentToCollabInner(slug: string, options: CollabExternalApplyOptions): Promise<boolean> {
    if (!slug)
        return false;
    if (!runtime.enabled)
        return false;
    ...
    if (shouldBlockLegacyLiveApplySource(origin)) {
        ...
        const projectionMarkedStale = hadLiveDoc
            ? (await markProjectionStaleForLegacyReverseFlowBlock(slug, origin, liveState, {
                hasHocuspocusEntry,
                hadLoadedDoc,
            })) : false;
...
export async function refreshLoadedCollabMetaFromDb(slug: string): Promise<boolean> {
    const ydoc = getLiveHocuspocusDoc(slug) ?? loadedDocs.get(slug);
    if (!ydoc)
        return false;
    (await refreshLoadedDocDbMetaFromDb(slug, ydoc));
    return true;
}
export async function loadedCollabMarksMatch(slug: string, expectedMarks: Record<string, unknown>): Promise<boolean> {
    const ydoc = getLiveHocuspocusDoc(slug) ?? loadedDocs.get(slug);
    if (!ydoc)
        return false;
    try {
        const liveMarks = (await mergePreservedActionMarks(slug, encodeMarksMap(ydoc.getMap('marks')), {
            includeSuggestions: true,
        }));
...
export async function getLoadedCollabMarkdownForVerification(slug: string): Promise<{
    markdown: string | null;
    source: 'ytext' | 'fragment' | 'none';
}> {
    const ydoc = getLiveHocuspocusDoc(slug) ?? loadedDocs.get(slug);
...
export function applyAgentPresenceToLoadedCollab(slug: string, entry: Record<string, unknown>, activity?: Record<string, unknown>): boolean {
    if (!runtime.enabled)
        return false;
    const ydoc = getLiveHocuspocusDoc(slug) ?? loadedDocs.get(slug);
    if (!ydoc)
...
export function removeAgentPresenceFromLoadedCollab(slug: string, agentId: string, activity?: Record<string, unknown>): boolean {
    if (!runtime.enabled)
        return false;
    const normalizedAgentId = normalizeAgentScopedId(agentId);
    if (!normalizedAgentId)
...
export function applyAgentCursorHintToLoadedCollab(slug: string, hint: AgentCursorHint): boolean {
    if (!runtime.enabled)
        return false;
    const ydoc = getLiveHocuspocusDoc(slug) ?? loadedDocs.get(slug);
    if (!ydoc)
...
export async function applyCanonicalDocumentToCollab(slug: string, options: CollabExternalApplyOptions): Promise<boolean> {
    const prev = canonicalApplyQueues.get(slug) ?? Promise.resolve(true);
    const next = prev
        .catch(() => { })
        .then(() => applyCanonicalDocumentToCollabInner(slug, options));
    canonicalApplyQueues.set(slug, next);
    try {
        return await next;
    }
    finally {
        if (canonicalApplyQueues.get(slug) === next) {
            canonicalApplyQueues.delete(slug);
        }
    }
}
...
export async function applyCanonicalDocumentToCollabWithVerification(slug: string, options: CollabExternalApplyOptions, timeoutMs: number): Promise<CollabApplyVerificationResult> {
    if (!runtime.enabled) {
        return {
            applied: false,
            confirmed: true,
            reason: 'collab_disabled',
            yStateVersion: (await getLatestYStateVersion(slug)),
    ...
    let yStateVersion = (await getLatestYStateVersion(slug));
    if (!applied) {
        return {
            applied: false,
            confirmed: false,
    ...
    while (Date.now() <= deadline || timeoutMs <= 0) {
        ...
        fragmentConfirmed = (expectedFragmentTextHash !== null
            && liveFragmentTextHash !== null
    ...
    const reason = (() => {
        ...
        if (!hasLiveDoc && liveFragmentTextHash === null)
            return 'no_live_doc';
        if (!markdownConfirmed && !fragmentConfirmed)
            return 'markdown_fragment_mismatch';
        if (!markdownConfirmed)
            return 'markdown_mismatch';
        if (expectedFragmentTextHash === null)
            return 'expected_fragment_unavailable';
...
export async function verifyCanonicalDocumentInLoadedCollab(slug: string, options: CollabExternalApplyOptions, timeoutMs: number): Promise<CollabApplyVerificationResult> {
    if (!runtime.enabled) {
        return {
            applied: false,
            confirmed: true,
...
function evictStaleLocalStateForAccessEpoch(slug: string, accessEpoch: number): void {
    const loadedMeta = loadedDocDbMeta.get(slug);
    if (!loadedMeta || loadedMeta.accessEpoch === accessEpoch)
        return;
...
async function reconcileStaleProjectionsOnStartup(): Promise<void> {
    ...
    const staleDocs = (await listDocsWithStaleProjection(limit));
...
function scheduleStartupProjectionReconcile(): void {
    const enabled = parseBooleanFlag(process.env.COLLAB_STARTUP_RECONCILE_ENABLED, DEFAULT_STARTUP_STALE_PROJECTION_RECONCILE_ENABLED);
    if (!enabled) {
        console.log('[collab] startup stale projection reconcile disabled');
        return;
    }
    const delayMs = parsePositiveInt(process.env.COLLAB_STARTUP_RECONCILE_DELAY_MS, DEFAULT_STARTUP_STALE_PROJECTION_RECONCILE_DELAY_MS);
...
async function scanAndQueueSuspiciousProjectionRepairs(expectedGeneration: number = projectionRepairWorkerGeneration): Promise<void> {
    ...
    if (expectedGeneration !== projectionRepairWorkerGeneration)
        return;
    const limit = parsePositiveInt(process.env.COLLAB_PROJECTION_REPAIR_WORKER_LIMIT, DEFAULT_PROJECTION_REPAIR_WORKER_LIMIT);
    const minMarkdownChars = parsePositiveInt(process.env.COLLAB_PROJECTION_REPAIR_WORKER_MIN_CHARS, DEFAULT_PROJECTION_REPAIR_WORKER_MIN_CHARS);
    const oversizedCooldownMs = parsePositiveInt(process.env.COLLAB_PROJECTION_REPAIR_WORKER_OVERSIZED_COOLDOWN_MS, DEFAULT_PROJECTION_REPAIR_WORKER_OVERSIZED_COOLDOWN_MS);
    const candidates = (await listSuspiciousProjectionCandidates(limit, minMarkdownChars));
    if (expectedGeneration !== projectionRepairWorkerGeneration)
        return;
    ...
    for (const candidate of candidates) {
        if (expectedGeneration !== projectionRepairWorkerGeneration)
            return;
        if ((await isCollabQuarantined(candidate.slug))) {
            continue;
        }
        ...
        if (reasons.length > 0) {
            try {
                const handle = (await loadCanonicalYDocSync(candidate.slug));
                if (handle) {
...
function scheduleProjectionRepairWorker(initialDelayMs?: number, expectedGeneration: number = projectionRepairWorkerGeneration): void {
    if (expectedGeneration !== projectionRepairWorkerGeneration)
        return;
    const enabled = parseBooleanFlag(process.env.COLLAB_PROJECTION_REPAIR_WORKER_ENABLED, DEFAULT_PROJECTION_REPAIR_WORKER_ENABLED);
    if (!enabled) {
    ...
    const delayMs = initialDelayMs ?? parsePositiveInt(process.env.COLLAB_PROJECTION_REPAIR_WORKER_DELAY_MS, DEFAULT_PROJECTION_REPAIR_WORKER_DELAY_MS);
    const intervalMs = parsePositiveInt(process.env.COLLAB_PROJECTION_REPAIR_WORKER_INTERVAL_MS, DEFAULT_PROJECTION_REPAIR_WORKER_INTERVAL_MS);
    if (projectionRepairWorkerTimer) {
        clearTimeout(projectionRepairWorkerTimer);
        projectionRepairWorkerTimer = null;
    }
...
export async function reconcileCanonicalDocumentToYjs(slug: string, source: string = 'canonical-reconcile', options: {
    forcePersistOnly?: boolean;
} = {}): Promise<boolean> {
    if (!slug)
        return false;
    const doc = (await getDocumentBySlug(slug));
...
function evictIdleDocs(): void {
    ...
    for (const [slug, lastAccessedAt] of evictionCandidates) {
        ...
        if (!shouldEvictForIdle && !shouldEvictForCapacity)
            continue;
...
const docEvictionInterval = setInterval(evictIdleDocs, parsePositiveInt(process.env.COLLAB_DOC_EVICTION_INTERVAL_MS, DEFAULT_DOC_EVICTION_INTERVAL_MS));
if (typeof (docEvictionInterval as {
    unref?: () => void;
}).unref === 'function') {
    (docEvictionInterval as {
        unref: () => void;
    }).unref();
}
...
export async function buildCollabSession(slug: string, role: ShareRole, options?: {
...
}): Promise<CollabSessionInfo | null> {
    ...
    const liveCollabBlock = (await getLiveCollabBlockStatus(slug));
    if (liveCollabBlock.active) {
        const durationMs = Math.max(0, Date.now() - startedAtMs);
        const blockCode = liveCollabBlock.code ?? 'COLLAB_AUTO_QUARANTINED';
...
export function handleCollabWebSocketConnection(socket: unknown, request: unknown): void {
    ...
    if (!hocuspocusInstance || typeof hocuspocusInstance.handleConnection !== 'function') {
        try {
            (socket as {
                close?: (code?: number, reason?: string) => void;
            })?.close?.(1011, 'Collab runtime unavailable');
        }
        catch {
            // ignore
        }
...
export async function startCollabRuntime(mainHttpPort: number): Promise<CollabRuntime> {
    ...
    if (shouldAttachToMainHttpServer()) {
        ...
        traceCollabStartupIncident('warn', 'startup.degraded', runtime.reason ?? 'unknown', {
            mode: 'split-port',
        });
        return runtime;
    }
    ...
    try {
        ...
        const factory = (hocuspocusModule as unknown as {
            Server?: {
                configure?: (options: unknown) => HocuspocusInstance;
            };
        }).Server;
        ...
        hocuspocusInstance = factory.configure({
            name: 'proof-collab',
            extensions: await createSharedRedisExtensions(),
            port: collabPort,
            address: collabHost,
            ...
            async onLoadDocument(data: {
                ...
                if (!loadedDocs.has(slug)) {
                    ...
                    if (existing)
                        (await refreshLoadedDocDbMetaFromDb(slug, existing));
                }
            ...
            async onStoreDocument(data: {
                ...
                if (getContextAccessEpoch(data.context) === null && data.transactionOrigin !== REDIS_TRANSACTION_ORIGIN) {
                    // Server-origin transactions (e.g. projection refresh / canonical apply) persist explicitly.
                    return;
                }
                if (shouldDropWriteDuringShutdown(data.documentName, 'onStoreDocument'))
                    return;
                if ((await shouldDropStaleContextWrite(data.documentName, data.context, 'onStoreDocument'))) {
                    return;
                }
            ...
            async onChange(data: {
                ...
                if (shouldDropWriteDuringShutdown(data.documentName, 'onChange'))
                    return;
                if ((await shouldDropStaleContextWrite(data.documentName, data.context, 'onChange'))) {
                    return;
                }
                if (collabInvalidations.has(data.documentName)) {
            ...
            async onDisconnect(data: {
                ...
                (await detachAuthenticatedCollabPresence(data.context));
            },
        } as unknown);
        ...
        hocuspocusInstance = factory.configure({
            ...
            async onConnect(data: {
                ...
                const token = data.requestParameters.get('token')
                    || extractCollabTokenFromHeaders((data as unknown as {
                        requestHeaders?: unknown;
                    }).requestHeaders);
                try {
                    const auth = await authenticateCollabSession(data.documentName, token);
...
// ---------------------------------------------------------------------------
async function shouldFlushDocForShutdown(slug: string, ydoc: Y.Doc): Promise<boolean> {
    if (collabInvalidations.has(slug)) {
    ...
    if (isRewriteLocked(slug)) {
        ...
        (await maybeThrowOnDirtyShutdownGuard(slug, ydoc, 'rewrite_locked'));
        return false;
    }
    if ((await isCollabQuarantined(slug))) {
...
export async function flushAllDocumentsForShutdown(): Promise<void> {
    ...
    for (const [slug, ydoc] of loadedDocs.entries()) {
        try {
            if (!(await shouldFlushDocForShutdown(slug, ydoc)))
                continue;
            entries.push({
                slug,
                ydoc,
    ...
    if (failures.length > 0) {
        ...
        const aggregateError = new Error(`Failed to flush ${failures.length} collab document(s) during shutdown: ${failureMessages.join('; ')}`) as Error & {
            slugs?: string[];
        };
...
export async function stopCollabRuntime(options?: {
    skipDocFlush?: boolean;
}): Promise<void> {
    ...
    if (!options?.skipDocFlush) {
        try {
            ...
            const failedSlugs = Array.isArray((error as {
                slugs?: unknown;
            } | null | undefined)?.slugs)
                ? ((error as {
                    slugs: unknown[];
                }).slugs.filter((slug): slug is string => typeof slug === 'string' && slug.trim().length > 0))
                : [];
    ...
    if (shuttingDown && !flushError) {
        ...
        if (stuckSlugs.length > 0) {
            flushError = new Error(`Timed out waiting for in-flight collab persistence to drain before snapshot flush (${stuckSlugs.join(', ')})`) as Error & {
                slugs?: string[];
            };
            (flushError as Error & {
                slugs?: string[];
    ...
    await waitForPersistInFlightDrain(10000);
    loadedDocs.clear();
...
export function __unsafeSetActiveCollabConnectionCountOverrideForTests(override: (() => number) | null): void {
    activeCollabConnectionCountOverrideForTests = override;
}
export function __unsafeSetShutdownForceCloseHookForTests(hook: (() => void) | null): void {
...
export async function __unsafeMaybeQuarantineStaleOnStoreReloadForTests(slug: string, options: {
    reason?: 'access_epoch_mismatch' | 'projection_drift_onstore_reload' | 'concurrent_external_edit';
    dbMissingBytes?: number;
    localUnsavedBytes?: number;
    source?: 'persistDoc' | 'onStoreDocument';
    ...
    const ydoc = new Y.Doc();
    const authoritativeBaseline = buildAuthoritativeBaseline(ydoc);
    return (await maybeQuarantineStaleOnStoreReload(slug, {
        action: 'reload',
        persistedState: {
            ydoc,
            degradedReason: null,
...
export function __unsafeShouldSuppressProjectionDriftNoiseForTests(inMemoryMarkdown: string, persistedMarkdown: string): boolean {
    return shouldSuppressProjectionDriftNoise(inMemoryMarkdown, persistedMarkdown);
}
export async function __unsafeAuthenticateCollabSessionForTests(documentName: string, token: string): Promise<CollabAuthContext> {
...
export async function __unsafeReadPersistedDocStateSyncForTests(slug: string): Promise<PersistedDocState> {
    return (await readPersistedDocState(slug));
}
...
export async function __unsafeMaybeQuarantineCollabRepairLoopForTests(slug: string, options: {
    ...
    return (await maybeQuarantineCollabRepairLoop(slug, options.pathology ?? 'projection_guard_block', options.details));
}
export async function __unsafeMaybeQuarantineRepeatedRepairGuardBlockForTests(slug: string, options: {
    source?: ProjectionOperationSource;
    guardReason?: string;
    details?: Record<string, unknown>;
    extras?: Record<string, unknown>;
    ...
    return (await maybeQuarantineRepeatedRepairGuardBlock(slug, {
...
export async function __unsafeMaybeQuarantineRepeatedFragmentDriftForTests(slug: string, options: {
    source?: 'persist' | 'repair' | 'materialize';
    event?: 'persist_block' | 'repair_success' | 'projection_wipe';
    details?: Record<string, unknown>;
} = {}): Promise<{
    ...
    return (await maybeQuarantineRepeatedFragmentDrift(slug, {
        source: options.source ?? 'persist',
        event: options.event ?? 'persist_block',
        details: options.details,
    }));
}
export async function __unsafeMaybeQuarantineRepeatedLargeDocPathologyForTests(slug: string, options: {
    ...
    return (await maybeQuarantineRepeatedLargeDocPathology(slug, options.kind ?? 'pending_delta_clear', {
        source: options.source ?? 'test',
        details: options.details,
    }));
}
export async function __unsafeNoteDocumentIntegrityWarningForTests(slug: string, options: {
    actor?: string;
    revision?: number;
    ...
    return (await noteDocumentIntegrityWarning(slug, {
...
export function __unsafeSetCanonicalSyncForcedRefusalForTests(reason: CanonicalCollabSyncFailureReason | null = null): void {
...
export function __unsafeSetCanonicalSyncPreviewPauseHookForTests(hook: ((context: {
    slug: string;
    source: string;
    hasMarkdown: boolean;
    hasMarks: boolean;
}) => Promise<void> | void) | null = null): void {
...
export function __unsafeSetAutoCollabQuarantineForTests(slug: string, options: {
    reason?: string;
    durationMs?: number;
} = {}): void {
    const now = Date.now();
    autoCollabQuarantines.set(slug, {
        ...
        untilMs: now + Math.max(1, options.durationMs ?? 60000),
        triggeredAt: now,
        lastTriggeredAt: now,
        count: 1,
    });
...
export async function __unsafeSetGlobalCollabAdmissionGuardForTests(options: {
    reason?: string;
    durationMs?: number;
} = {}): Promise<void> {
    const now = Date.now();
    ...
    (await upsertPersistedGlobalCollabAdmissionGuard(globalCollabAdmissionGuard));
}
export async function __unsafeClearGlobalCollabAdmissionGuardForTests(): Promise<void> {
    globalCollabAdmissionEvents.length = 0;
    globalCollabAdmissionGuard = null;
    (await clearPersistedGlobalCollabAdmissionGuard());
}
...
export function __unsafeSetPersistPauseHookForTests(hook: ((context: {
    slug: string;
}) => Promise<void> | void) | null): void {
    persistPauseHookForTests = hook;
}
export async function __unsafePrimeLoadedDocForTests(slug: string, ydoc: Y.Doc): Promise<void> {
    rememberLoadedDoc(slug, ydoc);
    (await refreshLoadedDocDbMetaFromDb(slug, ydoc));
}
...
export function __unsafeAttachCollabSocketErrorHandlerForTests(socket: unknown, request: unknown, source: string): void {
...
async function logPendingYjsDeltaBeforeClear(slug: string, reason: string): Promise<void> {
    try {
        const gate = (await getCollabQuarantineGateStatus(slug));
        if (gate.active) {
            recordSuspiciousDocBlocked('pending_delta_clear', gate.reason ?? 'COLLAB_AUTO_QUARANTINED');
        }
        const latest = (await getLatestYUpdate(slug));
        if (!latest)
            return;
        const details = {
            reason,
        ...
        const quarantine = (await maybeQuarantineRepeatedLargeDocPathology(slug, 'pending_delta_clear', {
            source: 'invalidate',
            details,
        }));
...
function invalidateCollabDocumentInner(slug: string, options?: InvalidateCollabOptions): Promise<void> {
    const clearPersistedState = options?.clearPersistedState !== false;
    let current = invalidationOperations.get(slug);
    if (!current) {
        const operation = { promise: Promise.resolve(), clearPersistedState };
        invalidationOperations.set(slug, operation);
        operation.promise = performCollabInvalidation(slug, operation).finally(() => {
            if (invalidationOperations.get(slug) === operation) invalidationOperations.delete(slug);
        });
        current = operation;
    } else if (clearPersistedState) {
        current.clearPersistedState = true;
    }
    // A persistence callback cannot await its own drain. Its transaction observes
    // the generation fence; teardown still waits for it before completing.
    const self = persistOperationContext.getStore()?.operation;
    if (self && persistOperations.get(self) === slug) {
        void current.promise.catch(error => console.error('[collab] Deferred invalidation failed', { slug, error }));
        return Promise.resolve();
    }
    return current.promise;
}
async function performCollabInvalidation(slug: string, options: { clearPersistedState: boolean }): Promise<void> {
    if (!slug)
        return;
    const liveDocBeforeInvalidate = getLiveHocuspocusDoc(slug);
    const loadedDocBeforeInvalidate = loadedDocs.get(slug);
    if (liveDocBeforeInvalidate)
        invalidatedOnStoreDocRefs.add(liveDocBeforeInvalidate);
    if (loadedDocBeforeInvalidate)
        invalidatedOnStoreDocRefs.add(loadedDocBeforeInvalidate);
    clearProjectionRepairState(slug);
    skipOnStoreFingerprints.delete(slug);
    const nextPersistGeneration = cancelPendingPersistWork(slug, { advanceGeneration: true });
    ...
    // Generation is fenced synchronously before waiting, including preflight work.
    await Promise.allSettled([...persistOperations.entries()].filter(([, name]) => name === slug).map(([promise]) => promise));
    if (options.clearPersistedState) {
        try {
            (await logPendingYjsDeltaBeforeClear(slug, 'invalidate:pre'));
            (await clearYjsState(slug));
        }
        catch (error) {
    ...
    try {
        ...
        if (options.clearPersistedState) {
            try {
                (await logPendingYjsDeltaBeforeClear(slug, 'invalidate:post'));
                (await clearYjsState(slug));
            }
            catch (error) {
...
export async function invalidateCollabDocument(slug: string): Promise<void> {
...
export function invalidateLoadedCollabDocumentAndWait(slug: string): Promise<void> {
    return invalidateCollabDocumentInner(slug, { clearPersistedState: false });
}
