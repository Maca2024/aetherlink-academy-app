...
async function getCanonicalReadableDocument(slug: string) {
  return (await getCanonicalReadableDocumentSync(slug, 'state')) ?? (await getDocumentBySlug(slug));
}

type MutationReadyDocument = NonNullable<Awaited<ReturnType<typeof getCanonicalReadableDocument>>> & { plain_text?: string };
...
async function getMutationReadyDocument(
  slug: string,
  context?: AsyncDocumentMutationContext,
):
  Promise<| { doc: MutationReadyDocument; error: null }
  ...
  const doc = context?.doc ?? (await getCanonicalReadableDocument(slug));
  if (!doc) {
    return { doc: null, error: { status: 404, body: { success: false, error: 'Document not found' } } };
  }
  if (!isMutationReadyRead(doc)) {
    const fallbackDoc = (await getDocumentBySlug(slug));
...
async function resolveReadyDocument(
  slug: string,
  ...
  if (context && 'doc' in context) {
    return (await getMutationReadyDocument(slug, context));
  }
  if (context) {
    return { doc: context, error: null };
  }
  return (await getMutationReadyDocument(slug));
}
...
> {
  ...
  if (context?.mutationBase) {
    const persistedDoc = (await getDocumentBySlug(slug));
...
async function readState(slug: string): Promise<EngineExecutionResult> {
  const doc = (await getCanonicalReadableDocument(slug));
...
async function persistMarks(slug: string, marks: Record<string, StoredMark>, actor: string, eventType: string, eventData: JsonRecord): Promise<EngineExecutionResult> {
  const scrubbed = (await removeResurrectedMarksFromPayload(slug, marks as unknown as Record<string, unknown>));
...
async function persistMarksAsync(
  ...
  if (!shouldCommitCanonical) {
    if (hasPotentiallyLiveCollabDoc(slug)) {
      return (await persistMarksWithAuthoritativeSync(
        slug,
    ...
    return (await persistMarks(slug, normalizedMarks, actor, eventType, eventData));
  }
  ...
  const eventId = (await addDocumentEvent(
    slug,
    eventType,
    eventData,
    actor,
...
async function addComment(
  ...
  const ready = (await resolveReadyDocument(slug, context));
  ...
  return (await persistMarks(slug, marks, by, 'comment.added', { markId: id, by, quote, text }));
}
...
async function addCommentAsync(
  ...
  return (await persistMarksAsync(slug, doc, marks, by, 'comment.added', { markId: id, by, quote, text }, context));
}
...
async function addSuggestion(
  ...
  return (await persistMarks(slug, marks, by, `suggestion.${kind}.added`, {
    markId: id,
    by,
    quote,
...
async function updateSuggestionStatus(
  ...
  if (!existing) {
    ...
    if ((await shouldRejectMarkMutationByResolvedRevision(slug, markId, revisionHint))) {
      return {
        status: 409,
        body: {
          success: false,
  ...
  const ok = (await updateDocumentAtomic(
    slug,
    doc.updated_at,
    nextMarkdown,
...
async function addSuggestionAsync(
  ...
  if (!requestedStatus || requestedStatus === 'pending') {
    ...
    return (await persistMarksAsync(slug, doc, marks, by, `suggestion.${kind}.added`, {
      markId: id,
      by,
      quote,
      content: typeof body.content === 'string' ? body.content : undefined,
  ...
  (await upsertMarkTombstone(slug, id, 'accepted', mutation.document.revision));
  return {
    status: 200,
    body: {
      success: true,
...
async function updateSuggestionStatusAsync(
  ...
  if (!structuredResult.ok) {
    if (
      ...
      (await upsertMarkTombstone(slug, markId, status, mutation.document.revision));
...
async function resolveComment(
  ...
  const actor = typeof body.by === 'string' && body.by.trim() ? body.by.trim() : 'owner';
  const result = (await persistMarks(slug, marks, actor, 'comment.resolved', { markId, by: actor }));
  if (result.status >= 200 && result.status < 300) {
    const updated = (await getDocumentBySlug(slug));
    const resolvedRevision = typeof updated?.revision === 'number' ? updated.revision : (doc.revision + 1);
    (await upsertMarkTombstone(slug, markId, 'resolved', resolvedRevision));
  }
  return result;
}
...
async function unresolveComment(
  ...
  const actor = typeof body.by === 'string' && body.by.trim() ? body.by.trim() : 'owner';
  return (await persistMarks(slug, marks, actor, 'comment.unresolved', { markId, by: actor }));
}
...
async function unresolveCommentAsync(
  ...
  marks[markId] = { ...existing, resolved: false };
  const actor = typeof body.by === 'string' && body.by.trim() ? body.by.trim() : 'owner';
  return (await persistMarksAsync(slug, doc, marks, actor, 'comment.unresolved', { markId, by: actor }, context));
}
...
async function replyComment(
  ...
  return (await persistMarks(slug, marks, by, 'comment.replied', { markId, by, text }));
}
...
async function replyCommentAsync(
  ...
  const replies = [...baseReplies, { by, text, at: new Date().toISOString() }];
  marks[markId] = { ...existing, thread: replies, replies, threadId: existing.threadId ?? markId };
  return (await persistMarksAsync(slug, doc, marks, by, 'comment.replied', { markId, by, text }, context));
}
...
export async function executeDocumentOperation(
  ...
  if (method === 'GET' && routePath === '/state') return (await readState(slug));
  if (method === 'POST' && routePath === '/marks/comment') return (await addComment(slug, body));
  if (method === 'POST' && routePath === '/marks/suggest-replace') return (await addSuggestion(slug, body, 'replace'));
  if (method === 'POST' && routePath === '/marks/suggest-insert') return (await addSuggestion(slug, body, 'insert'));
  if (method === 'POST' && routePath === '/marks/suggest-delete') return (await addSuggestion(slug, body, 'delete'));
  if (method === 'POST' && routePath === '/marks/accept') return (await updateSuggestionStatus(slug, body, 'accepted'));
  if (method === 'POST' && routePath === '/marks/reject') return (await updateSuggestionStatus(slug, body, 'rejected'));
  if (method === 'POST' && routePath === '/marks/resolve') return (await resolveComment(slug, body));
  if (method === 'POST' && routePath === '/marks/unresolve') return (await unresolveComment(slug, body));
  if (method === 'POST' && routePath === '/marks/reply') return (await replyComment(slug, body));
...
export async function executeDocumentOperationAsync(
  ...
  if (method === 'POST' && routePath === '/marks/comment') {
    return (await addCommentAsync(slug, body, context));
  }
  if (method === 'POST' && routePath === '/marks/suggest-replace') {
    return (await addSuggestionAsync(slug, body, 'replace', context));
  }
  if (method === 'POST' && routePath === '/marks/suggest-insert') {
    return (await addSuggestionAsync(slug, body, 'insert', context));
  }
  if (method === 'POST' && routePath === '/marks/suggest-delete') {
    return (await addSuggestionAsync(slug, body, 'delete', context));
  }
  if (method === 'POST' && routePath === '/marks/accept') {
    return (await updateSuggestionStatusAsync(slug, body, 'accepted', context));
  }
  if (method === 'POST' && routePath === '/marks/reject') {
    return (await updateSuggestionStatusAsync(slug, body, 'rejected', context));
  }
  if (method === 'POST' && routePath === '/marks/resolve') {
    return (await resolveCommentAsync(slug, body, context));
  }
  if (method === 'POST' && routePath === '/marks/unresolve') {
    return (await unresolveCommentAsync(slug, body, context));
  }
  if (method === 'POST' && routePath === '/marks/reply') {
    return (await replyCommentAsync(slug, body, context));
  }
  return (await executeDocumentOperation(slug, method, routePath, body));
}
