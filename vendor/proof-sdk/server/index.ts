...
import { initializeDatabase, closeDatabase, getDb } from './db-postgres.js';
...
import { getCollabRuntime, startCollabRuntimeEmbedded, stopCollabRuntime } from './collab.js';
...
async function main(): Promise<void> {
  ...
  await getDb().transaction(async () => {
    await getDb().query("SELECT pg_advisory_xact_lock(hashtextextended('proof-mark-range-backfill', 0))");
    await runLegacyMarkRangeBackfillOnce();
  });
  const app = express();
...
<html lang="en">
  ...
  app.get('/health', async (_req, res) => {
    try { await getDb().query('SELECT 1'); } catch {
  ...
  const stop = async () => {
    ...
    try {
      await stopCollabRuntime();
      await closeDatabase();
      clearTimeout(deadline);
      process.exit(0);
    } catch {
      console.error('[proof-sdk] graceful shutdown failed');
      process.exit(1);
    }
  };
