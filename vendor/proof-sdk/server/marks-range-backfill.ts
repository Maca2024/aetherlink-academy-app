import { hasMaintenanceRun, listActiveDocuments, recordMaintenanceRun, updateMarks } from './db-postgres.js';
...
export async function backfillLegacyMarkRanges(): Promise<BackfillStats> {
  ...
  for (const doc of docs) {
    ...
    if (docChanged) {
      (await updateMarks(doc.slug, marks as Record<string, unknown>));
      stats.docsUpdated += 1;
    }
  }
...
export async function runLegacyMarkRangeBackfillOnce(): Promise<void> {
  if (process.env.SHARE_SKIP_MARK_RANGE_BACKFILL === '1') {
    console.log('[backfill] Skipping legacy mark range backfill (env override)');
    return;
  }

  if ((await hasMaintenanceRun(RUN_KEY))) {
    console.log('[backfill] Legacy mark range backfill already completed');
    return;
  }

  const stats = (await backfillLegacyMarkRanges());
  (await recordMaintenanceRun(RUN_KEY, stats));
