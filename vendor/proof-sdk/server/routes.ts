...
async function getAccessRole(req: Request, slug: string): Promise<ShareRole | null> {
  const secret = (await getPresentedSecret(req));
  if (secret) return (await resolveDocumentAccessRole(slug, secret));
...
async function resolveOpenContextAccess(
  ...
  const bearerToken = getPresentedBearerToken(req);
  const explicitResolved = explicitSecret ? (await resolveDocumentAccess(slug, explicitSecret)) : null;
  const bearerResolved = !explicitResolved && bearerToken ? (await resolveDocumentAccess(slug, bearerToken)) : null;
...
apiRoutes.post('/documents', async (req: Request, res: Response) => {
  ...
  const doc = (await createDocument(slug, sanitizedMarkdown, normalizedMarks, title, ownerId, ownerSecret));
  const defaultAccess = (await createDocumentAccessToken(slug, 'editor'));
  ...
  (await addEvent(slug, 'document.created', {
    title,
    ownerId,
...
apiRoutes.post('/documents/:slug/access-links', async (req: Request, res: Response) => {
  ...
  const doc = (await getCanonicalReadableDocumentSync(slug, 'share')) ?? (await getDocumentBySlug(slug));
  if (!doc) {
  ...
  const created = (await createDocumentAccessToken(slug, requestedRole));
...
export async function handleShareMarkdown(req: Request, res: Response): Promise<void> {
  ...
  const slug = generateSlug();
  const ownerSecret = randomUUID();
  const doc = (await createDocument(slug, sanitizedMarkdown, marks, title, ownerId, ownerSecret));
  const access = (await createDocumentAccessToken(slug, requestedRole));
  const links = buildShareLink(req, doc.slug);
...
apiRoutes.put('/documents/:slug/title', async (req: Request, res: Response) => {
  ...
  const accessRole = (await getAccessRole(req, slug));
  ...
  const writeSucceeded = (await updateDocumentTitle(slug, canonicalTitle));
  if (!writeSucceeded) {
    res.status(409).json({ error: 'Document changed during update; retry with latest state', code: 'STALE_BASE' });
    return;
  }

  const updatedDoc = (await getDocumentBySlug(slug));
  if (!updatedDoc) {
  ...
  (await addEvent(slug, 'document.title.updated', { actor }, actor || 'anonymous'));
...
apiRoutes.put('/documents/:slug', async (req: Request, res: Response) => {
  ...
  const currentDoc = hasMarkdownUpdate ? ((await getDocumentBySlug(slug)) ?? doc) : doc;
  let didUpdate = false;
  ...
  if (hasMarkdownUpdate) {
    ...
    writeSucceeded = (await updateMarks(slug, normalizedMarks ?? {}));
    if (writeSucceeded) {
      marksHandledDuringUpdate = true;
      const collabClientBreakdown = (await getActiveCollabClientBreakdown(slug));
      ...
      if (!syncResult?.applied) {
        const liveRoomAlreadyHasRequestedMarks = (await loadedCollabMarksMatch(slug, normalizedMarks ?? {}));
        ...
        if (preservedAuthoritativeMarks) {
          (await refreshLoadedCollabMetaFromDb(slug));
        } else {
          if (
          ...
          const rolledBack = (await updateMarks(slug, previousCanonicalMarks));
          if (!rolledBack) {
  ...
  if (hasTitleUpdate) {
    didUpdate = true;
    writeSucceeded = writeSucceeded && (await updateDocumentTitle(slug, normalizedTitle));
  }
  if (!didUpdate) {
  ...
  updatedDoc = (await getDocumentBySlug(slug));
  if (!updatedDoc) {
  ...
  for (const event of commentEvents) {
    (await addDocumentEvent(slug, event.type, event.data, event.actor));
  }
  (await addEvent(slug, 'document.updated', {
    actor: mutationActor,
    source: 'rest-put',
    integrity: {
      revision: updatedDoc.revision,
...
apiRoutes.post('/documents/:slug/ops', opsRateLimiter, async (req: Request, res: Response) => {
  ...
  if (idempotencyKey) {
    const existing = (await getStoredIdempotencyRecord(slug, routeKey, idempotencyKey));
    if (existing) {
  ...
  let rewriteGate: Awaited<ReturnType<typeof evaluateRewriteLiveClientGate>> | null = null;
  if (op === 'rewrite.apply') {
    ...
    rewriteGate = await evaluateRewriteLiveClientGate(slug, payload);
  ...
  if (idempotencyKey && result.status >= 200 && result.status < 300) {
    (await storeIdempotencyResult(slug, routeKey, idempotencyKey, result.body, requestHash, { statusCode: result.status }));
  }
...
apiRoutes.delete('/documents/:slug', async (req: Request, res: Response) => {
  ...
  (await revokeDocumentAccessTokens(slug, undefined, { bumpEpoch: false }));
  (await invalidateCollabDocument(slug));
  closeRoom(slug);
  (await addEvent(slug, 'document.deleted', {}, 'owner'));
  res.json({ success: true, shareState: 'DELETED', snapshotUrl: getSnapshotPublicUrl(slug) });
});

apiRoutes.post('/documents/:slug/pause', async (req: Request, res: Response) => {
  const slug = getSlugParam(req);
  if (!slug) {
    res.status(400).json({ error: 'Invalid slug' });
    return;
  ...
  (await pauseDocument(slug));
  (await invalidateLoadedCollabDocumentAndWait(slug));
  closeRoom(slug);
  (await addEvent(slug, 'document.paused', {}, 'owner'));
  (await refreshSnapshotForSlug(slug));
  res.json({ success: true, shareState: 'PAUSED', snapshotUrl: null });
});

apiRoutes.post('/documents/:slug/resume', async (req: Request, res: Response) => {
  const slug = getSlugParam(req);
  if (!slug) {
    res.status(400).json({ error: 'Invalid slug' });
    return;
  ...
  (await resumeDocument(slug));
  (await addEvent(slug, 'document.resumed', {}, 'owner'));
  (await refreshSnapshotForSlug(slug));
  res.json({ success: true, shareState: 'ACTIVE', snapshotUrl: getSnapshotPublicUrl(slug) });
});

apiRoutes.post('/documents/:slug/revoke', async (req: Request, res: Response) => {
  const slug = getSlugParam(req);
  if (!slug) {
    res.status(400).json({ error: 'Invalid slug' });
    return;
  ...
  (await revokeDocument(slug));
  (await revokeDocumentAccessTokens(slug, undefined, { bumpEpoch: false }));
  (await invalidateLoadedCollabDocumentAndWait(slug));
  closeRoom(slug);
  (await addEvent(slug, 'document.revoked', {}, 'owner'));
  (await refreshSnapshotForSlug(slug));
...
apiRoutes.get('/documents/:slug/info', async (req: Request, res: Response) => {
  const slug = getSlugParam(req);
  if (!slug) {
    res.status(400).json({ error: 'Invalid slug' });
    return;
...
apiRoutes.get('/documents/:slug/open-context', async (req: Request, res: Response) => {
  ...
  const session = (await buildCollabSession(slug, role, {
...
apiRoutes.get('/documents/:slug/collab-session', async (req: Request, res: Response) => {
  ...
  if (doc.share_state === 'PAUSED' || doc.share_state === 'REVOKED') {
    const role = (await getAccessRole(req, slug));
  ...
  const ownerAuthorized = canOwnerMutate(req, doc);
  const presentedSecret = (await getPresentedSecret(req));
  const access = presentedSecret ? (await resolveDocumentAccess(slug, presentedSecret)) : null;
  const requestedRole = access?.role ?? (await getAccessRole(req, slug));
