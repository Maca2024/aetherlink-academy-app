...
shareWebRoutes.get('/og/share/:slug.png', async (req: Request, res: Response) => {
  ...
  const doc = slug ? ((await getCanonicalReadableDocumentSync(slug, 'share')) ?? null) : null;
...
shareWebRoutes.get('/d/:slug', async (req: Request, res: Response) => {
  ...
  const roleFromQuery = slug && tokenFromQuery ? (await resolveDocumentAccessRole(slug, tokenFromQuery)) : null;
  const queryOwner = Boolean(doc && tokenFromQuery && canMutateByOwnerIdentity(doc, tokenFromQuery));
  const roleFromCookie = slug && tokenFromCookie ? (await resolveDocumentAccessRole(slug, tokenFromCookie)) : null;
  ...
  if (doc && doc.share_state !== 'ACTIVE' && !ownerOverride) {
    ...
    const snapshot = slug ? (await getSnapshotHtml(slug)) : null;
    if (snapshot) {
  ...
  if (!doc && slug) {
    ...
    if (!wantsJson(req)) {
      const snapshot = (await getSnapshotHtml(slug));
      if (snapshot) {
  ...
  if (
    ...
    (await noteDocumentLiveCollabLease(slug, doc.access_epoch));
