...
export async function getActiveCollabClientBreakdown(slug: string): Promise<ActiveCollabClientBreakdown> {
  const auth = await getDocumentAuthStateBySlug(slug);
  const accessEpoch = typeof auth?.access_epoch === 'number' ? auth.access_epoch : null;
  const exactEpochCount = await countActiveCollabConnections(slug, accessEpoch);
  const anyEpochCount = exactEpochCount > 0
    ? exactEpochCount
    : await countActiveCollabConnections(slug, null);
  const documentLeaseBreakdown = await getRecentDocumentLiveCollabLeaseBreakdown(slug, accessEpoch);
  const recentLeaseCount = await getRecentCollabSessionLeaseCount(slug, accessEpoch);
  return {
    slug,
    accessEpoch,
    exactEpochCount,
...
export async function getActiveCollabClientCount(slug: string): Promise<number> {
  ...
  return (await getActiveCollabClientBreakdown(slug)).total;
}
...
export function setupWebSocket(wss: WebSocketServer): void {
  wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
    ...
    deadline.unref();
    void acceptConnection(ws, req).catch(() => {
      try { ws.close(1011, 'Connection initialization failed'); } catch { /* socket already closed */ }
    }).finally(() => {
...
async function acceptConnection(ws: WebSocket, req: IncomingMessage): Promise<void> {
    ...
    if (isCollabConnection) {
      ...
      if (collabToken) {
        const claims = await getCollabSessionClaims(collabToken);
        if (!claims) {
        ...
        const doc = await getDocumentAuthStateBySlug(claims.slug);
        ...
        const liveCollabAllowed = !(await getLiveCollabBlockStatus(claims.slug)).active;
    ...
    const access = await resolveDocumentAccess(slug, token);
    if (!access) {
