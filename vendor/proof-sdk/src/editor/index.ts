...
class ProofEditorImpl implements ProofEditor {
  ...
  private pendingCollabMarkDeletions = new Set<string>();
  ...
  private async initFromShare(options?: ShareRuntimeActivationOptions): Promise<void> {
    ...
    try {
      ...
      if (collabSession && 'session' in collabSession && collabSession.session) {
        ...
        collabClient.onMarks((marks, deletedIds = []) => {
          const incomingMarks = (marks && typeof marks === 'object' && !Array.isArray(marks))
            ? (marks as Record<string, StoredMark>)
            : {};
          for (const id of deletedIds) this.pendingCollabMarkDeletions.add(id);
          for (const id of Object.keys(incomingMarks)) this.pendingCollabMarkDeletions.delete(id);
          ...
          let mergedIncomingMarks = mergePendingServerMarks(this.lastReceivedServerMarks, incomingMarks, { deletedIds });
          if (this.editor) {
            try {
              this.editor.action((ctx) => {
                const view = ctx.get(editorViewCtx);
                mergedIncomingMarks = mergePendingServerMarks(getMarkMetadata(view.state), incomingMarks, { deletedIds });
              });
            } catch (error) {
          ...
          this.initialMarksSynced = true;
          if ((Object.keys(mergedIncomingMarks).length > 0 || deletedIds.length > 0) && !this.isEditorDocStructurallyEmpty()) {
            this.applyLatestCollabMarksToEditor(deletedIds);
          }
        });
  ...
  private resetShareMarksSyncState(): void {
    ...
    this.pendingCollabMarkDeletions.clear();
  }
  ...
  applyExternalMarks(marks: Record<string, StoredMark>, deletedIds: readonly string[] = []): void {
    if (!this.editor) return;
    ...
    this.editor.action((ctx) => {
      ...
      applyRemoteMarks(view, marks, { hydrateAnchors: this.collabCanEdit, deletedIds });
    });
  }

  private applyLatestCollabMarksToEditor(deletedIds: readonly string[] = []): void {
    for (const id of deletedIds) this.pendingCollabMarkDeletions.add(id);
    if (!this.isShareMode || !this.collabEnabled || !this.editor) return;
    const removedIds = [...this.pendingCollabMarkDeletions];
    if (Object.keys(this.lastReceivedServerMarks).length === 0 && removedIds.length === 0) return;
    if (this.isEditorDocStructurallyEmpty()) return;

    this.applyingCollabRemote = true;
    this.suppressMarksSync = true;
    try {
      this.applyExternalMarks(this.lastReceivedServerMarks, removedIds);
      this.pendingCollabMarkDeletions.clear();
    } finally {
    ...
    if (this.isShareMode) {
      ...
      void shareClient.acceptSuggestion(markId, actor).then((result) => {
        ...
        if (this.editor) {
          this.editor.action((innerCtx) => {
            const innerView = innerCtx.get(editorViewCtx);
            applyRemoteMarks(innerView, serverMarks, { hydrateAnchors: this.collabCanEdit, deletedIds: [markId] });
  ...
  markAcceptAll(): number {
    ...
    if (this.isShareMode) {
      ...
      void (async () => {
        ...
        if (this.editor) {
          this.editor.action((innerCtx) => {
            const innerView = innerCtx.get(editorViewCtx);
            applyRemoteMarks(innerView, latestServerMarks!, { hydrateAnchors: this.collabCanEdit, deletedIds: confirmedAcceptedIds });
            const stats = getAuthorshipStats(innerView);
            this.bridge.authorshipStatsUpdated(stats);
          });
        }
    ...
    this.editor.action((ctx) => {
      ...
      if (count > 0 && this.isShareMode && acceptedIds.length > 0) {
        ...
        void (async () => {
          let latestServerMarks: Record<string, StoredMark> | null = null;
          const confirmedAcceptedIds: string[] = [];
          for (const suggestionId of acceptedIds) {
            const result = await shareClient.acceptSuggestion(suggestionId, actor);
            if (!result || 'error' in result || result.success !== true) continue;
            const serverMarks = (result.marks && typeof result.marks === 'object' && !Array.isArray(result.marks))
            ...
            if (!serverMarks) continue;
            latestServerMarks = serverMarks;
            confirmedAcceptedIds.push(suggestionId);
          }
          if (!latestServerMarks) return;
          this.lastReceivedServerMarks = { ...latestServerMarks };
          this.initialMarksSynced = true;
