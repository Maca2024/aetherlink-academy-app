...
type StoredMarkWithTarget = StoredMark & { target?: StoredAnchorTarget };

function buildNormalizedTargetText(text: string): {
  text: string;
  starts: number[];
  ends: number[];
} {
...
function resolveStoredMarkTargetRange(doc: ProseMirrorNode, stored: StoredMark): MarkRange | null {
  const target = (stored as StoredMarkWithTarget).target;
  if (!target || typeof target !== 'object' || typeof target.anchor !== 'string') return null;

  const canonicalTarget = canonicalizeAnchorTargetText(target);
  const index = buildTextIndex(doc);
  if (!index || !canonicalTarget.anchor) return null;

  const mode = canonicalTarget.mode ?? 'exact';
  let searchText = index.text;
  let anchor = canonicalTarget.anchor;
  let starts: number[] | null = null;
  let ends: number[] | null = null;
  if (mode === 'normalized') {
    const normalized = buildNormalizedTargetText(index.text);
    searchText = normalized.text;
    const normalizedAnchor = buildNormalizedTargetText(anchor);
    anchor = normalizedAnchor.text;
  ...
  while (searchFrom <= searchText.length) {
    const found = searchText.indexOf(anchor, searchFrom);
    if (found < 0) break;
    const end = found + anchor.length;
    const sourceStart = starts ? starts[found] : found;
    const sourceEnd = starts && ends ? ends[end - 1] : end;
    if (sourceStart !== undefined && sourceEnd !== undefined) {
      candidates.push({ searchStart: found, searchEnd: end, sourceStart, sourceEnd });
    }
    searchFrom = found + 1;
  }

  const contextBefore = typeof canonicalTarget.contextBefore === 'string'
    ? (mode === 'normalized' ? buildNormalizedTargetText(canonicalTarget.contextBefore).text : canonicalTarget.contextBefore)
    : '';
  const contextAfter = typeof canonicalTarget.contextAfter === 'string'
    ? (mode === 'normalized' ? buildNormalizedTargetText(canonicalTarget.contextAfter).text : canonicalTarget.contextAfter)
    : '';
  const contextual = candidates.filter((candidate) => {
    if (contextBefore && !searchText.slice(Math.max(0, candidate.searchStart - contextBefore.length - 240), candidate.searchStart).includes(contextBefore)) {
      return false;
    }
    if (contextAfter && !searchText.slice(candidate.searchEnd, candidate.searchEnd + contextAfter.length + 240).includes(contextAfter)) {
      return false;
    }
    return true;
  });
  let selected: (typeof contextual)[number] | null = null;
  if (canonicalTarget.occurrence !== undefined) {
    if (canonicalTarget.occurrence === 'first') {
      selected = contextual[0] ?? null;
    } else if (canonicalTarget.occurrence === 'last') {
      selected = contextual[contextual.length - 1] ?? null;
    } else if (Number.isInteger(canonicalTarget.occurrence) && canonicalTarget.occurrence >= 0) {
      selected = contextual[canonicalTarget.occurrence] ?? null;
    }
  } else if (contextual.length === 1) {
    selected = contextual[0];
  }
  if (!selected) return null;
  return mapTextOffsetsToRange(index, selected.sourceStart, selected.sourceEnd);
}

function resolveStoredMarkRange(doc: ProseMirrorNode, stored: StoredMark): MarkRange | null {
  if ((stored as StoredMarkWithTarget).target) {
    return resolveStoredMarkTargetRange(doc, stored);
  }
...
export function mergePendingServerMarks(
  ...
  for (const id of options?.deletedIds ?? []) delete merged[id];
  const now = Date.now();
...
export function applyRemoteMarks(
  view: EditorView,
  metadata: Record<string, StoredMark>,
  options?: { hydrateAnchors?: boolean; deletedIds?: readonly string[] }
): void {
  ...
  if (deletedIds.size > 0) {
    tr.doc.descendants((node, pos) => {
      ...
      for (const mark of node.marks) {
        if (deletedIds.has(mark.attrs.id)) tr = tr.removeMark(pos, pos + node.nodeSize, mark);
      }
    });
  }
