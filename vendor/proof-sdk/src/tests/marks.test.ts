...
test('applyRemoteMarks fails closed for an out-of-range target occurrence', () => {
  const markId = 'm-remote-target-occurrence-out-of-range';
  const doc = marksSchema.node('doc', null, [
    marksSchema.node('paragraph', null, marksSchema.text('Only once')),
  ]);
  const marksStatePlugin = new Plugin({
    key: marksPluginKey,
    state: {
  ...
  applyRemoteMarks(view, {
    [markId]: {
      ...
      status: 'pending' as const,
      quote: 'Only once',
      target: { anchor: 'Only once', mode: 'exact' as const, occurrence: 1 },
    },
  });

  assert(!getMarks(state).some(mark => mark.id === markId), 'Out-of-range target occurrence should not anchor a mark');
});
...
test('explicit remote deletions remove only acknowledged IDs and their anchors', () => {
  const deletedId = 'remote-deleted-suggestion';
  const localId = 'unsent-local-suggestion';
  const metadata: Record<string, StoredMark> = {
    [deletedId]: { kind: 'replace', by: 'ai:test', status: 'pending', content: 'accepted', quote: 'Remote', createdAt: '2026-09-13T00:00:00.000Z' },
    [localId]: { kind: 'replace', by: 'human:test', status: 'pending', content: 'local', quote: 'Local', createdAt: '2026-09-13T00:00:00.000Z' },
  };
  const merged = mergePendingServerMarks(metadata, {}, { deletedIds: [deletedId] });
  assert(!merged[deletedId], 'Explicit remote deletion removes acknowledged suggestion');
  assert(merged[localId], 'Missing unsent local suggestion must survive');
  const suggestion = marksSchema.marks.proofSuggestion;
  const doc = marksSchema.node('doc', null, [marksSchema.node('paragraph', null, [
    marksSchema.text('Remote', [suggestion.create({ id: deletedId, by: 'ai:test', kind: 'replace' })]),
    marksSchema.text(' '),
    marksSchema.text('Local', [suggestion.create({ id: localId, by: 'human:test', kind: 'replace' })]),
  ])]);
  const plugin = new Plugin({ key: marksPluginKey, state: {
    init: () => ({ metadata, activeMarkId: null }),
    apply: (tr, value) => {
      const change = tr.getMeta(marksPluginKey);
      return change?.type === 'SET_METADATA' ? { ...value, metadata: change.metadata } : value;
    },
  } });
  let state = EditorState.create({ schema: marksSchema, doc, plugins: [plugin] });
  const view = { get state() { return state; }, dispatch(tr: any) { state = state.apply(tr); } } as any;
  applyRemoteMarks(view, {}, { deletedIds: [deletedId] });
  assert(!getMarkMetadata(state)[deletedId], 'Remote deletion cannot resurrect from local metadata');
  assert(getMarkMetadata(state)[localId], 'Unacknowledged local metadata survives authoritative deletion');
  assert(!getMarks(state).some(mark => mark.id === deletedId), 'Deleted suggestion anchor is removed');
  assert(getMarks(state).some(mark => mark.id === localId), 'Unsent local anchor survives');
  assertEqual(state.doc.textContent, 'Remote Local', 'Removing metadata never changes document text');
});
