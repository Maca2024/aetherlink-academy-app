...
async function run(): Promise<void> {
  if (!process.env.DATABASE_URL) throw new Error('DATABASE_URL is required for the PostgreSQL anchor integration test');
  const schema = `proof_anchor_${randomUUID().replaceAll('-', '')}`;
  const prevSchema = process.env.PROOF_DATABASE_SCHEMA;
  ...
  const db = await import('../../server/db-postgres.ts');
  ...
  try {
    ...
    await db.createDocument(slug, '# Title\n\nHello world', {}, 'Suggestion anchor test');

    const missingReplace = await executeDocumentOperation(slug, 'POST', '/marks/suggest-replace', {
    ...
    const missingInsert = await executeDocumentOperation(slug, 'POST', '/marks/suggest-insert', {
      quote: 'Also missing',
      content: ' insert',
      by: 'ai:test',
    });
    ...
    const missingDelete = await executeDocumentOperation(slug, 'POST', '/marks/suggest-delete', {
      quote: 'Still missing',
      by: 'ai:test',
    });
    ...
    const missingComment = await executeDocumentOperation(slug, 'POST', '/marks/comment', {
    ...
    const missingSelectorComment = await executeDocumentOperation(slug, 'POST', '/marks/comment', {
    ...
    const validComment = await executeDocumentOperation(slug, 'POST', '/marks/comment', {
      quote: 'Hello world',
      text: 'Looks good',
      by: 'ai:test',
    });
    ...
    const validReplace = await executeDocumentOperation(slug, 'POST', '/marks/suggest-replace', {
      quote: 'Hello',
      content: 'Hi',
      by: 'ai:test',
    });
    ...
    await db.createDocument(markdownSlug, '- List item two with **bold text**', {}, 'Markdown anchor test');

    const markdownComment = await executeDocumentOperation(markdownSlug, 'POST', '/marks/comment', {
    ...
    const explicitTargetComment = await executeDocumentOperation(markdownSlug, 'POST', '/marks/comment', {
    ...
    const explicitTargetDoc = await db.getDocumentBySlug(markdownSlug);
    ...
    await db.createDocument(crossParagraphSlug, 'Context marker foo\n\nbar\n\nfoo bar', {}, 'Cross paragraph target test');
    const crossParagraphSuggestion = await executeDocumentOperation(crossParagraphSlug, 'POST', '/marks/suggest-replace', {
    ...
    await db.createDocument(htmlSlug, '<p>foo</p><p>bar</p>', {}, 'HTML anchor test');

    const htmlComment = await executeDocumentOperation(htmlSlug, 'POST', '/marks/comment', {
      quote: 'foo bar',
    ...
    await db.createDocument(taskSlug, '- [ ] Task one\n- [x] Task two', {}, 'Task list anchor test');

    const taskComment = await executeDocumentOperation(taskSlug, 'POST', '/marks/comment', {
      quote: 'Task one',
    ...
    await db.createDocument(emphasisSlug, '***bold italic***', {}, 'Nested emphasis anchor test');

    const emphasisComment = await executeDocumentOperation(emphasisSlug, 'POST', '/marks/comment', {
      quote: 'bold italic',
    ...
    await db.createDocument(authoredSlug, authoredMarkdown, {}, 'Authored span anchor test');

    const authoredSuggest = await executeDocumentOperation(authoredSlug, 'POST', '/marks/suggest-replace', {
    ...
    await db.createDocument(authoredAcceptedSlug, authoredMarkdown, {}, 'Authored span accepted suggestion test');
    ...
    await db.createDocument(rejectCycleSlug, rejectCycleMarkdown, {}, 'Reject cycle regression');

    const firstRejectSuggestion = await executeDocumentOperation(rejectCycleSlug, 'POST', '/marks/suggest-replace', {
    ...
    const firstRejectResult = await executeDocumentOperation(rejectCycleSlug, 'POST', '/marks/reject', { markId: firstRejectId, by: 'human:test' });
    assert(firstRejectResult.status === 200, `Expected first reject-cycle rejection to succeed, got ${firstRejectResult.status}`);
    const firstRejectedMarkdown = (await db.getDocumentBySlug(rejectCycleSlug))?.markdown ?? '';
    assert(
    ...
    const secondRejectSuggestion = await executeDocumentOperation(rejectCycleSlug, 'POST', '/marks/suggest-replace', {
      quote: '| 2. | Token tracking per plus1 | Med |',
    ...
    const secondRejectResult = await executeDocumentOperation(rejectCycleSlug, 'POST', '/marks/reject', { markId: secondRejectId, by: 'human:test' });
    assert(secondRejectResult.status === 200, `Expected second reject-cycle rejection to succeed, got ${secondRejectResult.status}`);
    const secondRejectedMarkdown = (await db.getDocumentBySlug(rejectCycleSlug))?.markdown ?? '';
    assert(
    ...
    async function suggestAndAccept(c: AcceptCase): Promise<void> {
      const slug = `accept-${c.kind}-${Math.random().toString(36).slice(2, 10)}`;
      await db.createDocument(slug, c.markdown, {}, c.name);
      ...
      const suggestResult = await executeDocumentOperation(slug, 'POST', suggestRoute, suggestBody);
      ...
      const acceptResult = await executeDocumentOperation(slug, 'POST', '/marks/accept', { markId, by: 'ai:test' });
    ...
    async function suggestAndAcceptExplicitTarget(c: ExplicitTargetAcceptCase): Promise<void> {
    ...
    for (const c of explicitTargetAcceptCases) {
      await suggestAndAcceptExplicitTarget(c);
    }

    const duplicateSlug = `anchor-dupe-${Math.random().toString(36).slice(2, 10)}`;
    await db.createDocument(duplicateSlug, '# Title\\n\\nRepeat\\n\\nRepeat', {}, 'Duplicate anchor test');

    const ambiguousSuggestion = await executeDocumentOperation(duplicateSlug, 'POST', '/marks/suggest-replace', {
    ...
    const disambiguatedSuggestion = await executeDocumentOperation(duplicateSlug, 'POST', '/marks/suggest-replace', {
    ...
    await db.createDocument(driftSlug, '# Title\n\nRepeat\n\nContext target\nRepeat', {}, 'Target drift test');
    const targetSuggestion = await executeDocumentOperation(driftSlug, 'POST', '/marks/suggest-replace', {
      target: { anchor: 'Repeat', occurrence: 1 },
      content: 'Changed',
      by: 'ai:test',
    });
    ...
    const driftDocBefore = await db.getDocumentBySlug(driftSlug);
    ...
    const driftUpdated = await mutateCanonicalDocument({
      slug: driftSlug,
      baseUpdatedAt: driftDocBefore.updated_at,
      nextMarkdown: driftUpdatedMarkdown,
      nextMarks: driftMarks,
      source: 'test-anchor-drift',
    });
    assert(driftUpdated.ok, 'Expected drift document setup update to succeed');
    const targetAccept = await executeDocumentOperationAsync(driftSlug, 'POST', '/marks/accept', { markId: targetMarkId, by: 'ai:test' });
    assert(targetAccept.status === 200, `Expected target drift accept to succeed, got ${targetAccept.status}: ${JSON.stringify(targetAccept.body)}`);
    assert(
      stripProofSpanTags(String(targetAccept.body.markdown ?? '')).trimEnd() === '# Title\n\nRepeat\n\nRepeat\n\nContext target\nChanged',
      `Expected accept to mutate the original targeted duplicate, got ${JSON.stringify(targetAccept.body.markdown)}`,
    );

    const driftAuthored = Object.values(targetAccept.body.marks ?? {}).find((mark: any) =>
      mark?.kind === 'authored' && mark.by === 'ai:test' && mark.quote === 'Changed') as any;
    assert(Boolean(driftAuthored), 'Expected accepted drift replacement to retain ai:test authorship');
    assert(typeof driftAuthored.range?.from === 'number' && driftAuthored.range.to > driftAuthored.range.from,
      'Expected accepted drift replacement authorship to retain a nonempty range');
    ...
    try {
      ...
      if (prevSchema === undefined) delete process.env.PROOF_DATABASE_SCHEMA;
      else process.env.PROOF_DATABASE_SCHEMA = prevSchema;
    }
